...
public class RequestAnswerJoinPledge extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (_response == 0) {
            request.cancel();
            requestor.sendPacket(new SystemMessage(SystemMsg.S1_DECLINED_YOUR_CLAN_INVITATION).addName(player));
            return;
        }
        ...
        try {
            ...
            final UnitMember member = new UnitMember(clan, player.getName(), player.getTitle(), player.getLevel(),
                    player.getPlayerClassComponent().getClassId().getId(), player.getObjectId(), pledgeType,
                    player.getPowerGrade(), player.getApprentice(), player.getSex().ordinal(), player.getPlayerTemplateComponent().getPlayerRace().ordinal(),
                    Clan.SUBUNIT_NONE);
