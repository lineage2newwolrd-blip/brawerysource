...
public class RequestExMagicSkillUseGround extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (skillEntry != null) {
            ...
            if(!GeoEngine.canSeeCoord(activeChar, _loc.getX(), _loc.getY(), _loc.getZ() + 72, false))
            {
                activeChar.sendPacket(SystemMsg.CANNOT_SEE_TARGET);
                activeChar.sendActionFailed();
                return;
            }
            if(Math.abs(GeoEngine.getHeight(_loc.getX(), _loc.getY(), _loc.getZ() + 48, activeChar.getGeoIndex()) - _loc.getZ())>72)
            {
                activeChar.sendPacket(SystemMsg.CANNOT_SEE_TARGET);
                activeChar.sendActionFailed();
                return;
