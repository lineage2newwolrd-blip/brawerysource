    ...
    protected void runImpl() {
        ...
        try {
                            ...
                            // Если энчант не обрабатывается берется вещь с наименьшим энчантом
                                ...
                                for (final ItemInstance item : list) {
                                    if (!items.contains(new ItemData(item.getItemId(), item.getCount(), item))
                                            && (itemToTake == null || item.getEnchantLevel() < itemToTake.getEnchantLevel())
                                            && !item.isEquipped()
                                            && !item.isShadowItem()
                                            && !item.isTemporalItem()
                                            && (!item.isAugmented() || AllSettingsConfig.ALT_ALLOW_DROP_AUGMENTED)
                                            && (item.getTemplate().isQuest() || ItemFunctions.checkIfCanDiscard(activeChar, item))) {
                                        itemToTake = item;
            ...
            for (final MultiSellIngredient in : entry.getProduction()) {
                ...
                } else if ((template = ItemTemplateHolder.getInstance().getTemplate(in.getItemId())).isStackable()) {
                    final long total = Math.multiplyExact(in.getItemCount(), _amount);
                    ItemFunctions.addItem(activeChar, in.getItemId(), total, true);
                }  else if (template.isTalisman() && template.isShadowItem() && activeChar.isTalismanStack() && !activeChar.getInventory().getItemsByItemId(in.getItemId()).isEmpty()) {
                    final ItemInstance product = activeChar.getInventory().getItemsByItemId(in.getItemId()).get(0);
                    final int total = Math.multiplyExact((int)in.getItemCount(), (int)_amount);
                    product.setLifeTime(product.getLifeTime() + total*template.getDurability());
                    product.setJdbcState(JdbcEntityState.UPDATED);
                    product.update();
                    activeChar.sendPacket(SystemMessage.obtainItems(in.getItemId(), total, 0));
                } else {
