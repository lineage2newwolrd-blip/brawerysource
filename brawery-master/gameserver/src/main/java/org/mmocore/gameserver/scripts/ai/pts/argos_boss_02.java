...
{
	public argos_boss_02(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (target.isPlayer())
					{
						...
						{
							GiveItem1(target, 7393, 1);
							SetFlagJournal(target, 71, 16);
							ShowQuestMark(target, 71);
							SoundEffect(target, "ItemSound.quest_middle");
							DeleteItem1(target, 7486, AiUtils.OwnItemCount(target, 7486));
							Say(AiUtils.MakeFString(7154, "", "", "", "", ""));
						}
						else
						{
							Say(AiUtils.MakeFString(7155, "", "", "", "", ""));
						}
					}
				}
			}
	...
	protected void onEvtSpawn()
	{
		...
		if (c0 !=null)
		{
			...
			if (c0.isPlayer())
			{
				Say(AiUtils.MakeFString(7153, c0.getName(), "", "", "", ""));
			}
		}
		super.onEvtSpawn();
	}
