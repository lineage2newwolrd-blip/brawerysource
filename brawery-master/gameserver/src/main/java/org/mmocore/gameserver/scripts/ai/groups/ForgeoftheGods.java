...
public class ForgeoftheGods extends Fighter {
    ...
    private static final long _activeStage = 300000L;
    ...
    protected boolean randomWalk() {
        return ArrayUtils.contains(RANDOM_SPAWN_MOBS, getActor().getNpcId()) || getActor().getNpcId() == TAR_BEETLE ? false : !getActor().isMoving && maybeMoveToHome();
    }

    @Override
    protected boolean maybeMoveToHome(){
        NpcInstance actor = getActor();
        if(actor.isInWater() && actor.getDistance(actor.getSpawnedLoc())>4000)
            teleportHome();
        return super.maybeMoveToHome();
    }
    ...
    {
        if(getActor().getNpcId() == TAR_BEETLE) {
            ThreadPoolManager.getInstance().schedule(new DecayTask(), _activeStage);
        }
        super.onEvtSpawn();
    }

    protected class DecayTask extends RunnableImpl
    {
        @Override
