...
public class GameTimeManager {
    ...
    private ScheduledFuture<?> onStartSpawnTask;
    ...
    private class OnStartListenerImpl implements OnStartListener {
        @Override
        public void onStart() {
            ThreadPoolManager.getInstance().scheduleAtFixedRate(new MinuteCounter(), 10000, 10000);
            if(ServerConfig.DELAYEDSPAWN)
                onStartSpawnTask = ThreadPoolManager.getInstance().schedule(new RunnableImpl() {
                    @Override
        ...
        public void run() {
            ...
            if (oldHour != newHour) {
                ...
                if (isNight != isNowNight()) {
                    if(onStartSpawnTask!=null) {
                        onStartSpawnTask.cancel(false);
                        onStartSpawnTask = null;
                    }
                    if (isNowNight()) {
                        listenerEngine.onNight();
                    } else {
