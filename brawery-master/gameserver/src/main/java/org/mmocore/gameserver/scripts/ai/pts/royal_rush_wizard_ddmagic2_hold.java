...
public class royal_rush_wizard_ddmagic2_hold extends wizard
{
	public int W_ShortRangeDDMagic = 272629761;
	public int W_LongRangeDDMagic = 272039937;

	public royal_rush_wizard_ddmagic2_hold(final NpcInstance actor){super(actor);}
			...
			if (i_ai0 == 0)
			{
				...
				if (DistFromMe(attacker) > 100 && AiUtils.Rand(100) < 80)
				{
					if (i0 == 1)
					{
						...
						{
							AddUseSkillDesire(attacker, W_LongRangeDDMagic, DesireType.ATTACK, DesireMove.STAND, 1000000);
						}
					}
				...
				{
					if (Skill_GetConsumeMP(W_ShortRangeDDMagic) < actor._currentMp && Skill_GetConsumeHP(W_ShortRangeDDMagic) < actor.currentHp && Skill_InReuseDelay(W_ShortRangeDDMagic) == 0)
					{
						AddUseSkillDesire(attacker, W_ShortRangeDDMagic, DesireType.ATTACK, DesireMove.STAND, 1000000);
					}
				}
				else if (AiUtils.Rand(100) < 2)
				{
	...
	protected void onEvtClanAttacked(Creature attacker, Creature victim, int damage, int skill_name_id, int skill_id)
	{
		...
		if ((GetLifeTime() > 7 && (attacker.isPlayer() || IsInCategory(Category.summon_npc_group, attacker.getClassId()))) && GetHateInfoCount() == 0)
		{
			...
			else if (Skill_GetConsumeMP(W_ShortRangeDDMagic) < actor._currentMp && Skill_GetConsumeHP(W_ShortRangeDDMagic) < actor.currentHp && Skill_InReuseDelay(W_ShortRangeDDMagic) == 0)
			{
				AddUseSkillDesire(attacker, W_ShortRangeDDMagic, DesireType.ATTACK, DesireMove.STAND, 1000000);
			}
		}
		...
		if (i0 == 1)
		{
			if (i_ai0 != 1)
			{
				if (DistFromMe(h0.attacker) > 100)
				{
					if (Skill_GetConsumeMP(W_LongRangeDDMagic) < actor._currentMp && Skill_GetConsumeHP(W_LongRangeDDMagic) < actor.currentHp && Skill_InReuseDelay(W_LongRangeDDMagic) == 0)
					{
						AddUseSkillDesire(h0.attacker, W_LongRangeDDMagic, DesireType.ATTACK, DesireMove.STAND, 1000000);
					}
				}
				else if (Skill_GetConsumeMP(W_ShortRangeDDMagic) < actor._currentMp && Skill_GetConsumeHP(W_ShortRangeDDMagic) < actor.currentHp && Skill_InReuseDelay(W_ShortRangeDDMagic) == 0)
				{
					AddUseSkillDesire(h0.attacker, W_ShortRangeDDMagic, DesireType.ATTACK, DesireMove.STAND, 1000000);
				}
			}
		}
	}
