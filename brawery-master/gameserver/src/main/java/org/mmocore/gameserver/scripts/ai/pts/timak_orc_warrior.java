...
public class timak_orc_warrior extends warrior_passive_casting_enchant_self
{
	public timak_orc_warrior(final NpcInstance actor){super(actor);}
			...
			switch (code_info.getCode())
			{
				...
				case 2:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						{
							c1.quest_last_reward_time = GetCurrentTick();
							if (AiUtils.HaveMemo(c1, 335)!=0)
							{
								if (AiUtils.OwnItemCount(c1, 3728)!=0 && AiUtils.OwnItemCount(c1, 3770) < 50)
								{
									if (AiUtils.Rand(100) < 95)
									{
										GiveItem1(c1, 3770, 1);
								...
								if (AiUtils.OwnItemCount(c1, 3696)!=0 && AiUtils.OwnItemCount(c1, 3717) < 20)
								{
									if (AiUtils.Rand(100) < 60)
									{
										GiveItem1(c1, 3717, 1);
										if (AiUtils.OwnItemCount(c1, 3717) >= 20)
										{
											SoundEffect(c1, "ItemSound.quest_middle");
										}
										else
								...
								if (AiUtils.OwnItemCount(c1, 3743)!=0 && AiUtils.OwnItemCount(c1, 3785) < 1)
								{
									if (AiUtils.Rand(10) < 2)
									{
										CreateOnePrivate(27158, "timak_raider_kaikee", 0, 1);
									}
								}
							}
						}
