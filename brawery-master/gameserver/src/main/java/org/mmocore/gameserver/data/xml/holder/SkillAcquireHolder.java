...
public final class SkillAcquireHolder extends AbstractHolder {
    ...
    public Collection<SkillLearn> getMaxSkills(Player player)
    {
        Collection<SkillLearn> skills = normalSkillTree.get(player.getPlayerClassComponent().getActiveClassId());
        Map<Integer, SkillLearn> skillLearnMap = new TreeMap<Integer, SkillLearn>();
        SkillEntry[] pSkills = player.getAllSkillsArray();

        SkillLearn max;
        int level = player.getLevel();
        for(SkillLearn s : skills)
            if(s.getMinLevel()<=level) {
                max = s;
                if (!skillLearnMap.containsKey(max.getId())) {

                    for (SkillLearn temp : skills)
                        if (max.getId() == temp.getId() && max.getLevel() <= temp.getLevel() && temp.getMinLevel() <= level)
                            max = temp;

                    boolean knownSkill = false;
                    for(int j = 0; j < pSkills.length && !knownSkill; j++)
                        if(pSkills[j].getId() == max.getId())
                        {
                            knownSkill = true;
                            if(max.getLevel() > pSkills[j].getLevel())
                                skillLearnMap.put(max.getId(), max);
                        }
