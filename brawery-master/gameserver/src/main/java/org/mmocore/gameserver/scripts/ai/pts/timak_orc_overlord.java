...
public class timak_orc_overlord extends warrior_ag_casting_enchant_1of4
{
	public timak_orc_overlord(final NpcInstance actor){super(actor);}
			...
			switch (code_info.getCode())
			{
				...
				case 2:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						if (AiUtils.HaveMemo(c1, 222)!=0 && AiUtils.OwnItemCount(c1, 2778) > 0 && AiUtils.OwnItemCount(c1, 2782) < 3 && AiUtils.GetMemoState(c1, 222) == 2)
						{
							i0 = AiUtils.GetMemoStateEx(c1, 222, 1);
							SetMemoStateEx(c1, 222, 1, i0 + 1);
							c1.quest_last_reward_time = GetCurrentTick();
							GiveItem1(c1, 2782, 1);
							if (AiUtils.OwnItemCount(c1, 2782) >= 3)
							{
								SoundEffect(c1, "ItemSound.quest_middle");
								if (i0 >= 5)
								{
				...
				case 3:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						{
							c1.quest_last_reward_time = GetCurrentTick();
							if (AiUtils.HaveMemo(c1, 335)!=0 && AiUtils.OwnItemCount(c1, 3728)!=0 && AiUtils.OwnItemCount(c1, 3770) < 50)
							{
								GiveItem1(c1, 3770, 1);
								if (AiUtils.OwnItemCount(c1, 3770) >= 50)
								{
									SoundEffect(c1, "ItemSound.quest_middle");
								}
								else
							...
							if (AiUtils.HaveMemo(c1, 335)!=0 && AiUtils.OwnItemCount(c1, 3760)!=0 && AiUtils.OwnItemCount(c1, 3802) < 1)
							{
								if (AiUtils.Rand(10) == 0)
								{
									CreateOnePrivate(27159, "timak_overlord_okun", 0, 1);
								}
							}
						}
					}
