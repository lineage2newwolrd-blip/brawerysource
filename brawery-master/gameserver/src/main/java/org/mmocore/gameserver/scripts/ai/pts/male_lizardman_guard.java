...
public class male_lizardman_guard extends warrior_aggressive
{
	public male_lizardman_guard(final NpcInstance actor){super(actor);}
			...
			switch (code_info.getCode())
			{
				case 0:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						if (i4 < 500)
						{
							if (AiUtils.OwnItemCount(target, 7179) + 1 >= 100)
							{
								if (AiUtils.OwnItemCount(target, 7179) < 100)
								{
									GiveItem1(target, 7179, 100 - AiUtils.OwnItemCount(target, 7179));
									SoundEffect(target, "ItemSound.quest_middle");
								}
				...
				case 1:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						if (i4 < 300)
						{
							if (AiUtils.OwnItemCount(target, 7180) + 1 >= 30)
							{
								if (AiUtils.OwnItemCount(target, 7180) < 30)
								{
									GiveItem1(target, 7180, 30 - AiUtils.OwnItemCount(target, 7180));
									SoundEffect(target, "ItemSound.quest_middle");
								}
