...
public class skull_animator extends wizard_ag_corpse_necro
{
	public skull_animator(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 637) == 1 && AiUtils.OwnItemCount(target, 8066) < 10)
		{
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			...
			{
				SetCurrentQuestID(637);
				target = code_info.RandomSelectOne();
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (AiUtils.Rand(100) < 92)
					{
						GiveItem1(target, 8066, 1);
						if (AiUtils.OwnItemCount(target, 8066) >= 9)
						{
							SetFlagJournal(target, 637, 2);
							ShowQuestMark(target, 637);
