...
public final class PetBabyInstance extends PetInstance {
    ...
    private static final int Weapon_Maintenance = 5987; // 1
    private static final int Armor_Maintenance = 5988; // 1
    // debuff (unused)
    @SuppressWarnings("unused")
    ...
    private static final SkillEntry[][] FAIRY_PRINCESS_BUFFS = {
            { SkillTable.getInstance().getSkillEntry(Pet_Empower, 3), SkillTable.getInstance().getSkillEntry(Pet_Wind_Walk, 2) },
            {
                    SkillTable.getInstance().getSkillEntry(Pet_Empower, 3),
                    SkillTable.getInstance().getSkillEntry(Pet_Blessed_Soul, 6),
    ...
    private static final SkillEntry[][] WHITE_WEASEL_BUFFS = {
            { SkillTable.getInstance().getSkillEntry(Pet_Wind_Walk, 2), SkillTable.getInstance().getSkillEntry(Pet_Blessed_Body, 6) },
            {
                    SkillTable.getInstance().getSkillEntry(Pet_Wind_Walk, 2),
                    SkillTable.getInstance().getSkillEntry(Pet_Blessed_Body, 6),
                    SkillTable.getInstance().getSkillEntry(Pet_Shield, 3)},
    ...
    private static final SkillEntry[][] TURTLE_ASCETIC_BUFFS = {
                    ...
                    SkillTable.getInstance().getSkillEntry(Pet_Shield, 3) },
            {
                    ...
                    SkillTable.getInstance().getSkillEntry(Pet_Blessed_Body, 6),
                    SkillTable.getInstance().getSkillEntry(Pet_Shield, 3),
                    SkillTable.getInstance().getSkillEntry(Armor_Maintenance, 1)},
            {
                    SkillTable.getInstance().getSkillEntry(Pet_Wind_Walk, 2),
                    SkillTable.getInstance().getSkillEntry(Pet_Blessed_Soul, 6),
                    SkillTable.getInstance().getSkillEntry(Pet_Blessed_Body, 6),
                    SkillTable.getInstance().getSkillEntry(Pet_Shield, 3),
                    SkillTable.getInstance().getSkillEntry(Armor_Maintenance, 1),
                    SkillTable.getInstance().getSkillEntry(Weapon_Maintenance, 1) }
    };
    ...
    public SkillEntry onActionTask() {
        try {
            if(!buffEnabled || isActionsDisabled())
                return null;

            final Player owner = getPlayer();
            if (!owner.isDead() && !owner.isInvul()) {
                if (getEffectList().getEffectsCountForSkill(5753) > 0) // Awakening
                {
                    return null;
                }

                final boolean improved = PetUtils.isImprovedBabyPet(getNpcId()) || PetUtils.isImprovedVitaminPet(getNpcId());
                ...
                if (!AllSettingsConfig.ALT_PET_HEAL_BATTLE_ONLY || owner.isInCombat()) {
                    ...
                    // проверка речарджа
                    if (skill == null && getNpcId() == PetId.IMPROVED_BABY_KOOKABURRA_ID || getNpcId() == PetId.FAIRY_PRINCESS_ID || getNpcId() == PetId.SPIRIT_SHAMAN_ID) // Речардж только у Kookaburra и в комбат моде
                    {
    ...
    public int getBuffLevel() {
        if (PetUtils.isVitaminPet(getNpcId())) {
