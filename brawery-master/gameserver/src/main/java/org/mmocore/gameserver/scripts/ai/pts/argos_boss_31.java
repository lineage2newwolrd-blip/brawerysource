...
{
	public argos_boss_31(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (target.isPlayer())
					{
						...
						{
							GiveItem1(target, 7422, 1);
							SetFlagJournal(target, 100, 16);
							ShowQuestMark(target, 100);
							SoundEffect(target, "ItemSound.quest_middle");
							DeleteItem1(target, 7515, AiUtils.OwnItemCount(target, 7515));
							Say(AiUtils.MakeFString(10054, "", "", "", "", ""));
						}
						else
						{
							Say(AiUtils.MakeFString(10055, "", "", "", "", ""));
						}
					}
				}
			}
	...
	protected void onEvtSpawn()
	{
		...
		if (c0 !=null)
		{
			...
			if (c0.isPlayer())
			{
				Say(AiUtils.MakeFString(10053, c0.getName(), "", "", "", ""));
			}
		}
		super.onEvtSpawn();
	}
