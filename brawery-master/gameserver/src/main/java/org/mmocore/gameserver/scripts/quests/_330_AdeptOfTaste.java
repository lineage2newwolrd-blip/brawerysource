...
public class _330_AdeptOfTaste extends Quest {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30469_03.htm") && _state == CREATED) {
            if (st.ownItemCount(Ingredient_List) == 0) {
            ...
            if (st.ownItemCount(White_Mandragora_Root) + st.ownItemCount(Red_Mandragora_Root) < 40) {
                return null;
            }
            Root2Sap(st, Red_Mandragora_Sap);
        } else if (event.equalsIgnoreCase("30067_05.htm") && _state == STARTED) {
            if (st.ownItemCount(Brown_Marsh_Moss) + st.ownItemCount(Green_Marsh_Moss) < 20) {
                return null;
            }
            Moss2Bundle(st, Green_Moss_Bundle);
        } else if (event.equalsIgnoreCase("30073_05.htm") && _state == STARTED) {
            if (st.ownItemCount(Nectar) < 20) {
                return null;
            }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        long ingredients_count = st.ownItemCount(ingredients);
        long spec_ingredients_count = st.ownItemCount(spec_ingredients);
        long all_ingredients_count = ingredients_count + spec_ingredients_count;
        boolean Has_Ingredient_List = st.ownItemCount(Ingredient_List) > 0;
        ...
        if (npcId == Jonas) {
            if (Has_Ingredient_List) {
                ...
                st.soundEffect(spec_ingredients_count == 4 ? SOUND_JACKPOT : SOUND_MIDDLE);
            ...
            if (all_ingredients_count == 0) {
                long Jonass_Steak_Dish_count = st.ownItemCount(Jonass_Steak_Dishes);
                long Miriens_Review_count = st.ownItemCount(Miriens_Reviews);
                if (Jonass_Steak_Dish_count > 0 && Miriens_Review_count == 0) {
                    return "30469_06.htm";
                }
                if (Jonass_Steak_Dish_count == 0 && Miriens_Review_count > 0) {
                    for (int i = Miriens_Reviews.length; i > 0; i--) {
                        if (st.ownItemCount(Miriens_Reviews[i - 1]) > 0) {
                            st.takeAllItems(Miriens_Reviews);
                            if (adena_rewards[i - 1] > 0) {
                                st.giveReward(ADENA_ID, adena_rewards[i - 1], true);
                            }
                            if (rewards[i - 1] > 0) {
        ...
        if (npcId == Mirien) {
            ...
            if (all_ingredients_count == 0) {
                if (st.ownItemCount(Miriens_Reviews) > 0) {
                    return "30461_04.htm";
                }
                for (int i = Jonass_Steak_Dishes.length; i > 0; i--) {
                    if (st.ownItemCount(Jonass_Steak_Dishes[i - 1]) > 0) {
        ...
        if (npcId == Sonia) {
            boolean has_sap = st.ownItemCount(Red_Mandragora_Sap) > 0 || st.ownItemCount(White_Mandragora_Sap) > 0;
            if (st.ownItemCount(Sonias_Botany_Book) > 0) {
                if (!has_sap) {
                    long Root_count = st.ownItemCount(White_Mandragora_Root);
                    if (Root_count >= 40) {
                        Root2Sap(st, White_Mandragora_Sap);
                        return "30062_06.htm";
                    }
                    Root_count += st.ownItemCount(Red_Mandragora_Root);
        ...
        if (npcId == Glyvka) {
            boolean has_bundle = st.ownItemCount(Green_Moss_Bundle) > 0 || st.ownItemCount(Brown_Moss_Bundle) > 0;
            if (st.ownItemCount(Glyvkas_Botany_Book) > 0) {
                if (!has_bundle) {
                    long moss_count = st.ownItemCount(Brown_Marsh_Moss);
                    if (moss_count >= 20) {
                        Moss2Bundle(st, Brown_Moss_Bundle);
                        return "30067_06.htm";
                    }
                    moss_count += st.ownItemCount(Green_Marsh_Moss);
        ...
        if (npcId == Rollant) {
            boolean has_meat = st.ownItemCount(Meat_of_Monster_Eye) > 0;
            if (st.ownItemCount(Rollants_Creature_Book) > 0) {
                if (!has_meat) {
                    if (st.ownItemCount(Body_of_Monster_Eye) < 30) {
                        return "30069_02.htm";
                    }
        ...
        if (npcId == Jacob) {
            boolean has_honey = st.ownItemCount(Honey) > 0 || st.ownItemCount(Golden_Honey) > 0;
            if (st.ownItemCount(Jacobs_Insect_Book) > 0) {
                if (!has_honey) {
                    if (st.ownItemCount(Nectar) < 20) {
                        return "30073_02.htm";
                    }
                    if (st.ownItemCount(Royal_Jelly) < 10) {
                        return "30073_03.htm";
                    }
        ...
        if (npcId == Pano) {
            boolean has_potato = st.ownItemCount(Dionian_Potato) > 0;
            if (st.ownItemCount(Panos_Contract) > 0) {
                if (!has_potato) {
                    if (st.ownItemCount(Hobgoblin_Amulet) < 30) {
                        return "30078_02.htm";
                    }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Hobgoblin && st.ownItemCount(Panos_Contract) > 0) {
            st.rollAndGive(Hobgoblin_Amulet, 1, 1, 30, 100);
        } else if (npcId == Mandragora_Sprout && st.ownItemCount(Sonias_Botany_Book) > 0) {
            MandragoraDrop(st, 70, 77);
        } else if (npcId == Mandragora_Sapling && st.ownItemCount(Sonias_Botany_Book) > 0) {
            MandragoraDrop(st, 77, 85);
        } else if (npcId == Mandragora_Blossom && st.ownItemCount(Sonias_Botany_Book) > 0) {
            MandragoraDrop(st, 87, 96);
        } else if (npcId == Mandragora_Sprout2 && st.ownItemCount(Sonias_Botany_Book) > 0) {
            MandragoraDrop(st, 70, 77);
        } else if (npcId == Bloody_Bee && st.ownItemCount(Jacobs_Insect_Book) > 0) {
            BeeDrop(st, 80, 95);
        } else if (npcId == Stinger_Wasp && st.ownItemCount(Jacobs_Insect_Book) > 0) {
            BeeDrop(st, 92, 100);
        } else if (npcId == Gray_Ant && st.ownItemCount(Glyvkas_Botany_Book) > 0) {
            AntDrop(st, 87, 96);
        } else if (npcId == Giant_Crimson_Ant && st.ownItemCount(Glyvkas_Botany_Book) > 0) {
            AntDrop(st, 90, 100);
        } else if (npcId == Monster_Eye_Searcher && st.ownItemCount(Rollants_Creature_Book) > 0) {
            st.rollAndGive(Body_of_Monster_Eye, 1, 3, 30, 97);
        } else if (npcId == Monster_Eye_Gazer && st.ownItemCount(Rollants_Creature_Book) > 0) {
