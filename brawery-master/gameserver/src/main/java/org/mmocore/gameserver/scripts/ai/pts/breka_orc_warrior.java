...
public class breka_orc_warrior extends warrior_aggressive_physicalspecial
{
	public breka_orc_warrior(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 335)!=0 && AiUtils.OwnItemCount(target, 3695)!=0 && AiUtils.OwnItemCount(target, 3711) + AiUtils.OwnItemCount(target, 3712) + AiUtils.OwnItemCount(target, 3713) < 3)
		{
			...
			switch (code_info.getCode())
			{
				case 0:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						{
							if (AiUtils.OwnItemCount(c1, 3341) >= 29)
							{
								GiveItem1(c1, 3341, 1);
			...
			switch (code_info.getCode())
			{
				case 2:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						{
							if (AiUtils.OwnItemCount(target, 3711) == 0)
							{
								CreateOnePrivate(27140, "breka_overlord_haka", 0, 1);
							}
							else if (AiUtils.OwnItemCount(target, 3712) == 0)
							{
								CreateOnePrivate(27141, "breka_overlord_jaka", 0, 1);
							}
							else if (AiUtils.OwnItemCount(target, 3713) == 0)
							{
								CreateOnePrivate(27142, "breka_overlord_marka", 0, 1);
							}
						}
					}
					break;
