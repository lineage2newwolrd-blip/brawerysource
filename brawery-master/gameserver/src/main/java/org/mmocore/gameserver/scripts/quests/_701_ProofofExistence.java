...
public class _701_ProofofExistence extends Quest {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == warmage_artius) {
            ...
            st.giveReward(ADENA_ID, st.ownItemCount(q_undead_bone_piece) * 2500, true);
            ...
            st.giveReward(ADENA_ID, st.ownItemCount(q_undead_bone_piece) * 2500 + st.ownItemCount(q_banshee_eyeball) * 50000 + 23835, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (GetMemoState == 1 && npcId == floating_skeleton_1lv) {
            int i0 = Rnd.get(1000);
            if (i0 < 518) {
                st.giveItems(q_undead_bone_piece, 1, true);
            ...
            int i4 = Rnd.get(1000);
            if (i4 < 708) {
                st.giveItems(q_banshee_eyeball, Rnd.get(2) + 1, true);
                st.soundEffect(SOUND_ITEMGET);
            } else if (i4 < 978) {
                st.giveItems(q_banshee_eyeball, Rnd.get(3) + 3, true);
                st.soundEffect(SOUND_ITEMGET);
            } else if (i4 < 994) {
                st.giveItems(q_banshee_eyeball, Rnd.get(4) + 6, true);
                st.soundEffect(SOUND_ITEMGET);
            } else if (i4 < 998) {
                st.giveItems(q_banshee_eyeball, Rnd.get(4) + 10, true);
                st.soundEffect(SOUND_ITEMGET);
            } else if (i4 < 1000) {
                st.giveItems(q_banshee_eyeball, Rnd.get(5) + 14, true);
                st.soundEffect(SOUND_ITEMGET);
            }
        }
        return null;
