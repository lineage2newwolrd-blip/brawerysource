...
import static org.mmocore.gameserver.configuration.config.AllSettingsConfig.ALT_PARTY_DISTRIBUTION_RANGE;
...
public class Party implements PlayerGroup {
    ...
    public boolean removePartyMember(final Player player, final boolean kick) {
        ...
        else if(isInReflection() && getReflection() instanceof DelusionChamber) {
            ((DelusionChamber) getReflection()).partyMemberExited(player);
        }
        else if (isInReflection() && getReflection() instanceof DimensionalRift) {
            ((DimensionalRift) getReflection()).partyMemberExited(player);
        }
        if(reflection != null && player.getReflection() == reflection &&(reflection instanceof DarknessFestival||reflection instanceof DimensionalRift)&& reflection.getReturnLoc() != null)
        ...
        switch (itemDistribution) {
            ...
            case ITEM_RANDOM_SPOIL: {
	            ...
	            for (Player member : members)
		            if (validLootMember(player, member, item, fromNpc))
			            ret.add(member);
            ...
            case ITEM_ORDER_SPOIL: {
                ...
                while (target == null && !ret.isEmpty()) {
                    ...
                    if (looterPlayer != null) {
	                    if (!looterPlayer.isDead() && looterPlayer.isInRangeZ(player, ALT_PARTY_DISTRIBUTION_RANGE) &&
	...
	private boolean validLootMember(Player actor, Player member, ItemInstance item, NpcInstance npc) {
		if (member != actor && !member.isInRangeZ(actor, ALT_PARTY_DISTRIBUTION_RANGE))
			return false;

		if (!ItemFunctions.canAddItem(member, item))
			return false;
	...
	public void distributeXpAndSp(final double xpReward, final double spReward, final List<Player> rewardedMembers, final Creature lastAttacker, final MonsterInstance monster) {
        ...
        for (final Player member : rewardedMembers) {
	        if (!monster.isInRangeZ(member, ALT_PARTY_DISTRIBUTION_RANGE)) {
		        continue;
	        }
