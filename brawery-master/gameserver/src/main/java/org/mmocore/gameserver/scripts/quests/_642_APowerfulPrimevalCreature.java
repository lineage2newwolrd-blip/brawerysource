...
public class _642_APowerfulPrimevalCreature extends Quest {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        long Dinosaur_Tissue_Count = st.ownItemCount(Dinosaur_Tissue);
        if (event.equalsIgnoreCase("dindin_q0642_04.htm") && _state == CREATED) {
            ...
            st.giveReward(ADENA_ID, Dinosaur_Tissue_Count * 3000, true);
            ...
            try {
                ...
                if (Dinosaur_Tissue_Count < 150 || st.ownItemCount(Dinosaur_Egg) == 0) {
                    return "dindin_q0642_08a.htm";
                }
                for (int reward : Rewards) {
                    if (reward == rew_id) {
                        ...
                        st.giveReward(reward, 1, true);
                        st.giveReward(ADENA_ID, 44000, true);
                        st.soundEffect(SOUND_MIDDLE);
