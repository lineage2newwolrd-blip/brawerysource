...
public class _225_TestOfTheSearcher extends Quest {
    ...
    public _225_TestOfTheSearcher() {
        ...
        addLevelCheck(39);
        addClassIdCheck(ClassId.rogue, ClassId.elven_scout, ClassId.assassin, ClassId.scavenger);
    }
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30690-05.htm")) {
            ...
            if (isQuest == null) {
                if (st.ownItemCount(RustedKey) == 0) {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        int cond = st.getCond();
        ClassId classId = st.getPlayer().getPlayerClassComponent().getClassId();
        if (npcId == Luther) {
            if (st.ownItemCount(MarkOfSearcher) != 0) {
                htmltext = "completed";
                st.exitQuest(true);
            } else if (cond == 0) {
                ...
                if (!st.getPlayer().getPlayerVariables().getBoolean(PlayerVariables.PROF2_3)) {
                    ...
                    st.giveReward(ADENA_ID, 160186, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 5 && npcId == DeluChiefKalkis) {
            if (st.ownItemCount(StrangeMap) == 0) {
                st.giveItems(StrangeMap, 1);
            }
            if (st.ownItemCount(ChiefKalkisFang) == 0) {
            ...
            if (npcId == RoadScavenger && st.ownItemCount(SoltsMap) == 0) {
                st.giveItems(TornMapPiece1st, 1);
                if (st.ownItemCount(TornMapPiece1st) >= 4) {
                    st.takeItems(TornMapPiece1st, -1);
                    st.giveItems(SoltsMap, 1);
                }
            } else if (npcId == HangmanTree && st.ownItemCount(MakelsMap) == 0) {
                st.giveItems(TornMapPiece2st, 1);
                if (st.ownItemCount(TornMapPiece2st) >= 4) {
                    st.takeItems(TornMapPiece2st, -1);
                    st.giveItems(MakelsMap, 1);
                }
            }
            if (st.ownItemCount(SoltsMap) != 0 && st.ownItemCount(MakelsMap) != 0) {
                st.setCond(15);
                st.setState(STARTED);
            }
        }
