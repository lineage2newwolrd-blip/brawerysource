    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == malruk_succubus || npcId == malruk_succubus_hold) {
            if (GetMemoState == 5 * 10 + 1) {
                int i4 = Rnd.get(1000);
                if (i4 < 450) {
                    if (st.ownItemCount(q_nobelesse_mark_3) + 1*st.getRateQuestsDrop() >= 10) {
                        if (st.ownItemCount(q_nobelesse_mark_3) < 10) {
                            st.setCond(7);
                        ...
                        st.giveItems(q_nobelesse_mark_3, 1, true);
            ...
            if (GetMemoState == 12 * 10 + 1) {
                int i4 = Rnd.get(1000);
                if (i4 < 300) {
                    if (st.ownItemCount(q_nobelesse_mark_4) + 1*st.getRateQuestsDrop() >= 5) {
                        if (st.ownItemCount(q_nobelesse_mark_4) < 5) {
                            st.setCond(15);
                        ...
                        st.giveItems(q_nobelesse_mark_4, 1, true);
                        st.soundEffect(SOUND_ITEMGET);
                    }
                }
            }
