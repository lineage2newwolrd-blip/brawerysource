...
{
	public argos_boss_24(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (target.isPlayer())
					{
						...
						{
							GiveItem1(target, 7415, 1);
							SetFlagJournal(target, 93, 16);
							ShowQuestMark(target, 93);
							SoundEffect(target, "ItemSound.quest_middle");
							DeleteItem1(target, 7508, AiUtils.OwnItemCount(target, 7508));
							Say(AiUtils.MakeFString(9354, "", "", "", "", ""));
						}
						else
						{
							Say(AiUtils.MakeFString(9355, "", "", "", "", ""));
						}
					}
				}
			}
	...
	protected void onEvtSpawn()
	{
		...
		if (c0 !=null)
		{
			...
			if (c0.isPlayer())
			{
				Say(AiUtils.MakeFString(9353, c0.getName(), "", "", "", ""));
			}
		}
		super.onEvtSpawn();
	}
