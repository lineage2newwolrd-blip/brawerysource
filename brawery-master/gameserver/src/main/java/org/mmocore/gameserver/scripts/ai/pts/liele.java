...
public class liele extends warrior_passive_casting_ddmagic
{
	public liele(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 335)!=0 && AiUtils.OwnItemCount(target, 3755)!=0 && AiUtils.OwnItemCount(target, 3797) < 40)
		{
			...
			switch (code_info.getCode())
			{
				...
				case 1:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						{
							GiveItem1(target, 3797, 1);
							if (AiUtils.OwnItemCount(target, 3797) >= 40)
							{
								SoundEffect(target, "ItemSound.quest_middle");
							}
							else
