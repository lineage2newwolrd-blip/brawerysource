...
public class Antharas extends DefaultAI {
    ...
    protected void onEvtSpawn() {
        super.onEvtSpawn();
        _minionsSpawnDelay = System.currentTimeMillis() + 5 + 60 * 1000L;
    }
    ...
    protected boolean createNewTask() {
        ...
        // Minions spawn
        if (_minionsSpawnDelay < System.currentTimeMillis() && getAliveMinionsCount() < AllSettingsConfig.ALT_ANTHARAS_MINIONS_NUMBER) {
            _minionsSpawnDelay = System.currentTimeMillis()+ 5 * 60 * 1000L;
            int counter = AllSettingsConfig.ALT_ANTHARAS_MINIONS_NUMBER > 40 ? 4 : AllSettingsConfig.ALT_ANTHARAS_MINIONS_NUMBER > 10 ? 2 : 1;
            for(int i=0; i< counter; i++){
                NpcInstance minion = NpcUtils.spawnSingle(29190, Location.findPointToStay(actor.getLoc(), 400, 700, actor.getGeoIndex()));  // Antharas Minions
                _minions.add(minion);
                AntharasManager.addSpawnedMinion(minion);
            }
            for(int i=0; i< counter; i++){
                NpcInstance minion = NpcUtils.spawnSingle(29069, Location.findPointToStay(actor.getLoc(), 400, 700, actor.getGeoIndex()));  // Antharas Minions
                _minions.add(minion);
                AntharasManager.addSpawnedMinion(minion);
            }
        }
