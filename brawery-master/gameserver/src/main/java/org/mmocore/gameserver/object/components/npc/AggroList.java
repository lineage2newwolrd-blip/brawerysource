...
public class AggroList {
    ...
    public static final HateInfo EMPTY_HATE_INFO = new HateInfo();
    /**
    ...
    public void clear(final int radius) {
        writeLock.lock();
        try {
            if (hateList.isEmpty()) {
                return;
            ...
            final TIntHashSet char_map = new TIntHashSet();
            for (final Creature cha : World.getAroundCharacters(npc, radius, radius))
                char_map.add(cha.getObjectId());

            AggroInfo ai;
            for (TIntObjectIterator<AggroInfo> itr = hateList.iterator(); itr.hasNext(); ) {
                itr.advance();
                ai = itr.value();
                if (char_map.contains(ai.attackerId) ) {
                    continue;
                }
                ai.hate = 0;
                if (ai.damage == 0) {
    ...
    public HateInfo getTopHateInfo() {
	    AggroInfo[] hated;
	    readLock.lock();
	    try {
