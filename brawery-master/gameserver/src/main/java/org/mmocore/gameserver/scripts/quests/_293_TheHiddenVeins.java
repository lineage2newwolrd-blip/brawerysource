...
public class _293_TheHiddenVeins extends Quest {
                ...
                if (npcId == elder_filaur) {
                    if (st.ownItemCount(oriharukon_ore_1) < 1 && st.ownItemCount(hidden_vein_map) < 1)
                        ...
                        if (st.ownItemCount(hidden_vein_map) >= 10)
                            st.giveReward(ADENA_ID, st.ownItemCount(hidden_vein_map) * 500 + 2000, true);
                        else
                            st.giveReward(ADENA_ID, st.ownItemCount(hidden_vein_map) * 500, true);
                        ...
                        if (st.ownItemCount(oriharukon_ore_1) >= 10)
                            st.giveReward(ADENA_ID, st.ownItemCount(oriharukon_ore_1) * 5 + 2000, true);
                        else
                            st.giveReward(ADENA_ID, st.ownItemCount(oriharukon_ore_1) * 5, true);
                        ...
                        if (st.ownItemCount(oriharukon_ore_1) + st.ownItemCount(hidden_vein_map) >= 10)
                            st.giveReward(ADENA_ID, st.ownItemCount(oriharukon_ore_1) * 5 + st.ownItemCount(hidden_vein_map) * 500 + 2000, true);
                        else
                            st.giveReward(ADENA_ID, st.ownItemCount(oriharukon_ore_1) * 5 + st.ownItemCount(hidden_vein_map) * 500, true);
                        st.takeItems(hidden_vein_map, -1);
                        st.takeItems(oriharukon_ore_1, -1);
                        if (talker_level < 25 && class_level && !st.getPlayer().getPlayerVariables().getBoolean(PlayerVariables.NG_57)) {
                            st.playTutorialVoice("tutorial_voice_026");
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == utuku_orc || npcId == utuku_orc_archer || npcId == utuku_orc_grunt) {
            int i0 = Rnd.get(100);
            if (i0 > 50) {
                st.giveItems(oriharukon_ore_1, 1, true);
