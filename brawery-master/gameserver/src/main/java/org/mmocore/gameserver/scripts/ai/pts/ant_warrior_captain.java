...
public class ant_warrior_captain extends warrior_aggressive_casting_enchant_self
{
	public ant_warrior_captain(final NpcInstance actor){super(actor);}
			...
			switch (code_info.getCode())
			{
				...
				case 2:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						if (AiUtils.HaveMemo(c1, 213)!=0 && AiUtils.OwnItemCount(c1, 2667) == 1 && AiUtils.OwnItemCount(c1, 2670) == 0)
						{
							if (AiUtils.Rand(100) < 100)
							{
								c1.quest_last_reward_time = GetCurrentTick();
								GiveItem1(c1, 2670, 1);
								SoundEffect(c1, "ItemSound.quest_middle");
								if (AiUtils.OwnItemCount(c1, 2668) >= 1 && AiUtils.OwnItemCount(c1, 2669) >= 1 && AiUtils.OwnItemCount(c1, 2671) >= 1)
								{
