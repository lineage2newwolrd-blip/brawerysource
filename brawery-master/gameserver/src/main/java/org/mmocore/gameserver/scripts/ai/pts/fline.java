...
{
	public fline(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 335)!=0 && AiUtils.OwnItemCount(target, 3754)!=0 && AiUtils.OwnItemCount(target, 3796) < 40)
		{
		...
		if (!AiUtils.IsNull(code_info))
		{
			switch (code_info.getCode())
			{
				...
				case 1:
				{
					...
					{
						target.quest_last_reward_time = GetCurrentTick();
						if (AiUtils.Rand(100) < 59)
						{
							GiveItem1(target, 3796, 1);
							if (AiUtils.OwnItemCount(target, 3796) >= 40)
							{
								SoundEffect(target, "ItemSound.quest_middle");
							}
							else
