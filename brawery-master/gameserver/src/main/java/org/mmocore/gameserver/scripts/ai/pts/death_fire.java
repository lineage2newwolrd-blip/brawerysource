...
public class death_fire extends party_leader_ag_casting_ddmagic
{
	public death_fire(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					...
					if (!IsNullCreature(c2) && DistFromMe(c2) <= 1500)
					{
						...
						{
							if (AiUtils.GetMemoState(c2, 369) == 1 && AiUtils.OwnItemCount(c2, 5882) >= 49)
							{
								GiveItem1(c2, 5882, 1);
							}
							else if (AiUtils.GetMemoState(c2, 369) == 3 && AiUtils.OwnItemCount(c2, 5882) >= 199)
							{
								GiveItem1(c2, 5882, 1);
							}
							else
