...
{
	public lennunt_orc_sniper(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 303) != 0 && AiUtils.OwnItemCount(target, 963) < 10)
		{
		...
		if (!AiUtils.IsNull(code_info))
		{
			switch (code_info.getCode())
			{
				...
				case 1:
				{
					SetCurrentQuestID(303);
					target = code_info.RandomSelectOne();
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						if (AiUtils.Rand(100) < 40)
						{
							GiveItem1(target, 963, 1);
							if (AiUtils.OwnItemCount(target, 963) >= 9)
							{
								SoundEffect(target, "ItemSound.quest_middle");
								SetFlagJournal(target, 303, 2);
								ShowQuestMark(target, 303);
							}
							else
							{
								SoundEffect(target, "ItemSound.quest_itemget");
