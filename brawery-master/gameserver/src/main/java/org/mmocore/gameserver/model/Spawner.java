...
public abstract class Spawner extends EventOwner implements Cloneable {
    ...
    public void decreaseCount0(final NpcTemplate template, final NpcInstance spawnedNpc, final long deadTime) {
        ...
        if (_doRespawn && _scheduledCount + _currentCount < _maximumCount) {
            ...
            long delay;
            if (template.isRaid) {
                if (RaidBossSpawnManager.getInstance().isFixedSpawnTime(template.npcId)) {
                    long respawnTime = RaidBossSpawnManager.getInstance().getFixedSpawnTime(template.npcId);
                    delay = respawnTime * 1000L - System.currentTimeMillis();
                } else {
                    delay = (long) (getRespawnDelayWithRnd() * AllSettingsConfig.ALT_RAID_RESPAWN_MULTIPLIER * 1000L);
