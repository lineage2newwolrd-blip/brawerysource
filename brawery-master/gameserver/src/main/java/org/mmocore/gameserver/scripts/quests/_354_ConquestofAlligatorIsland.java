...
public class _354_ConquestofAlligatorIsland extends Quest {
        ...
        if (npcId == warehouse_keeper_kluck)
            if (event.equalsIgnoreCase("quest_accept")) {
                ...
                if (st.ownItemCount(croc_tooth) >= 100) {
                    st.giveReward(ADENA_ID, st.ownItemCount(croc_tooth) * 220 + 10700, true);
                    ...
                    st.giveReward(ADENA_ID, st.ownItemCount(croc_tooth) * 220 + 3100, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == crokian_lad) {
            if (Rnd.get(100) < 84) {
                st.giveItems(croc_tooth, 1, true);
            ...
            if (Rnd.get(100) < 14) {
                st.giveItems(croc_tooth, 2, true);
                st.soundEffect(SOUND_ITEMGET);
            } else {
                st.giveItems(croc_tooth, 1, true);
                st.soundEffect(SOUND_ITEMGET);
