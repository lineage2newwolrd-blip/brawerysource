...
public class RequestRecipeItemMakeSelf extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        double chance = activeChar.getPremiumAccountComponent().getPremiumBonus().getCraftChance();
        if (chance <= 0)
            chance = 1;
        int success = craft(activeChar, recipe, chance);

        if (activeChar.getSkillLevel(330) > 0
                && activeChar.calcStat(Stats.SKILL_MASTERY, activeChar.getSTR(), null, null)
                >= Rnd.get(FormulasConfig.ALT_DOUBLE_CRAFT)) {//5000
            success += craft(activeChar, recipe, chance);
        }

        activeChar.sendPacket(new RecipeItemMakeInfo(activeChar, recipe, Math.min(1, success)));
    }

    private int craft(final Player activeChar, final Recipe recipe, final double craftChance) {
