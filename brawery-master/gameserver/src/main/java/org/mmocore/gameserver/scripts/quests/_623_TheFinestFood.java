            ...
            if (event.equalsIgnoreCase("reply_3") && GetHTMLCookie == 2 - 1)
                if (st.ownItemCount(q_leaf_of_flava) >= 100 && st.ownItemCount(q_buffalo_meat) >= 100 && st.ownItemCount(q_horn_of_antelope) >= 100) {
                    ...
                    if (i1 < 120) {
                        ...
                        st.giveReward(ADENA_ID, 65000, true);
                        ...
                        st.giveReward(ADENA_ID, 73000, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (GetMemoState == 1 * 10 + 1)
            if (npcId == thermal_buffalo) {
                int i4 = Rnd.get(1000);
                if (i4 < 1000)
                    if (st.ownItemCount(q_buffalo_meat) + 1*st.getRateQuestsDrop() >= 100) {
                        ...
                        st.giveItems(q_buffalo_meat, 1, true);
                        st.soundEffect(SOUND_ITEMGET);
                    }
            } else if (npcId == thermal_flava) {
                int i4 = Rnd.get(1000);
                if (i4 < 1000)
                    if (st.ownItemCount(q_leaf_of_flava) + 1*st.getRateQuestsDrop() >= 100) {
                        ...
                        st.giveItems(q_leaf_of_flava, 1, true);
                        st.soundEffect(SOUND_ITEMGET);
                    }
            } else if (npcId == thermal_antelope) {
                int i4 = Rnd.get(1000);
                if (i4 < 1000)
                    if (st.ownItemCount(q_horn_of_antelope) + 1*st.getRateQuestsDrop() >= 100) {
                        ...
                        st.giveItems(q_horn_of_antelope, 1, true);
                        st.soundEffect(SOUND_ITEMGET);
                    }
            }
        return null;
