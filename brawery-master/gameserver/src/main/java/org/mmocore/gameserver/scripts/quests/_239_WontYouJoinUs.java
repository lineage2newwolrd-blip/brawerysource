            ...
            case STARTED:
                if (npcId == minehr) {
                    if (st.ownItemCount(q_letter_of_confirmation_elf) < 1)
                        ...
                        st.giveReward(ADENA_ID, 283346, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (GetMemoState == 1 || GetMemoState == 2 && npc.getNpcId() == eggs_of_murcrokian) {
            if (st.ownItemCount(q_parts_of_reclaiming_worker) <= 9) {
                st.giveItems(q_parts_of_reclaiming_worker, 1, true);
            }
            if (st.ownItemCount(q_parts_of_reclaiming_worker) >= 10) {
                st.setCond(2);
            ...
            if (i0 < 774) {
                if (st.ownItemCount(q_piece_of_helldozer) <= 19) {
                    st.giveItems(q_piece_of_helldozer, 1, true);
                }
                if (st.ownItemCount(q_piece_of_helldozer) >= 20) {
                    st.setCond(4);
