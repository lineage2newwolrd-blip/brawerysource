...
public class PositionUtils {
    ...
    private static final double BACK_MAX_ANGLE = 100;
     ...
     * Return True if attacker is IN FRONT of the target.<BR><BR>
     */
    public static boolean isInFrontOf(final Creature target, final Creature attacker, double maxAngle) {
        if (target == null) {
            return false;
        }

        double maxAngleDiff = maxAngle / 2.0d;
        double angleTarget = calculateAngleFrom(target, attacker);
        double angleChar = convertHeadingToDegree(target.getHeading());
        if (angleChar < 0)
            angleChar += MAX_ANGLE;
        if (angleChar >= MAX_ANGLE)
            angleChar -= MAX_ANGLE;
        double angleDiff = (angleChar - angleTarget + 180 + MAX_ANGLE) % MAX_ANGLE - 180;
        if (angleDiff <= maxAngleDiff && angleDiff >= -maxAngleDiff)
            return true;
        return false;
    }

    public static boolean isInFrontOf(Creature target, Creature attacker) {
        return isInFrontOf(target, attacker, FRONT_MAX_ANGLE);
    }
    ...
    public static boolean isBehind(Creature target, Creature attacker) {
        return isBehind(target, attacker, BACK_MAX_ANGLE);
    }

    public static boolean isBehind(final Creature target, final Creature attacker, double maxAngle) {
        if (target == null) {
            return false;
        }
        ...
        if (angleTarget >= MAX_ANGLE)
            angleTarget -= MAX_ANGLE;
        double angleDiff = (angleTarget - angleChar + 180 + MAX_ANGLE) % MAX_ANGLE - 180;
        if (angleDiff <= maxAngleDiff && angleDiff >= -maxAngleDiff)
            return true;
        return false;
    }
    ...
    public static Location calculateAnglePoint(final Location from, final Location to, final double addAngle, double dist, int geoIndex){
        double set = Math.toRadians(PositionUtils.calculateAngleFrom(from.x, from.y, to.x, to.y) + addAngle);
        int x = from.x + (int)(Math.cos(set) * dist);
        int y = from.y + (int)(Math.sin(set) * dist);
        int z = to.z + 300;
        return Location.findPointToStay(x, y, z, 0, 50, geoIndex);
    }
