...
public class wizard_growth_step3 extends wizard_growh_basic
{
	public int SelfBuff = 264110083;
	public int SelfRangeDDMagic = 264306689;

	public wizard_growth_step3(final NpcInstance actor){super(actor);}
	...
	protected void onEvtTimerFiredEx(int timer_id)
	{
		...
		if ((timer_id == 2001 && (i_ai4 == 1 || i_ai4 == 3)) && actor.isAlive())
		{
			...
			if (AiUtils.Rand(100) > 50)
			{
				if (AiUtils.Rand(100) < 50)
				{
					CreateOnePrivateEx(silhouette1, ai_type1, 0, 0, AiUtils.FloatToInt(actor.getX()), AiUtils.FloatToInt(actor.getY()), AiUtils.FloatToInt(actor.getZ()), i0 * 182, AiUtils.GetIndexFromCreature(c_ai0), i_ai3, 0);
				}
				else
				{
			...
			else if (IsInCategory(Category.fighter_group, c_ai0.getClassId()))
			{
				CreateOnePrivateEx(silhouette_ex, ai_type_ex, 0, 0, AiUtils.FloatToInt(actor.getX()), AiUtils.FloatToInt(actor.getY()), AiUtils.FloatToInt(actor.getZ()), i0 * 182, AiUtils.GetIndexFromCreature(c_ai0), i_ai3, 0);
			}
			else
			{
