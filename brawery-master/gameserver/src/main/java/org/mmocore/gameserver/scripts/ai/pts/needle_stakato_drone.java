...
{
	public needle_stakato_drone(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 236) == 1 && GetOneTimeQuestFlag(target, 236) == 0 && AiUtils.GetMemoState(target, 236) == 2 && AiUtils.OwnItemCount(target, 9745) == 0)
		{
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			if (code_info.getCode() == 2)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					...
					if (i0 < 20)
					{
						GiveItem1(target, 9745, 1);
						SetFlagJournal(target, 236, 3);
