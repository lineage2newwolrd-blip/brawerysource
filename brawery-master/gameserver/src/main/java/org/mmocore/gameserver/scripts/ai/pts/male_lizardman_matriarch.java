...
public class male_lizardman_matriarch extends warrior_aggressive_physicalspecial
{
	public male_lizardman_matriarch(final NpcInstance actor){super(actor);}
		...
		if (!AiUtils.IsNull(code_info))
		{
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					...
					if (i4 < 750)
					{
						if (AiUtils.OwnItemCount(target, 7183) + 1 >= 50)
						{
							if (AiUtils.OwnItemCount(target, 7183) < 50)
							{
								GiveItem1(target, 7183, 50 - AiUtils.OwnItemCount(target, 7183));
