            ...
            if (getEffected().isPlayer()) {
                ...
                if (!player.getCubics().isEmpty()) {
                    ...
                    if (engine.isPresent()) {
                        ...
                        Collection<CubicComponent> coll = player.getCubics().values();
                        List<CubicComponent> exists;
                        if (coll instanceof List)
                            exists = (List<CubicComponent>)coll;
                        else
                            exists = new ArrayList<>(coll);
                        exists.stream()
                                .min(Comparator.comparingLong(CubicComponent::getStartTime))
                                .ifPresent(lastCube -> lastCube.delete(false));
                    }
                }
