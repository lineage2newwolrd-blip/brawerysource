package org.mmocore.gameserver.geoengine.pathfind;

import org.mmocore.gameserver.configuration.config.GeodataConfig;
import org.mmocore.gameserver.geoengine.GeoEngine;
import org.mmocore.gameserver.geoengine.pathfind.PathFindBuffers.GeoNode;
import org.mmocore.gameserver.geoengine.pathfind.PathFindBuffers.PathFindBuffer;
...
import static org.mmocore.gameserver.geoengine.GeoEngine.*;
...
public class PathFind {
	...
	public static final List<Location> findPath(int x, int y, int z, int destX, int destY, int destZ, boolean isPlayable, int geoIndex) {
		...
		//Получение точки встречи с препятствием(для ускорения поиска)
		// следует получать только при наличии достаточного расстояния
		// иначе путь не строится правильно
		Location startPoint = new Location(x, y, z);
		if (GeodataConfig.PATHFIND_BOOST == BOOST_START | GeodataConfig.PATHFIND_BOOST == BOOST_BOTH) {
			Location temp = moveCheckWithCollision(x, y, z, destX, destY, true, geoIndex);
			if (startPoint.distance(temp) > 100)
				startPoint = temp;
		}

		Location endPoint = new Location(destX, destY, destZ);
		if (GeodataConfig.PATHFIND_BOOST == BOOST_BOTH || Math.abs(destZ - z) > 200) {
			Location temp = moveCheckBackwardWithCollision(destX, destY, destZ, startPoint.x, startPoint.y, true, geoIndex);
			if (endPoint.distance(temp) > 100)
				endPoint = temp;
		}
		...
		if (xdiff == 0 && ydiff == 0) {
			if (Math.abs(endPoint.z - startPoint.z) < GeodataConfig.PATHFIND_MAX_Z_DIFF) {
		...
		int mapSize = GeodataConfig.PATHFIND_MAP_MUL * Math.max(xdiff, ydiff);
		...
		targetRecorder.addAll(path.stream().map(Location::geo2world).collect(Collectors.toList()));
	...
	private List<Location> findPath() {
		...
		if (path != null) {
			buff.successUses++;
		} else if (searhTime > GeodataConfig.PATHFIND_MAX_TIME) {
			buff.overtimeUses++;
		}
	...
	private void handleNode(GeoNode node) {
		...
		if (GeodataConfig.PATHFIND_DIAGONAL) {
			// Юго-восток
	...
	private float pathCostEstimate(GeoNode n) {
		...
		return (float) Math.sqrt(diffx * diffx + diffy * diffy + diffz * diffz / 256f);
	}

	/**
	 * Диагональное и прямые перемещениями между нодами имеют разную стоимость
	 * Диагональное - проверяют наличие непроходимых участков по диагонали от след. ноды
	 * Прямое - проверяют непроходимые участки в соседних от следующей ноды
	 * Диагональное перемещение с наличием проблемных соседних нод возвращает большую стоимость
	 * из-за необходимости обходить углы и не срезать их по диагонали
	 *
	 * @param from     предыдущая нода
	 * @param n        следующая нода
	 * @param diagonal если двигаемся по диагонали
	 * @return стоимость прохода между нодами
	 */
	private float traverseCost(GeoNode from, GeoNode n, boolean diagonal) {
		if (n.nswe != NSWE_ALL || Math.abs(n.z - from.z) > 16) {
			return 3f;
		} else if (diagonal) {
			getHeightAndNSWE(n.x + 1, n.y + 1, n.z);
	...
	private void handleNeighbour(int x, int y, GeoNode from, boolean d) {
		...
		if (height > GeodataConfig.PATHFIND_MAX_Z_DIFF || n.nswe == NSWE_NONE) {
			return;
		}
