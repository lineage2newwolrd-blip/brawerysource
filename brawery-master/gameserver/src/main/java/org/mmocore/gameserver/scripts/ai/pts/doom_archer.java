...
public class doom_archer extends warrior_aggressive_use_power_shot
{
	public doom_archer(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 359) == 1 && AiUtils.GetMemoState(target, 359) == 1 && AiUtils.OwnItemCount(target, 5869) < 60)
		{
		...
		if (!AiUtils.IsNull(code_info))
		{
			switch (code_info.getCode())
			{
				...
				case 1:
				{
					SetCurrentQuestID(359);
					target = code_info.RandomSelectOne();
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						if (AiUtils.Rand(1000) < 503)
						{
							GiveItem1(target, 5869, 1);
							if (AiUtils.OwnItemCount(target, 5869) >= 59)
							{
								SetFlagJournal(target, 359, 2);
								ShowQuestMark(target, 359);
