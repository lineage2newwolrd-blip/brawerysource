...
public class _295_DreamsOfTheSkies extends Quest {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 0) {
            ...
            if (st.ownItemCount(RING_OF_FIREFLY) < 1) {
                ...
                st.giveReward(ADENA_ID, 2400, true);
            }
        }
        return htmltext;
    }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        if (st.getCond() == 1 && st.ownItemCount(FLOATING_STONE) < 50) {
            if (Rnd.chance(25)) {
                if (st.ownItemCount(FLOATING_STONE) +1*st.getRateQuestsDrop() >= 50) {
                    st.giveItems(FLOATING_STONE, 50 - st.ownItemCount(FLOATING_STONE));
                    st.soundEffect(SOUND_MIDDLE);
                    st.setCond(2);
                } else {
                    st.giveItems(FLOATING_STONE, 1, true);
                    st.soundEffect(SOUND_ITEMGET);
                }
            } else if (st.ownItemCount(FLOATING_STONE) +2*st.getRateQuestsDrop() >= 48) {
                st.giveItems(FLOATING_STONE, 50 - st.ownItemCount(FLOATING_STONE));
                st.soundEffect(SOUND_MIDDLE);
                st.setCond(2);
            } else {
                st.giveItems(FLOATING_STONE, 2, true);
                st.soundEffect(SOUND_ITEMGET);
            }
        }
        return null;
