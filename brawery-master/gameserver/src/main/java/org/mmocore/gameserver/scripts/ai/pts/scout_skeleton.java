...
public class scout_skeleton extends warrior_passive_use_bow
{
	public scout_skeleton(final NpcInstance actor){super(actor);}
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			switch (code_info.getCode())
			{
				case 0:
				{
					...
					{
						c1 = GetLastAttacker();
						if (AiUtils.HaveMemo(c_quest0, 412) == 1 && AiUtils.OwnItemCount(c_quest0, 1279) == 1 && i_quest0 == 1 && AiUtils.OwnItemCount(c_quest0, 1260) < 3)
						{
							if (AiUtils.Rand(2) == 0)
							{
								GiveItem1(c_quest0, 1260, 1);
								if (AiUtils.OwnItemCount(c_quest0, 1260) == 3)
								{
				...
				case 1:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						if (AiUtils.HaveMemo(c_quest0, 406) != 0 && AiUtils.OwnItemCount(c_quest0, 1203) == 0 && AiUtils.OwnItemCount(c_quest0, 1205) < 20 && i_quest0 == 1 && AiUtils.Rand(100) < 70)
						{
							...
							if (AiUtils.OwnItemCount(c_quest0, 1205) == 20)
							{
								SoundEffect(c_quest0, "ItemSound.quest_middle");
							}
							else
