    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == zaken_day_83) {
            if (GetMemoState == 1 && st.ownItemCount(g_q_soulpiece_of_zaken) < 20) {
                if (st.ownItemCount(g_q_soulpiece_of_zaken) + 1*st.getRateQuestsReward() >= 20) {
                    st.setCond(2);
                    st.giveReward(g_q_soulpiece_of_zaken, 20 - st.ownItemCount(g_q_soulpiece_of_zaken), false);
                    st.soundEffect(SOUND_MIDDLE);
                } else if (Rnd.get(2) == 0) {
                    if (st.ownItemCount(g_q_soulpiece_of_zaken) + 2*st.getRateQuestsReward() >= 20) {
                        st.setCond(2);
                        st.giveReward(g_q_soulpiece_of_zaken, 20 - st.ownItemCount(g_q_soulpiece_of_zaken), false);
                        st.soundEffect(SOUND_MIDDLE);
                    } else {
                        st.giveReward(g_q_soulpiece_of_zaken, 2, true);
                        st.soundEffect(SOUND_ITEMGET);
                    }
                } else {
                    st.giveReward(g_q_soulpiece_of_zaken, 1, true);
                    st.soundEffect(SOUND_ITEMGET);
                }
            }
        }
