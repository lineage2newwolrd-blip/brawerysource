...
public final class DressArmorParser extends AbstractFileParser<DressArmorHolder> {
    ...
    @Override
    protected void readData(DressArmorHolder holder, Element rootElement) throws Exception {
        for (Element dress : rootElement.getChildren("dress")) {
            String name = null;
            int id, chest = 0, legs = 0, gloves = 0, feet = 0, helmet = 0, cloak = 0, itemId = 0;
            long itemCount = 0L;
            ...
            helmet = Integer.parseInt(set.getAttributeValue("helmet"));
            cloak = Integer.parseInt(set.getAttributeValue("cloak"));
            ...
            holder.addDress(new DressArmorData(id, name, chest, legs, gloves, feet, helmet, cloak, itemId, itemCount));
        }
    }
}