        ...
        if (npcId == high_summoner_galatea) {
            if (event.equalsIgnoreCase("quest_accept")) {
                ...
                if (st.ownItemCount(q_wind_slyph_breath) >= 10)
                    st.giveReward(ADENA_ID, 5365 + 50 * st.ownItemCount(q_wind_slyph_breath), true);
                else
                    st.giveReward(ADENA_ID, 50 * st.ownItemCount(q_wind_slyph_breath), true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == whispering_wind) {
            if (GetMemoState == 1 * 10 + 1) {
                int i4 = Rnd.get(100);
                if (i4 < 98) {
                    st.giveItems(q_wind_slyph_breath, 1, true);
