...
public class delu_lizardman_agent extends warrior_passive_use_power_shot
{
	public delu_lizardman_agent(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 1)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					...
					if (AiUtils.OwnItemCount(target, 9773) < 30)
					{
						...
						if (i4 < 240 && AiUtils.OwnItemCount(target, 9773) < 29)
						{
							GiveItem1(target, 9773, 1);
						}
					}
