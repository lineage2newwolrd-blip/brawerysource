...
public class MonsterInstance extends NpcInstance {
    ...
    static {
        ...
        String crpStr = OtherCustom.crpRewardMonsters;
    ...
    public boolean canChampion() {
        return !isMinion() && getTemplate().rewardExp > 0 && getTemplate().level >= AllSettingsConfig.ALT_CHAMPION_MIN_LEVEL && getTemplate().level <= AllSettingsConfig.ALT_CHAMPION_TOP_LEVEL;
    }
    ...
    protected void onReduceCurrentHp(final double damage, final Creature attacker, final SkillEntry skill, final boolean awake,
        ...
        if(getNpcId() >= 22650 && getNpcId() <= 22659 && attacker.getEffectList().getEffectsBySkillId(2900) != null)
        {
            getAggroList().addDamageHate(attacker,(int)getCurrentHp(),0);
            super.onReduceCurrentHp(getCurrentHp(), attacker, skill, awake, standUp, directHp, lethal);
            return;
        }
    ...
    private void addCrpReward(Player player) {
        ...
        if (maxHate == 0)
            return;
        if (Util.percentOf(maxHate, playerInfo.hate) > OtherCustom.crpRewardMinHatePercent)
