...
public class ai_drake_lord extends ai_antaras_cave_raid_basic {
    ...
    protected void onEvtTimerFiredEx(int timer_id, Object arg1, Object arg2) {
        ...
        if (timer_id == USE_SKILL03_TIME) {
            if (i_ai0 == 1) {
                AddTimerEx(USE_SKILL03A_TIME, (120 + Rnd.get(60)) * 1000);
                for (final Creature creature : getActor().getAggroList().getHateList(1000)) {
                    if (creature!=null && !creature.isDead() && creature.isPlayable()) {
                        final Location loc = Location.findPointToStay(Location.coordsRandomize(getActor().getSpawnedLoc(), 100, 300), 300, getActor().getGeoIndex());
            ...
            AddTimerEx(USE_SKILL03_TIME, 5000);
        }
