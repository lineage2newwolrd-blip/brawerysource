        ...
        if (npcId == guard_luis) {
            ...
            if (event.equalsIgnoreCase("reply_1") && GetHTMLCookie == 3 - 1) {
                ...
                if (st.ownItemCount(q_liz_totem_tooth2) >= 50) {
                    ...
                    if (i1 < 250) {
                        st.giveReward(bone_helmet, 1, false);
                        st.giveReward(ADENA_ID, 5200, true);
                    } else if (i1 < 500) {
                        st.giveReward(aspis, 1, false);
                        st.giveReward(ADENA_ID, 1500, true);
                    } else if (i1 < 750) {
                        st.giveReward(blue_buckskin_boots, 1, false);
                        st.giveReward(ADENA_ID, 3200, true);
                    } else if (i1 < 1000) {
                        st.giveReward(leather_gauntlet, 1, false);
                        st.giveReward(ADENA_ID, 3200, true);
                    }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (GetMemoState == 1 * 10 + 1) {
            if (npcId == langk_lizardman_sub_ldr || npcId == langk_lizardman_sentinel)
                if (st.ownItemCount(q_liz_feather) + 1*st.getRateQuestsDrop() >= 100) {
                    if (st.ownItemCount(q_liz_feather) < 100) {
                    ...
                    st.giveItems(q_liz_feather, 1, true);
            ...
            if (npcId == langk_lizardman_leader || npcId == langk_lizardman_shaman) {
                int i4 = Rnd.get(1000);
                if (i4 < 500)
                    if (st.ownItemCount(q_liz_totem_tooth2) + 1*st.getRateQuestsDrop() >= 50) {
                        if (st.ownItemCount(q_liz_totem_tooth2) < 50) {
                        ...
                        st.giveItems(q_liz_totem_tooth2, 1, true);
                        st.soundEffect(SOUND_ITEMGET);
                    }
            }
        return null;
