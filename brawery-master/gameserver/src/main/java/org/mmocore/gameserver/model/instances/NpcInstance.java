...
public class NpcInstance extends Creature {
    ...
    public int _respawn_minion = 0;
    public int _desirePoint = 1;
    ...
    public int activeWeaponEnchant;
    public int summoner_id;

    protected String _ptsName;
    ...
    private HardReference<? extends PrivatesList> _privateParty = null;
    private MultiValueSet<String> _parameters = StatsSet.EMPTY;
    private MultiValueSet<String> spawn_parameters = StatsSet.EMPTY;;
    ...
    private volatile HardReference<? extends Creature> _lastAttacker = HardReferences.emptyRef();
    ...
    public NpcInstance(final int objectId, final NpcTemplate template) {
        ...
        _ptsName = LinkerFactory.getInstance().npcPchName(template.npcId);
    ...
    public Location getVarSpawnedLoc() {
        return _spawnedLoc;
    }
    ...
    protected void onDeath(final Creature killer) {
        ...
        if (getLeader() != null) {
            minionDie(this, _respawn_minion);
        }
        if (hasOnePrivateEx() && getPrivatesList().hasAliveOnePrivateEx()) {
            for(NpcInstance minion : getPrivatesList().getAliveOnePrivateEx())
                minion.setLeader(null);
            getPrivatesList().clearOnePrivateEx();
        }
    ...
    public int getClassId() {
        return getTemplate().getNpcId() + 1000000;
    }

    @Override
    public int getRaceId() {
        return getTemplate().getRace().ordinal();
    }
    ...
    public void startAbnormalEffect(AbnormalEffect ae, Creature speller, SkillEntry se) {
        super.startAbnormalEffect(ae);
        if(getAI().isPTSAI())
            ThreadPoolManager.getInstance().execute(new NotifyAITask(this, CtrlEvent.EVT_ABNORMAL_CHANGE, speller, se));
    }
    ...
    protected void onSpawn() {
        ...
        if (Privates != null && !getAI().isPTSAI()) {
            if (this.hasPrivates()) {
    ...
    public void setChargedSoulShot(int shot){
        _chargedSoulshot = shot;
    }

    @Override
    public boolean getChargedSoulShot(){
        return _chargedSoulshot > 0;
    }
    public void setChargedSpiritShot(int shot){
        _chargedSpiritShot = shot;
    }
    ...
    public WeaponTemplate getActiveWeaponItem() {
        ...
        final int weaponId = activeWeaponId < 1? getTemplate().rhand : activeWeaponId;
    ...
    public void setActiveWeaponItem(int itemId) {
        activeWeaponId = itemId;
    }
    public void setActiveWeaponEchant(int enchantLevel) {
        activeWeaponEnchant = enchantLevel;
    }
    public int getActiveWeaponEchant() {
        return activeWeaponEnchant;
    }
    ...
    public void startRandomAnimation() {
        if (!hasRandomAnimation() || getAI().isPTSAI()) {
            return;
        }
            ...
            switch (npcId) {
                ...
                case 32645:
                    if(player.isInParty())
                    {
                        ...
                        if(!party.isLeader(player) || party.getMemberCountInRange(player, 1000) != party.getMemberCount()) {
                            Functions.show("default/32645-2.htm", player, lastNpc);
                            return;
                        }
                        else {
                            for(Player member : party.getPartyMembers())
                                member.teleToLocation(Location.findPointToStay(Integer.parseInt(param[0]), Integer.parseInt(param[1]), Integer.parseInt(param[2]), 50, 100, player.getGeoIndex()));
                            return;
                        }
                    }
                    break;
    ...
    public void showSkillList(final Player player) {
        ...
        if (!(getTemplate().canTeach(classId) || getTemplate().canTeach(classId.getParent(player.getSex().ordinal())))) {
    ...
    public MultiValueSet<String> getSpawnParameters() {
        return spawn_parameters;
    }

    public void setSpawnParameters(final MultiValueSet<String> set) {
        if (set.isEmpty()) {
            return;
        }
        setParameters(set);

        if (spawn_parameters == StatsSet.EMPTY) {
            spawn_parameters = new MultiValueSet<>(set.size());
        }
        spawn_parameters.putAll(set);
    }
    ...
    public void setParam1(final int i0) {
        param1 = i0;
    }
    ...
    public void setParam2(final int i1) {
        param2 = i1;
    }
    ...
    public void setPrivateParty(PrivatesList list) {
        _privateParty = list.getRef();
    }

    public PrivatesList getPrivateParty() {
        return _privateParty == null? null : _privateParty.get();
    }
    ...
    public Location getMinionPosition() {
        return isMonster()? Location.findPointToStay(this, 100, 200) : Location.findPointToStay(this, 50, 80);
    }
    ...
    public void updateZones() {
        if(territory!=null && !territory.isInside(getX(), getY(), getZ()))
            ThreadPoolManager.getInstance().execute(new NotifyAITask(this, CtrlEvent.EVT_OUT_TERRITORY));
        super.updateZones();
    }

    public Creature getLastAttacker() {
        return _lastAttacker.get();
    }

    public void setLastAttacker(Creature killer) {
        if (killer == null) {
            _lastAttacker = HardReferences.emptyRef();
        } else {
            _lastAttacker = killer.getRef();
        }
    }
    ...
    public void setTarget(GameObject object)
    {
        ...
        if(object != null)
        {
            broadcastPacket(new TargetSelected(getObjectId(), object.getObjectId(), getLoc()));
        }
