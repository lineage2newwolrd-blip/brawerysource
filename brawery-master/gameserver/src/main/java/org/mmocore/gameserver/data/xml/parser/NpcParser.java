...
public class NpcParser extends AbstractDirParser<NpcHolder> {
    ...
    protected void readData(final NpcHolder holder, final Element rootElement) throws Exception {
        for (final Element npcElement : rootElement.getChildren()) {
            ...
            final int templateId = npcElement.getAttributeValue("template_id") == null ? 0 : Integer.parseInt(npcElement.getAttributeValue("template_id"));
    ...
    private RewardData parseReward(final Element rewardElement) {
        ...
        if ((ServerConfig.NO_RATE_EQUIPMENT && data.getItem().isEquipment()) // отключаемая рейтовка эквипа
                ...
                || ServerConfig.notRatedTypes.contains(data.getItem().getItemType()) // отключаемая рейтовка для типов предметов
