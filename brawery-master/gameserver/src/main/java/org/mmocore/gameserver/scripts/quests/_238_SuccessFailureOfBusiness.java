    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if ((GetMemoState == 1 || GetMemoState == 2) && npcId == brazier_of_peace) {
            if (st.ownItemCount(q_piece_of_torch) <= 9) {
                st.giveItems(q_piece_of_torch, 1, true);
            }
            if (st.ownItemCount(q_piece_of_torch) >= 10) {
                st.setCond(2);
            ...
            if (i0 < 800) {
                if (st.ownItemCount(q_piece_of_guardian_spirit) <= 19) {
                    st.giveItems(q_piece_of_guardian_spirit, 1, true);
                }
                if (st.ownItemCount(q_piece_of_guardian_spirit) >= 20) {
                    st.setCond(4);
