...
public class RequestRecipeShopMakeDo extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        double chance = buyer.getPremiumAccountComponent().getPremiumBonus().getCraftChance();
        if (chance <= 0)
            chance = 1;
        success = craft(buyer, manufacturer, recipe.get(), chance);

        if (manufacturer.getSkillLevel(330) > 0
                && manufacturer.calcStat(Stats.SKILL_MASTERY, manufacturer.getSTR(), null, null)
                >= Rnd.get(FormulasConfig.ALT_DOUBLE_CRAFT)) {//5000
            success += craft(buyer, manufacturer, recipe.get(), chance);
        }

        buyer.sendChanges();
        buyer.sendPacket(new RecipeShopItemInfo(buyer, manufacturer, _recipeId, _price, Math.min(1, success)));
    }

    private int craft(final Player buyer, final Player manufacturer, final Recipe recipe, final double craftChance) {
        final Optional<RecipeProduct> product = recipe.getRandomProduct(craftChance);
        if (product.isPresent()) {
            ...
            buyer.sendPacket(new SystemMessage(SystemMsg.S1_MANUFACTURING_FAILURE).addItemName(recipe.getItemId()));
            return 0;
        }
    }
}