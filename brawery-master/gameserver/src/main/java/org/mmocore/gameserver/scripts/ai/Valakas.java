...
public class Valakas extends DefaultAI {
    ...
    protected void onEvtSpawn() {
        super.onEvtSpawn();
        _minion_timer = System.currentTimeMillis() + 2 + 60 * 1000L;
    }
    ...
    protected boolean createNewTask() {
        ...
        // Lava skin task
        if(_hpStage >= 2 && _lava_skin_timer < System.currentTimeMillis())
        {
            _lava_skin_timer = System.currentTimeMillis() + Rnd.get(150, 300) * 1000L ;
            actor.altOnMagicUseTimer(actor, s_lava_skin);
        }

        // Minions spawn
        if (getAliveMinionsCount() < 100 && _minion_timer < System.currentTimeMillis()) {
            _minion_timer = System.currentTimeMillis()+ 60 * 1000L;
            if(actor.getAggroList().size() >= 5)
                for(int i=0; i < 4; i++){
                    NpcInstance minion = NpcUtils.spawnSingle(29029, Location.findPointToStay(actor.getLoc(), 200, 700, actor.getGeoIndex()));  // Valakas Minions
                    _minions.add(minion);
    ...
    protected void onEvtDead(Creature killer) {
        if (!_minions.isEmpty()) {
            _minions.forEach(NpcInstance::deleteMe);
            _minions.clear();
        }
