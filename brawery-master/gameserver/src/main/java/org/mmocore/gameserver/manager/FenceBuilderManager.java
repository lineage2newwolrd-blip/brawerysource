...
public class FenceBuilderManager {
    ...
    public int spawnFence(final Location loc, Reflection reflection, final int type, final int wid, final int hi) {
        final int id = IdFactory.getInstance().getNextId();
        final Fence inst = new Fence(id, loc, type, wid, hi);
        inst.setReflection(reflection);
        inst.spawnMe(new Location(loc.getX(), loc.getY(), loc.getZ() + 32));
        inst.broadcastCharInfo();
        inst.setCollision(true);
    ...
    public void deleteFence(final int id) {
        if (!fences.isEmpty()) {
            final Fence f = fences.get(id);
            if (f != null) {
                f.decayMe();
                f.setCollision(false);
            }
