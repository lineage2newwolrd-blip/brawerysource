...
public class PlayerChatListener implements OnPlayerChatListener {
    ...
    public void onPlayerChat(Player activeChar, String text, ChatType chatType, String target) {
		...
		*/
        ...
        text = StrTable.format(text, GameServer.getInstance().getVersion());
        ...
        if (globalchat) {
            if (ServerConfig.HIDE_SPAM_MSG) {
                if (ChatFilterConfig.containsSpamWord(text)) {
                    allowSend = false;
                }
            }
        ...
        switch (chatType) {
            case TELL:
                ...
                if (receiver != null && receiver.isInOfflineMode()) {
                    ...
                    if (!receiver.getMessageRefusal()) {
                        if (activeChar.antiFlood.canTell(receiver.getObjectId(), text) && (allowSend || receiver==activeChar)) {
            ...
            case SHOUT:
                ...
                if (ServerConfig.GLOBAL_SHOUT && allowSend) {
            ...
            case TRADE:
                ...
                if (ServerConfig.GLOBAL_TRADE_CHAT && allowSend) {
                    ChatUtils.announce(activeChar, cs);
                } else if(allowSend) {
                    ChatUtils.shout(activeChar, cs);
                }
            ...
            case ALL:
                ...
                {
                    AbstractFightClub fightClub = activeChar.getFightClubEvent();
                    if(fightClub != null && allowSend)
                    {
                        ChatUtils.say(activeChar, fightClub.getAllFightingPlayers(), cs);
                        break;
                    }
                }
            ...
            case PARTY_ROOM:
                ...
                for (Player roomMember : room.getPlayers())
                    if (!roomMember.isInBlockList(activeChar) && !roomMember.isBlockAll() && (allowSend || roomMember==activeChar))
            ...
            case HERO_VOICE:
                ...
                if (activeChar.isHero()
                        ...
                        || premiumHeroChat) {
                    if (activeChar.getCustomPlayerComponent().isTemporalHero() && !EventsConfig.AllowHeroChatForTempHero)
                        return;
            ...
            case BATTLEFIELD:
                if (activeChar.isInFightClub())
                {
                    for (Player player : activeChar.getFightClubEvent().getMyTeamFightingPlayers(activeChar))
                        if(allowSend || player==activeChar)
                            player.sendPacket(cs);
                    return;
                }
