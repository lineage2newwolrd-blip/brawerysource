    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (cond == 0) {
            ...
            if (st.ownItemCount(ORC_AMULET) == 0 && st.ownItemCount(ORC_NECKLACE) == 0) {
                ...
                st.giveReward(ADENA_ID, st.ownItemCount(ORC_AMULET) * 20 + st.ownItemCount(ORC_NECKLACE) * 30 + 1100, true);
                st.takeItems(ORC_AMULET, -1);
                st.takeItems(ORC_NECKLACE, -1);
            } else {
                htmltext = "sentry_kayleen_q0263_05.htm";
                st.giveReward(ADENA_ID, st.ownItemCount(ORC_AMULET) * 20 + st.ownItemCount(ORC_NECKLACE) * 30, true);
                st.takeItems(ORC_AMULET, -1);
                st.takeItems(ORC_NECKLACE, -1);
            }
        }
