...
public abstract class AbstractAction implements IUseAction {
    ...
    protected static boolean isCondition(final Player player) {
        if (player == null) {
            ...
            return false;
        } else if (AcpConfig.acpUsePremium || AcpConfig.acpUseCert) {
            if(!((AcpConfig.acpUsePremium && player.getPremiumAccountComponent().hasBonus()) || (AcpConfig.acpUseCert && !player.getInventory().getItemsByItemId(AcpConfig.certItemId).isEmpty()))) {
                player.getAcpComponent().stopAllTask();
                return false;
            }
        }
