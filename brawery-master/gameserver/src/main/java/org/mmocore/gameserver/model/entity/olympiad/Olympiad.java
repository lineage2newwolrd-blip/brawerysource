...
public class Olympiad {
    ...
    public static ScheduledFuture<?> _scheduledCompetitionTask;
    ...
    private static int convertToGregorian(int day_of_week){
        return day_of_week > 1 ? day_of_week -1 : 7;
    }
    ...
    public synchronized static void init() {
        ...
        if(OlympiadConfig.OLYMPIAD_WORK_DAYS.length>0)
            for (int i = 0; i<7; i++) {
                if (ArrayUtils.contains(OlympiadConfig.OLYMPIAD_WORK_DAYS, convertToGregorian(_compStart.get(Calendar.DAY_OF_WEEK))))
                    break;
    ...
    private static boolean isValidHardware(Player leader, Player player, final CompType type) {
        String message = type == CompType.TEAM
                ? "Member of party is already registered at the Olympiad Games!"
                : "You are already registered at the Olympiad Games!";
        if (OlympiadConfig.OLYMPIAD_CHECK_IP) {
            for (RegisteredPlayerInfo info : registeredPlayersInfo)
                if (info.getIp().equals(player.getNetConnection().getIpAddr())) {
        ...
        if (OlympiadConfig.OLYMPIAD_CHECK_HWID) {
            for (RegisteredPlayerInfo info : registeredPlayersInfo)
                if (info.getHwid().equals(player.getNetConnection().getHWID())) {
                    leader.sendMessage(message);
                    return false;
                }
        }
    ...
    private static boolean isValidHardware(Player leader, Player player, List<RegisteredPlayerInfo> infos) {
        if (OlympiadConfig.OLYMPIAD_CHECK_IP) {
            for (RegisteredPlayerInfo info : infos)
                if (info.getIp().equals(player.getNetConnection().getIpAddr())) {
                    leader.sendMessage("Member of party is already registered at the Olympiad Games!");
                    return false;
                }
        }
    ...
    public static synchronized boolean registerNoble(final Player noble, final CompType type) {
        ...
        if(!isValidHardware(noble, noble, type)) {
            return false;
        }
        ...
        switch (type) {
            case CLASSED: {
                ...
                if(_scheduledManagerTask==null)
                    _scheduledManagerTask = ThreadPoolManager.getInstance().schedule(new StartMatch(type), 2000L);
                break;
            }
            case NON_CLASSED: {
            ...
            case TEAM: {
                ...
                List<RegisteredPlayerInfo> infos = new ArrayList<>(TEAM_PARTY_SIZE);
                for (final Player member : members) {
                    ...
                    if(!isValidHardware(noble, member, type) || !isValidHardware(noble, member, infos)) {
                        return false;
                    }

                    infos.add(new RegisteredPlayerInfo(member.getObjectId(),
                            member.getNetConnection().getIpAddr(), member.getNetConnection().getHWID()));
                }

                registeredPlayersInfo.addAll(infos);
                teamBasedRegisters.put(noble.getObjectId(), members.stream().map(Player::getObjectId).collect(Collectors.toList()));
    ...
    private static synchronized void updateCompStatus() {
        ...
        if(_scheduledCompetitionTask != null)
            _scheduledCompetitionTask.cancel(false);
        _scheduledCompetitionTask = ThreadPoolManager.getInstance().schedule(new CompStartTask(), milliToStart);
    }
    ...
    private static class StartMatch extends RunnableImpl {
        CompType _type;
        private StartMatch(CompType type){
            _type = type;
        }
        @Override
        public void runImpl() throws Exception
        {
            _manager.startMatch(_type);
        }
    }
}