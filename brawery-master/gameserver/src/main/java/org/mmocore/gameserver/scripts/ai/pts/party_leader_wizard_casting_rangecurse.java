...
public class party_leader_wizard_casting_rangecurse extends party_leader_wizard_dd2
{
	...
	public party_leader_wizard_casting_rangecurse(final NpcInstance actor){super(actor);}
	...
	protected void onEvtAttacked(Creature attacker, int skill_name_id, int skill_id, int damage)
	{
		...
		{
			if (i_ai3 == 0 && AiUtils.FloatToInt(actor.currentHp / actor.getMaxHp() * 100) > 50)
			{
				if (Skill_GetConsumeMP(Buff1) < actor._currentMp && Skill_GetConsumeHP(Buff1) < actor.currentHp && Skill_InReuseDelay(Buff1) == 0)
				{
					AddUseSkillDesire(actor, Buff1, DesireType.HEAL, DesireMove.MOVE_TO_TARGET, 9999999);
				}
				if (Skill_GetConsumeMP(Buff2) < actor._currentMp && Skill_GetConsumeHP(Buff2) < actor.currentHp && Skill_InReuseDelay(Buff2) == 0)
				{
					AddUseSkillDesire(actor, Buff2, DesireType.HEAL, DesireMove.MOVE_TO_TARGET, 9999999);
				}
				i_ai3 = 1;
			}
