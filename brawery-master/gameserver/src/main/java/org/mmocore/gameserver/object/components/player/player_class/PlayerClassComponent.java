...
public class PlayerClassComponent {
    ...
    {
        int classId = getActiveClassId();
        if((classId < 0 || classId > 136) || (classId > 118 && classId < 123) || (classId > 57 && classId < 88))
        {
            return new CustomMessage("utils.classId.name.default").toString(playerRef);
        }
        return new CustomMessage("utils.classId.name." + classId).toString(playerRef);
    }
    ...
    public boolean addSubClass(final int classId, final boolean storeOld, final int certification) {
        ...
        for(final SkillLearn s : SkillAcquireHolder.getInstance().getMaxSkills(player))
        {
