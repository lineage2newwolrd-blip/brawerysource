...
public class _299_GatherIngredientsforPie extends Quest {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("emilly_q0299_0104.htm") && _state == CREATED) {
            ...
            if (st.ownItemCount(Honey_Pouch) < 100) {
            ...
            if (st.ownItemCount(Avellan_Spice) < 1) {
            ...
            if (st.ownItemCount(Fruit_Basket) < 1) {
                return "emilly_q0299_0602.htm";
            }
            st.takeItems(Fruit_Basket, -1);
            if (Rnd.chance(Reward_Varnish_Chance)) {
                st.giveReward(Varnish, 50, true);
            } else {
                st.giveReward(ADENA_ID, 25000, true);
            }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Emily && _state == STARTED) {
            if (cond == 1 && st.ownItemCount(Honey_Pouch) <= 99) {
                return "emilly_q0299_0106.htm";
            }
            if (cond == 2 && st.ownItemCount(Honey_Pouch) >= 100) {
                return "emilly_q0299_0105.htm";
            }
            if (cond == 3 && st.ownItemCount(Avellan_Spice) == 0) {
                return "emilly_q0299_0203.htm";
            }
            if (cond == 4 && st.ownItemCount(Avellan_Spice) == 1) {
                return "emilly_q0299_0301.htm";
            }
            if (cond == 5 && st.ownItemCount(Fruit_Basket) == 0) {
                return "emilly_q0299_0403.htm";
            }
            if (cond == 6 && st.ownItemCount(Fruit_Basket) == 1) {
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        if (qs.getState() != STARTED || qs.getCond() != 1 || qs.ownItemCount(Honey_Pouch) >= 100) {
            return null;
        }

        int npcId = npc.getNpcId();
        if (npcId == Wasp_Worker && Rnd.chance(Wasp_Worker_Chance) || npcId == Wasp_Leader && Rnd.chance(Wasp_Leader_Chance)) {
            qs.giveItems(Honey_Pouch, 1, true);
