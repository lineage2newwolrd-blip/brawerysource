...
public class Formulas {
    ...
    public static AttackInfo calcPhysDam(final Creature attacker, final Creature target, final SkillEntry skill, final boolean dual, final boolean blow, final boolean ss, final boolean onCrit) {
        ...
        if (skill != null) {
            ...
            if (target.getEffectList().getEffectByIndexAndType(522, EffectType.Debuff)!= null && calcRTBreak(attacker, target, info.crit)) {
    ...
    public static boolean calcRTBreak(final Creature activeChar, final Creature target, final boolean crit) {
        return Rnd.chance(crit ? 10.0 : 2.0);
    }
     ...
     */
    public static int calcBowReuseTime(Player player, Creature target, WeaponTemplate weapon)
    {
        if (weapon == null)
        {
            return 0;
        ...
        boolean bow = weapon.getItemType() != null
                && (weapon.getItemType() == WeaponTemplate.WeaponType.BOW
                || weapon.getItemType() == WeaponTemplate.WeaponType.CROSSBOW);
        if(player.isPlayer() && bow)
            bow = !player.isInMeleeTransform();
        ...
        reuse *= player.getReuseModifier(target);
        double atkSpd = player.getPAtkSpd();
        return (int) (calcPAtkSpd(atkSpd) * (1 + reuse / 1500.)) + 50;
    }
    ...
    public static boolean calcShldUse(final Creature attacker, final Creature target) {
        ...
        if (angle < 360 && !PositionUtils.isInFrontOf(target, attacker, angle)) {
            return false;
        }
