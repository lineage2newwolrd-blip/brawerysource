...
public class leto_lizardman_soldier extends warrior_aggressive
{
	public leto_lizardman_soldier(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 335)!=0 && AiUtils.OwnItemCount(target, 3727)!=0 && AiUtils.OwnItemCount(target, 3769) < 40)
		{
			...
			switch (code_info.getCode())
			{
				case 5:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						{
							GiveItem1(target, 3769, 1);
							if (AiUtils.OwnItemCount(target, 3769) >= 40)
							{
				...
				case 8:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						if (AiUtils.Rand(100) < 46 && AiUtils.OwnItemCount(target, 10367) < 1)
						{
