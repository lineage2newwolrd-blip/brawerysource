...
{
	public argos_boss_05(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (target.isPlayer())
					{
						...
						{
							GiveItem1(target, 7396, 1);
							SetFlagJournal(target, 74, 16);
							ShowQuestMark(target, 74);
							SoundEffect(target, "ItemSound.quest_middle");
							DeleteItem1(target, 7489, AiUtils.OwnItemCount(target, 7489));
							Say(AiUtils.MakeFString(7454, "", "", "", "", ""));
						}
						else
						{
							Say(AiUtils.MakeFString(7455, "", "", "", "", ""));
						}
					}
				}
			}
	...
	protected void onEvtSpawn()
	{
		...
		if (c0 !=null)
		{
			...
			if (c0.isPlayer())
			{
				Say(AiUtils.MakeFString(7453, c0.getName(), "", "", "", ""));
			}
		}
		super.onEvtSpawn();
	}
