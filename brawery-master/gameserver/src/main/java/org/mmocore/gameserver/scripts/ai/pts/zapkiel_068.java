...
{
	public zapkiel_068(final NpcInstance actor){super(actor);}
	...
	protected void onEvtAttacked(Creature attacker, int skill_name_id, int skill_id, int damage)
	{
		...
		if ((AiUtils.HaveMemo(c1, 68) == 1 && (c1.getClassId() == 128 || c1.getClassId() == 129)) && (AiUtils.GetMemoState(c1, 68) == 13 || AiUtils.GetMemoState(c1, 68) == 14))
		{
			if (c1.isPlayer())
			{
				if (c1.getObjectId() == actor.param2)
				{
					...
					if (i_quest0 == 1)
					{
						if (c1.isPlayer())
						{
							Say(AiUtils.MakeFString(6867, c1.getName(), "", "", "", ""));
						}
					}
					if (i_quest0 > 15)
					{
						SetMemoState(c1, 68, 14);
						if (c1.isPlayer())
						{
							Say(AiUtils.MakeFString(6868, c1.getName(), "", "", "", ""));
						}
