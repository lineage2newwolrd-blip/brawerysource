...
public class _144_PailakaInjuredDragon extends Quest {
    ...
    private static final int VSWARRIOR3 = 18635;//5

    private static final int VSCOMMAO1 = 18646;
    private static final int VSCOMMAO2 = 18654;
    private static final int VSCOMMAO3 = 18653;//6

    private static final int VSGMAG1 = 18649;
    private static final int VSGMAG2 = 18650;//6

    private static final int VSHGAPG1 = 18655;
    private static final int VSHGAPG2 = 18657;//6

    private static final int[] Pailaka3rd = new int[]{
            VSWARRIOR3, VSWARRIOR1, 18638, 18639, 18640, 18641, VSWARRIOR2, 18644, 18645, VSCOMMAO1, 18648, VSGMAG1, VSGMAG2, 18652, VSCOMMAO3, VSCOMMAO2, VSHGAPG1, 18656, VSHGAPG2, 18658,
            18659
    };
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("Enter")) {
            if(player.getActiveReflection()==null || player.getActiveReflection().getInstancedZoneId()!=izId)
                st.setMemoState("counter", "0", false);
            ...
            if (st.getInt("spells") < 4) {
                ...
                st.setMemoState("spells", String.valueOf(st.getInt("spells") + 1));
                htmltext = "32509-06.htm";
                return htmltext;
            }
            if (st.getInt("spells") == 4) {
                makeBuff(npc, player, skill[0], skill[1]);
                st.setMemoState("spells", "5");
            ...
            st.setMemoState("spells", "0");
            st.setMemoState("counter", "0");
            st.setCond(1);
            st.setState(STARTED);
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == KETRAOSHAMAN) {
            ...
            if (st.ownItemCount(SPEAR) > 0 && st.ownItemCount(STAGE1) == 0) {
                htmltext = "32509-01.htm";
            }
            if (st.ownItemCount(ENCHSPEAR) > 0 && st.ownItemCount(STAGE2) == 0) {
                htmltext = "32509-01.htm";
            }
            if (st.ownItemCount(SPEAR) == 0 && st.ownItemCount(STAGE1) > 0) {
            ...
            if (st.ownItemCount(SPEAR) == 0 && st.ownItemCount(ENCHSPEAR) == 0) {
                htmltext = "32509-07.htm";
            }
            if (st.ownItemCount(STAGE1) == 0 && st.ownItemCount(STAGE2) == 0) {
                htmltext = "32509-01.htm";
            }
            if (st.ownItemCount(SPEAR) > 0 && st.ownItemCount(STAGE1) > 0) {
                st.takeItems(SPEAR, 1);
            ...
            if (st.ownItemCount(LASTSPEAR) > 0) {
                htmltext = "32509-03.htm";
            }
            ...
            if (cond == 4) {
                ...
                st.exitQuest(false);
                player.getVitalityComponent().setVitality(AllSettingsConfig.VITALITY_LEVELS[4]);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        switch (npcId) {
            ...
            case VSWARRIOR3:
                if (counter < 5) {
                    counter++;
                    st.setMemoState("counter", String.valueOf(counter), false);
                    if (counter == 5){
        ...
        if (ArrayUtils.contains(Antelopes, npcId)) {
            st.dropItem(npc, ANTELOPDROP[Rnd.get(ANTELOPDROP.length)], Rnd.get(1, 5));
        }
