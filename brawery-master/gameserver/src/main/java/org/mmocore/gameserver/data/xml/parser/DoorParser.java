...
public final class DoorParser extends AbstractDirParser<DoorHolder> {
    ...
    protected void readData(final DoorHolder holder, final Element rootElement) throws Exception {
        for (final Element doorElement : rootElement.getChildren("door")) {
            ...
            int[] x_coords =  new int[]{Integer.parseInt(shapeElement.getAttributeValue("ax"))
                    ,Integer.parseInt(shapeElement.getAttributeValue("bx"))
                    ,Integer.parseInt(shapeElement.getAttributeValue("cx"))
                    ,Integer.parseInt(shapeElement.getAttributeValue("dx"))};
            int[] y_coords =  new int[]{Integer.parseInt(shapeElement.getAttributeValue("ay"))
                    ,Integer.parseInt(shapeElement.getAttributeValue("by"))
                    ,Integer.parseInt(shapeElement.getAttributeValue("cy"))
                    ,Integer.parseInt(shapeElement.getAttributeValue("dy"))};
            if(x_coords[0]==x_coords[1] & x_coords[2]==x_coords[3] & x_coords[1]==x_coords[3]) {
                if (y_coords[0] == y_coords[1] & y_coords[2] == y_coords[3]) {
                    x_coords[0] -= 4;
                    x_coords[1] += 4;
                    x_coords[2] += 4;
                    x_coords[3] -= 4;
                }
                else if (y_coords[1] == y_coords[2] & y_coords[0] == y_coords[3]) {
                    x_coords[0] -= 4;
                    x_coords[1] -= 4;
                    x_coords[2] += 4;
                    x_coords[3] += 4;
            ...
            else if(y_coords[0]==y_coords[1] & y_coords[2]==y_coords[3] & y_coords[1]==y_coords[3]) {
                if (x_coords[0] == x_coords[1] & x_coords[2] == x_coords[3]) {
                    y_coords[0] -= 4;
                    y_coords[1] += 4;
                    y_coords[2] += 4;
                    y_coords[3] -= 4;
                }
                else if (x_coords[1] == x_coords[2] & x_coords[0] == x_coords[3]) {
                    y_coords[0] -= 4;
                    y_coords[1] -= 4;
                    y_coords[2] += 4;
                    y_coords[3] += 4;
            ...
            for(int i=0; i<4; i++){
                shape.add(x_coords[i], y_coords[i]);
            }
            shape.setZmin(minz);
            shape.setZmax(maxz);
