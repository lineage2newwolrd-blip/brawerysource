...
public final class _691_MatrasSuspiciousRequest extends Quest {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("quest_accept")) {
            ...
            st.giveReward(ADENA_ID, GetMemoStateEx * 10000, true);
    ...
    public final String onKill(NpcInstance npc, QuestState st) {
        ...
        if (GetMemoState == 1)
            if (npcId == body_destroy) {
                int i0 = Rnd.get(1000);
                if (i0 <= 897) {
                    st.giveItems(q_red_stone_q0691, 1, true);
                ...
                int i0 = Rnd.get(1000);
                if (i0 <= 129) {
                    st.giveItems(q_red_stone_q0691, 3, true);
                    st.soundEffect(SOUND_ITEMGET);
                } else {
                    st.giveItems(q_red_stone_q0691, 2, true);
                    st.soundEffect(SOUND_ITEMGET);
                }
            }
        return null;
