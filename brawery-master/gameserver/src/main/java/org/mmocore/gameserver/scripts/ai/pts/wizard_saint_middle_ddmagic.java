...
{
	public int W_MiddleRangeDDMagic = 272039937;

	public wizard_saint_middle_ddmagic(final NpcInstance actor){super(actor);}
		...
		if (attacker.isPlayer() || IsInCategory(Category.summon_npc_group, attacker.getClassId()))
		{
			if (i_ai0 == 0)
			{
				...
				if (DistFromMe(attacker) > 100 && AiUtils.Rand(100) < 80)
				{
					if (i0 == 1)
					{
						if (Skill_GetConsumeMP(W_MiddleRangeDDMagic) < actor._currentMp && Skill_GetConsumeHP(W_MiddleRangeDDMagic) < actor.currentHp)
						{
							if (Skill_InReuseDelay(W_MiddleRangeDDMagic) == 0)
							{
								AddUseSkillDesire(attacker, W_MiddleRangeDDMagic, DesireType.ATTACK, DesireMove.MOVE_TO_TARGET, 1000000);
							}
							else
							{
								AddUseSkillDesire(attacker, W_MiddleRangeDDMagic, DesireType.ATTACK, DesireMove.MOVE_TO_TARGET, 1000000);
	...
	protected void onEvtMoveFinished(int x, int y, int z)
	{
		...
		if (i0 == 1)
		{
			...
			else if (Skill_GetConsumeMP(W_MiddleRangeDDMagic) < actor._currentMp && Skill_GetConsumeHP(W_MiddleRangeDDMagic) < actor.currentHp)
			{
				if (Skill_InReuseDelay(W_MiddleRangeDDMagic) == 0)
				{
					AddUseSkillDesire(h0.attacker, W_MiddleRangeDDMagic, DesireType.ATTACK, DesireMove.MOVE_TO_TARGET, 1000000);
				}
				else
				{
					AddUseSkillDesire(h0.attacker, W_MiddleRangeDDMagic, DesireType.ATTACK, DesireMove.MOVE_TO_TARGET, 1000000);
	...
	protected void onEvtUseSkillFinished(int skill_name_id, Creature target, boolean success)
	{
		...
		if (i_ai0 == 0)
		{
			...
			if (i0 == 1)
			{
				...
				else if (AiUtils.Rand(100) < 33 && AiUtils.GetAbnormalLevel(h0.attacker, Skill_GetAbnormalType(DeBuff)) <= 0)
				{
					if (Skill_GetConsumeMP(DeBuff) < actor._currentMp && Skill_GetConsumeHP(DeBuff) < actor.currentHp)
					{
						if (Skill_InReuseDelay(DeBuff) == 0)
