...
public class CharInfo extends GameServerPacket {
    ...
    public CharInfo(final Player cha, final boolean sendVisual) {
        this((Creature) cha, sendVisual);
    }

    public CharInfo(final DecoyInstance cha) {
        this((Creature) cha, false);
    }

    public CharInfo(final Creature cha, final boolean sendVisual) {
        if (cha == null) {
        ...
        if(sendVisual){
            DressArmorData dress = DressArmorHolder.getInstance().getArmor(player.getInventory().getPaperdollVisualItemId(Inventory.PAPERDOLL_CHEST));
            if(dress!=null) {
                inv[Inventory.PAPERDOLL_GLOVES][0] = dress.getGloves();
                inv[Inventory.PAPERDOLL_CHEST][0] = dress.getChest();
                inv[Inventory.PAPERDOLL_LEGS][0] = dress.getLegs();
                inv[Inventory.PAPERDOLL_FEET][0] = dress.getFeet();
                if(dress.getHelmet()!=0) {
                    inv[Inventory.PAPERDOLL_HAIR][0] = dress.getHelmet();
                    inv[Inventory.PAPERDOLL_DHAIR][0] = dress.getHelmet();
                }
                if(dress.getCloak()!=0) {
                    inv[Inventory.PAPERDOLL_BACK][0] = dress.getCloak();
                }
            }
            DressWeaponData weapon = DressWeaponHolder.getInstance().getWeapon(player.getInventory().getPaperdollVisualItemId(Inventory.PAPERDOLL_RHAND));
            if(weapon!=null){
                inv[Inventory.PAPERDOLL_RHAND][0] = weapon.getWeaponId();
            }
            weapon = DressWeaponHolder.getInstance().getWeapon(player.getInventory().getPaperdollVisualItemId(Inventory.PAPERDOLL_LRHAND));
            if(weapon!=null){
                inv[Inventory.PAPERDOLL_LRHAND][0] = weapon.getWeaponId();
            }
        }
        ...
        if(player.isInFightClub())
        {
            ...
            title_color = fightClubEvent.getVisibleTitleColor(player, title_color, false);
