    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == splinter_stakato) {
            if (st.ownItemCount(q_goldenram_badge2) < 1 && st.ownItemCount(q_splinter_chitin) < 100) {
                int i0 = Rnd.get(100);
                if (i0 < 50) {
                    st.giveItems(q_splinter_chitin, 1, true);
                    if (st.ownItemCount(q_goldenram_badge1) >= 1) {
                        if (st.ownItemCount(q_splinter_chitin) >= 100) {
                            st.soundEffect(SOUND_MIDDLE);
                        } else {
                            st.soundEffect(SOUND_ITEMGET);
                        }
                    } else if (st.ownItemCount(q_splinter_chitin) >= 100) {
                        st.soundEffect(SOUND_MIDDLE);
                    } else {
                        st.soundEffect(SOUND_ITEMGET);
                    }
            ...
            if (st.ownItemCount(q_goldenram_badge2) < 1 && st.ownItemCount(q_goldenram_badge1) >= 1 && st.ownItemCount(q_needle_chitin) < 100) {
                int i0 = Rnd.get(100);
                if (i0 < 50) {
                    st.giveItems(q_needle_chitin, 1, true);
                    if (st.ownItemCount(q_needle_chitin) >= 100) {
                        st.soundEffect(SOUND_MIDDLE);
                    } else {
                        st.soundEffect(SOUND_ITEMGET);
                    }
