...
public abstract class Effect extends RunnableImpl implements Comparable<Effect>, FuncOwner {
    ...
    private AbnormalType pts_abnormal_type;
    ...
    protected Effect(final Creature creature, final Creature target, final SkillEntry skill, final EffectTemplate template) {
        ...
        SkillData skillInfo = SkillDataHolder.getInstance().getSkillInfo(_displayId, _displayLevel);
        if(skillInfo!=null && skillInfo.abnormal_type != null)
            pts_abnormal_type = AbnormalType.valueFrom(skillInfo.abnormal_type);
        else
            pts_abnormal_type = AbnormalType.none;
    ...
    protected void onStart() {
        ...
        if (getTemplate()._abnormalEffects != null) {
            for (final AbnormalEffect e : getTemplate()._abnormalEffects) {
                getEffected().startAbnormalEffect(e, getEffector(), getSkill());
            }
        }
    ...
    public boolean maybeScheduleNext(final Effect newEffect) {
        ...
        else if (newEffect.getSkill().getTemplate().isAbnormalInstant())// если новый короче то зашедулить старый
        {
            suspend();
    ...
    public void addMatchSpelledIcon(Player player, ExEventMatchSpelledInfo ms)
    {
        if(!isActive() || isHidden())
            return;
        int duration = _skill.getTemplate().isToggle() ? AbnormalStatusUpdate.INFINITIVE_EFFECT : getTimeLeft();
        ms.addSpellRecivedPlayer(player);
        ms.addEffect(_displayId, _displayLevel, duration);
    }
    ...
    {
        if (skill == null || !skill.getTemplate().isHandler() || !skill.getTemplate().altUse())
            exit();
    }
}