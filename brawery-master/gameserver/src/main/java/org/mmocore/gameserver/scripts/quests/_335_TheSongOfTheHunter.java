...
public class _335_TheSongOfTheHunter extends Quest {
    ...
    private static int CalcItemsConds(QuestState st, int[][][] ItemsConds) {
        ...
        for (int[][] ItemsCond : ItemsConds) {
            if (st.ownItemCount(ItemsCond[0]) >= ItemsCond[1][0]) {
                result++;
            }
        }
        return result;
    ...
    private static int Get_Blood_Crystal_Level(QuestState st) {
        for (int i = q_blood_crystal.length - 1; i >= 0; i--) {
            if (st.ownItemCount(q_blood_crystal[i]) > 0) {
                return i;
            }
        }
        return -1;
    ...
    private static boolean Blood_Crystal2Adena(QuestState st, int Blood_Crystal_Level) {
        ...
        st.giveReward(ADENA_ID, 3400 * (int) Math.pow(2, Blood_Crystal_Level - 2), true);
        return true;
    }
    ...
    private static void GenList(QuestState st) {
        ...
        if (st.get("list") == null || st.get("list").isEmpty()) {
            long Laurel_Leaf_Pin_count = st.ownItemCount(Laurel_Leaf_Pin);
            int[] list = new int[5];
            ...
            st.setMemoState("list", String.valueOf(packedlist));
        }
    }
    ...
    private static String FormatList(QuestState st, Request[] requests) {
        ...
        for (int i = 0; i <= 5; i++) {
            String s = "<a action=\"bypass -h npc_%objectId%_QuestEvent _335_TheSongOfTheHunter 30745-request-" + requests[listpacked[i]].request_id + "\">" + requests[listpacked[i]].text + "</a>";
    ...
    private static Request GetCurrentRequest(QuestState st, Request[] requests) {
        for (Request r : requests) {
            if (st.ownItemCount(r.request_id) > 0) {
                return r;
            }
        }
        return null;
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30744_03.htm") && _state == CREATED) {
            if (st.ownItemCount(_1st_Test_Instructions) == 0) {
            ...
            if (st.ownItemCount(_2nd_Test_Instructions) == 0) {
                st.soundEffect(SOUND_MIDDLE);
                st.giveItems(_2nd_Test_Instructions, 1);
            }
        } else if (event.equalsIgnoreCase("30744_16.htm") && _state == STARTED) {
            if (st.ownItemCount(Laurel_Leaf_Pin) >= 20) {
                st.giveReward(ADENA_ID, 20000, true);
                event = "30744_17.htm";
            }
            st.soundEffect(SOUND_FINISH);
            st.exitQuest(true);
        } else if (event.equalsIgnoreCase("30746_03.htm") && _state == STARTED) {
            if (st.ownItemCount(_1st_Circle_Hunter_License) == 0 && st.ownItemCount(_2nd_Circle_Hunter_License) == 0) {
                return null;
            }
            if (st.ownItemCount(Cybellins_Dagger) == 0) {
                st.giveItems(Cybellins_Dagger, 1);
            }
            if (st.ownItemCount(Cybellins_Request) == 0) {
            ...
            if (st.ownItemCount(Laurel_Leaf_Pin) > 0) {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == Grey) {
            ...
            if (st.ownItemCount(_1st_Circle_Hunter_License) > 0) {
            ...
            if (st.ownItemCount(_2nd_Circle_Hunter_License) > 0) {
                return "30744_14.htm";
            }
        }
        ...
        if (npcId == Tor) {
            ...
            if (st.ownItemCount(_1st_Circle_Hunter_License) > 0) {
                ...
                if (request == null) {
                    ...
                    return st.ownItemCount(_2nd_Test_Instructions) > 0 ? "30745_03.htm" : "30745_03a.htm";
                }
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        } else if (st.ownItemCount(_2nd_Test_Instructions) > 0) {
        ...
        if (Items_Circle != null) {
            ...
            if (st.ownItemCount(_1st_Test_Instructions) > 0) {
                long Hakas_Head_count = st.ownItemCount(Hakas_Head);
                long Jakas_Head_count = st.ownItemCount(Jakas_Head);
                long Markas_Head_count = st.ownItemCount(Markas_Head);
                if (npcId == Breka_Orc_Warrior) {
                    ...
                    if (st.ownItemCount(Windsus_Aleph_Skin) == 0 && Rnd.chance(10)) {
                        st.addSpawn(Windsus_Aleph, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), 100, 300000);
                    }
                }
            } else if (st.ownItemCount(_2nd_Test_Instructions) > 0) {
                long Athus_Head_count = st.ownItemCount(Athus_Head);
                long Lankas_Head_count = st.ownItemCount(Lankas_Head);
                long Triskas_Head_count = st.ownItemCount(Triskas_Head);
                long Moturas_Head_count = st.ownItemCount(Moturas_Head);
                long Kalaths_Head_count = st.ownItemCount(Kalaths_Head);
        ...
        if (st.ownItemCount(_1st_Circle_Hunter_License) > 0 || st.ownItemCount(_2nd_Circle_Hunter_License) > 0) {
            if (st.ownItemCount(Cybellins_Request) > 0 && st.getPlayer().getActiveWeaponItem() != null && st.getPlayer().getActiveWeaponItem().getItemId() == Cybellins_Dagger) {
                int Blood_Crystal_Level = Get_Blood_Crystal_Level(st);
                    ...
                    for (int lizardmen_id : q_blood_crystal_lizardmen) {
                        if (npcId == lizardmen_id) {
                            if (Rnd.chance(50)) {
                                ...
                                st.soundEffect(Blood_Crystal_Level < 6 ? SOUND_MIDDLE : SOUND_JACKPOT);
            ...
            if (request != null) {
                ...
                if (request.spawnlist.containsKey(npcId) && st.ownItemCount(request.request_item) < request.request_count) {
                    ...
                    if (Rnd.chance(spawn_n_chance[1])) {
                        ...
                        if (spawn_n_chance[0] == 27149) {
                            Functions.npcSay(npc, NpcString.SHOW_ME_THE_PRETTY_SPARKLING_THINGS_THEY_RE_ALL_MINE);
                        }
                    }
                }
            }
        ...
        if ((npcId == 27160 || npcId == 27162 || npcId == 27164) && Rnd.chance(50)) {
            Functions.npcSay(npc, NpcString.WELL_TAKE_THE_PROPERTY_OF_THE_ANCIENT_EMPIRE);
    ...
    public static class Request {
        ...
        public boolean Complete(QuestState st) {
            if (st.ownItemCount(request_item) < request_count) {
                return false;
            }
            ...
            st.giveReward(ADENA_ID, reward_adena, true);
            st.removeMemo("list");
            return true;
        }
    }
