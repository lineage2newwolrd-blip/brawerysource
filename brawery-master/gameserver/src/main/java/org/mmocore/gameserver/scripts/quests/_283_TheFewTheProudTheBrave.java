        ...
        if (npcId == subelder_perwan) {
            if (event.equalsIgnoreCase("quest_accept")) {
                ...
                if (st.ownItemCount(q_claw_of_carmine_spider) >= 10)
                    st.giveReward(ADENA_ID, 2187 + 45 * st.ownItemCount(q_claw_of_carmine_spider), true);
                else
                    st.giveReward(ADENA_ID, 45 * st.ownItemCount(q_claw_of_carmine_spider), true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == carmine_spider) {
            if (GetMemoState == 1 * 10 + 1) {
                int i4 = Rnd.get(1000);
                if (i4 < 600) {
                    st.giveItems(q_claw_of_carmine_spider, 1, true);
