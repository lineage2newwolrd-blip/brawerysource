...
public class RequestExSendPost extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (ServerConfig.HIDE_SPAM_MSG) {
            if (ChatFilterConfig.containsSpamWord(_topic) || ChatFilterConfig.containsSpamWord(_body)) {
                allowSend = false;
            }
        }
        ...
        if(!allowSend)
            allowSend = attachments.stream().anyMatch(a-> a.getTemplate().isAdena() && a.getCount() > 1);
        if(allowSend)
            mail.setReceiverId(recieverId);
        else
