...
{
	public ratman_warrior(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			switch (code_info.getCode())
			{
				case 0:
				{
					SetCurrentQuestID(404);
					target = code_info.RandomSelectOne();
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						c1 = GetLastAttacker();
						if (AiUtils.HaveMemo(c_quest0, 404) != 0 && AiUtils.OwnItemCount(c_quest0, 1280) != 0 && AiUtils.OwnItemCount(c_quest0, 1281) == 0 && i_quest0 == 1 && AiUtils.Rand(100) < 80)
						{
							GiveItem1(c_quest0, 1281, 1);
							SetFlagJournal(c_quest0, 404, 3);
							ShowQuestMark(c_quest0, 404);
				...
				case 1:
				{
					...
					{
						c1 = GetLastAttacker();
						if (AiUtils.HaveMemo(c_quest0, 415) == 1 && AiUtils.OwnItemCount(c_quest0, 1607) == 1 && AiUtils.OwnItemCount(c_quest0, 1610) < 3 && i_quest0 == 1)
						{
	...
	protected void onEvtAttacked(Creature attacker, int skill_name_id, int skill_id, int damage)
	{
		...
		SetCurrentQuestID(404);
		if (AiUtils.HaveMemo(attacker, 404)!=0)
		{
			switch (i_quest0)
			{
				case 0:
