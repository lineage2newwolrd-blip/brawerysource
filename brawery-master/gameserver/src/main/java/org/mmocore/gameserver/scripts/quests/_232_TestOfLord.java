...
public class _232_TestOfLord extends Quest {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (state == CREATED) {
            ...
            if (event.equalsIgnoreCase("30565-12.htm") && st.ownItemCount(IMMORTAL_FLAME) > 0) {
                ...
                if (!st.getPlayer().getPlayerVariables().getBoolean(PlayerVariables.PROF2_3)) {
                    ...
                    st.giveReward(ADENA_ID, 161806, true);
            ...
            } else if (event.equalsIgnoreCase("30558-02.htm") && st.ownItemCount(ADENA_ID) >= 1000) {
            ...
            } else if (event.equalsIgnoreCase("30649-04.htm") && st.ownItemCount(BEAR_FANG_NECKLACE) > 0) {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        long ORDEAL_NECKLACE_COUNT = st.ownItemCount(ORDEAL_NECKLACE);
        long HUGE_ORC_FANG_COUNT = st.ownItemCount(HUGE_ORC_FANG);
        long SWORD_INTO_SKULL_COUNT = st.ownItemCount(SWORD_INTO_SKULL);
        long AXE_OF_CEREMONY_COUNT = st.ownItemCount(AXE_OF_CEREMONY);
        long MONSTEREYE_WOODCARVING_COUNT = st.ownItemCount(MONSTEREYE_WOODCARVING);
        long HANDIWORK_SPIDER_BROOCH_COUNT = st.ownItemCount(HANDIWORK_SPIDER_BROOCH);
        long BEAR_FANG_NECKLACE_COUNT = st.ownItemCount(BEAR_FANG_NECKLACE);
        long MARTANKUS_CHARM_COUNT = st.ownItemCount(MARTANKUS_CHARM);
        long IMMORTAL_FLAME_COUNT = st.ownItemCount(IMMORTAL_FLAME);
        long VARKEES_CHARM_COUNT = st.ownItemCount(VARKEES_CHARM);
        long MANAKIAS_AMULET_COUNT = st.ownItemCount(MANAKIAS_AMULET);
        long MANAKIAS_ORDERS_COUNT = st.ownItemCount(MANAKIAS_ORDERS);
        long BREKA_ORC_FANG_COUNT = st.ownItemCount(BREKA_ORC_FANG);
        long TANTUS_CHARM_COUNT = st.ownItemCount(TANTUS_CHARM);
        long NERUGA_AXE_BLADE_COUNT = st.ownItemCount(NERUGA_AXE_BLADE);
        long HATOS_CHARM_COUNT = st.ownItemCount(HATOS_CHARM);
        long URUTU_BLADE_COUNT = st.ownItemCount(URUTU_BLADE);
        long TIMAK_ORC_SKULL_COUNT = st.ownItemCount(TIMAK_ORC_SKULL);
        long SUMARIS_LETTER_COUNT = st.ownItemCount(SUMARIS_LETTER);
        long TAKUNA_CHARM_COUNT = st.ownItemCount(TAKUNA_CHARM);
        ...
        if (npcId == Jakal) {
            if (TANTUS_CHARM_COUNT > 0 && AXE_OF_CEREMONY_COUNT == 0) {
                ...
                return st.ownItemCount(ADENA_ID) < 1000 ? "30558-03.htm" : "30558-01.htm";
            }
            if (TANTUS_CHARM_COUNT == 0 && AXE_OF_CEREMONY_COUNT > 0) {
                return "30558-05.htm";
            }
        ...
        if (npcId == Takuna) {
            if (HANDIWORK_SPIDER_BROOCH_COUNT == 0) {
                ...
                if (st.ownItemCount(MARSH_SPIDER_FEELER) < 10 || st.ownItemCount(MARSH_SPIDER_FEET) < 10) {
                    return "30641-03.htm";
                }
        ...
        if (npcId == Chianta) {
            long CHIANTA_CHARM_COUNT = st.ownItemCount(CHIANTA_CHARM);
            if (MONSTEREYE_WOODCARVING_COUNT == 0) {
                if (CHIANTA_CHARM_COUNT == 0) {
                    return "30642-01.htm";
                }
                if (st.ownItemCount(CORNEA_OF_EN_MONSTEREYE) < 20) {
                    return "30642-03.htm";
                }
        ...
        if (npcId == Ancestor_Martankus) {
            ...
            if (MARTANKUS_CHARM_COUNT > 0) {
                if (cond == 5 || st.ownItemCount(RAGNA_CHIEF_NOTICE) > 0 && st.ownItemCount(RAGNA_ORC_HEAD) > 0) {
        ...
        if (npcId == First_Orc && st.ownItemCount(IMMORTAL_FLAME) > 0) {
            st.setCond(7);
            return "30643-01.htm";
        }
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        ...
        for (int item_id : _drop.itemList) {
            long ORDEAL_NECKLACE_COUNT = qs.ownItemCount(ORDEAL_NECKLACE);
            if (item_id == TIMAK_ORC_SKULL && !(ORDEAL_NECKLACE_COUNT > 0 && qs.ownItemCount(HATOS_CHARM) > 0 && qs.ownItemCount(SWORD_INTO_SKULL) == 0)) {
                continue;
            }

            if (item_id == BREKA_ORC_FANG && !(ORDEAL_NECKLACE_COUNT > 0 && qs.ownItemCount(VARKEES_CHARM) > 0 && qs.ownItemCount(MANAKIAS_ORDERS) > 0)) {
                continue;
            }

            if (npcId == Marsh_Spider && !(ORDEAL_NECKLACE_COUNT > 0 && qs.ownItemCount(TAKUNA_CHARM) > 0)) {
                continue;
            }

            if (npcId == Enchanted_Monstereye && !(ORDEAL_NECKLACE_COUNT > 0 && qs.ownItemCount(CHIANTA_CHARM) > 0)) {
                continue;
            }

            long count = qs.ownItemCount(item_id);
            if (cond == _drop.condition && count < _drop.maxcount && Rnd.chance(_drop.chance)) {
                qs.giveItems(item_id, 1);
                if (count + 1 == _drop.maxcount) {
                    if (cond == 4 && qs.ownItemCount(RAGNA_ORC_HEAD) > 0 && qs.ownItemCount(RAGNA_CHIEF_NOTICE) > 0) {
                        qs.setCond(5);
                    }
