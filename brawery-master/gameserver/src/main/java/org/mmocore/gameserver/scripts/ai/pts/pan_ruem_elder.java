...
{
	public pan_ruem_elder(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 335)!=0 && AiUtils.OwnItemCount(target, 3756)!=0 && AiUtils.OwnItemCount(target, 3798) < 40)
		{
		...
		if (!AiUtils.IsNull(code_info))
		{
			switch (code_info.getCode())
			{
				...
				case 1:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						GiveItem1(target, 3798, 1);
						if (AiUtils.OwnItemCount(target, 3798) >= 40)
						{
							SoundEffect(target, "ItemSound.quest_middle");
						}
						else
