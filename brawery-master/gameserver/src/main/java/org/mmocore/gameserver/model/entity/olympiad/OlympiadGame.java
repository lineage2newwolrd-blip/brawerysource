...
public class OlympiadGame {
    ...
    static {
        try {
            parseAdditionalRewards(customWinnerRewardItems, OtherCustom.additionalOlyWinnerRewardItems);
            parseAdditionalRewards(customLoserRewardItems, OtherCustom.additionalOlyLoserRewardItems);
        } catch (Exception e) {
    ...
    private static void addCustomRewards(TeamMember winnerMember, boolean isWinner) {
        ...
        int fame = isWinner ? OtherCustom.additionalOlyWinnerRewardFame : OtherCustom.additionalOlyLoserRewardFame;
        int crp = isWinner ? OtherCustom.additionalOlyWinnerRewardCrp : OtherCustom.additionalOlyLoserRewardCrp;

        for (RewardTemplate template : templates)
            if (Rnd.chance(template.getChance()))
                ItemFunctions.addItem(player, template.getId(), template.getCount());
    ...
    public void winGame(final OlympiadTeam winnerTeam, final OlympiadTeam looseTeam) {
        ...
        for (final Player player : winnerTeam.getPlayers()) {
            ItemFunctions.addItem(player, OlympiadConfig.ALT_OLY_BATTLE_REWARD_ITEM, getType().getReward());
        }
    ...
    public void broadcastNames() {
        if (_type != CompType.NON_CLASSED || !_team1.checkPlayers() || !_team2.checkPlayers())
            return;
        final Player player1 = _team1.getPlayers().get(0);
        final Player player2 = _team2.getPlayers().get(0);
        final String str1 = player2.getName() + ", " + player2.getPlayerClassComponent().getClassName();
        final String str2 = player1.getName() + ", " + player1.getPlayerClassComponent().getClassName();
        player1.sendPacket(
                new Say2(player1.getObjectId(), ChatType.COMMANDCHANNEL_ALL, "Enemy", str1, null),
                new ExShowScreenMessage(NpcString.NONE, 4000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, false, str1)
        );
        player2.sendPacket(
                new Say2(player2.getObjectId(), ChatType.COMMANDCHANNEL_ALL, "Enemy", str2, null),
                new ExShowScreenMessage(NpcString.NONE, 4000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, false, str2)
        );
    }
