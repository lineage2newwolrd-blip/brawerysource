            ...
            if (event.equalsIgnoreCase("quest_accept")) {
                ...
                if (st.ownItemCount(q_necklace_of_storming_party) >= 1) {
                    if (st.ownItemCount(q_necklace_of_storming_party) >= 10)
                        st.giveReward(ADENA_ID, st.ownItemCount(q_necklace_of_storming_party) * 3000, true);
                    else
                        st.giveReward(ADENA_ID, st.ownItemCount(q_necklace_of_storming_party) * 3000, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (GetMemoState == 1)
            if (npcId == elcroki) {
                int i4 = Rnd.get(1000);
                if (i4 < 448) {
                    st.giveItems(q_necklace_of_storming_party, 1, true);
