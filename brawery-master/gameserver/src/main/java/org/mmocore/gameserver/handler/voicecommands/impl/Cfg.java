    ...
    public boolean useVoicedCommand(final String command, final Player activeChar, final String args) {
        if ("cfg".equals(command)) {
            if (args != null) {
                ...
                if(param.length > 2)
                    param = Arrays.copyOfRange(param, 0, 2);
                if (param.length == 2) {
                    ...
                    if("hidedress".equalsIgnoreCase(param[0]))
                        if(param[1].equalsIgnoreCase("on"))
                        {
                            activeChar.getPlayerVariables().set(PlayerVariables.HIDE_VISUAL, "on", -1);
                            activeChar.setHideVisualFlag(true);

                            for(Player character : World.getAroundPlayers(activeChar))
                            {
                                activeChar.sendPacket(new CharInfo(character,false));
                            }
                        }
                        else if(param[1].equalsIgnoreCase("of"))
                        {
                            activeChar.getPlayerVariables().remove(PlayerVariables.HIDE_VISUAL);
                            activeChar.setHideVisualFlag(false);
                            for(Player character : World.getAroundPlayers(activeChar))
                            {
                                activeChar.sendPacket(new CharInfo(character,true));
                            }
                        }
                    ...
                    if (AllSettingsConfig.AUTO_LOOT_INDIVIDUAL) {
                        if ((param[0].equalsIgnoreCase("autoloot") || param[0].equalsIgnoreCase("autolooth")) &&AllSettingsConfig.AUTO_LOOT_INDIVIDUAL_ONLY_FOR_PREMIUM && !activeChar.getPremiumAccountComponent().hasBonus()) {
        ...
        //DELETE dialog.replace("%dli%", activeChar.getPlayerVariables().getBoolean(PlayerVariables.DROPLIST_ICONS) ? "On" : "Off");
        ...
        if(AllSettingsConfig.AltAllowNoCarrier)
            dialog.replace("%curcarrier%", activeChar.getPlayerVariables().getInt(PlayerVariables.NO_CARRIER_TIME, 0) + "");
        else
            dialog.replace("%curcarrier%", "N/A");
        dialog.replace("%hidedress%", activeChar.isHideVisualFlag() ? "On" : "Off");
