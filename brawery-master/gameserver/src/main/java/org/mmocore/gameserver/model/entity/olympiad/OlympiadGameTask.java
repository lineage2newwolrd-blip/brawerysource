...
public class OlympiadGameTask extends RunnableImpl {
    ...
    public void runImpl() throws Exception {
        ...
        try {
            ...
            if (!_game.checkPlayersOnline() && _status != BattleStatus.ValidateWinner && _status != BattleStatus.Ending  && _status != BattleStatus.PortBack) {
            ...
            switch (_status) {
                ...
                case Started: {
                    ...
                    if (OlympiadConfig.OLYMPIAD_CUSTOM_EFFECT!=null && !OlympiadConfig.OLYMPIAD_CUSTOM_EFFECT.isEmpty())
                        for(int effect : OlympiadConfig.OLYMPIAD_CUSTOM_EFFECT.keySet())
                            _game.getAllParticipants().forEach(p -> p.addSkill(SkillTable.getInstance().getSkillEntry(effect, OlympiadConfig.OLYMPIAD_CUSTOM_EFFECT.get(effect)), false));
                    break;
                }
                case Heal: {
                    if(OlympiadConfig.OLYMPIAD_ENEMY_INFO)
                        _game.broadcastNames();
                    _game.heal();
                ...
                case Ending: {
                    if (OlympiadConfig.OLYMPIAD_CUSTOM_EFFECT!=null && !OlympiadConfig.OLYMPIAD_CUSTOM_EFFECT.isEmpty())
                        for(int effect : OlympiadConfig.OLYMPIAD_CUSTOM_EFFECT.keySet())
                            _game.getAllParticipants().forEach(p -> p.removeSkill(effect, false));
                    _game.collapse();
                    _terminated = true;
