...
{
	public lich_king_acron(final NpcInstance actor){super(actor);}
	...
	protected void onEvtAttacked(Creature attacker, int skill_name_id, int skill_id, int damage)
	{
		...
		if (AiUtils.HaveMemo(c1, 94) == 1 && c1.getClassId() == 13 && (AiUtils.GetMemoState(c1, 94) == 13 || AiUtils.GetMemoState(c1, 94) == 14))
		{
			if (c1.isPlayer())
			{
				if (c1.getObjectId() == actor.param2)
				{
					...
					if (i_quest0 == 1)
					{
						if (c1.isPlayer())
						{
							Say(AiUtils.MakeFString(9466, c1.getName(), "", "", "", ""));
						}
					}
					if (i_quest0 > 15)
					{
						SetMemoState(c1, 94, 14);
						if (c1.isPlayer())
						{
							Say(AiUtils.MakeFString(9467, c1.getName(), "", "", "", ""));
						}
