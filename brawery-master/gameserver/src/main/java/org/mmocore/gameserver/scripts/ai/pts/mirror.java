...
public class mirror extends warrior_pa_casting_3skill_magical
{
	public mirror(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 335)!=0 && AiUtils.OwnItemCount(target, 3762)!=0 && AiUtils.OwnItemCount(target, 3804) < 40)
		{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 136) == 1 && AiUtils.GetMemoState(target, 136) == 3 && AiUtils.OwnItemCount(target, 9787) < 35)
		{
			...
			switch (code_info.getCode())
			{
				case 0:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						{
							GiveItem1(target, 3804, 1);
							if (AiUtils.OwnItemCount(target, 3804) >= 40)
							{
				...
				case 2:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						if (AiUtils.OwnItemCount(target, 9787) < 34)
						{
							if (AiUtils.OwnItemCount(target, 9787) >= 33)
							{
								SetFlagJournal(target, 136, 4);
							...
							GiveItem1(target, 9787, 2);
							if (i4 < 290 && AiUtils.OwnItemCount(target, 9787) < 33)
							{
								GiveItem1(target, 9787, 1);
							}
						}
						else if (AiUtils.OwnItemCount(target, 9787) == 34)
						{
							GiveItem1(target, 9787, 1);
							SetFlagJournal(target, 136, 4);
							ShowQuestMark(target, 136);
