...
public class ai_shadow_summoner extends detect_party_warrior {
    ...
    private static final int FeedTimer = 2010507;
    ...
    protected void onEvtTimerFiredEx(int timer_id, Object arg1, Object arg2) {
        ...
        if (timer_id == FeedTimer && i_ai3 == 0)
        {
            for(NpcInstance npc : actor.getAroundNpc(500, 200))
                if(npc.getNpcId() == 25731 && !npc.isDead()) {
                    clearTasks();
                    actor.getAggroList().addDamageHate(npc, 0, 100000000);
                }
            AddTimerEx(FeedTimer, 30 * 1000);
        }
    }
