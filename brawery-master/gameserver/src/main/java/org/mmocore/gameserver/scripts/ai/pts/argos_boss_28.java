...
{
	public argos_boss_28(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (target.isPlayer())
					{
						...
						{
							GiveItem1(target, 7419, 1);
							SetFlagJournal(target, 97, 16);
							ShowQuestMark(target, 97);
							SoundEffect(target, "ItemSound.quest_middle");
							DeleteItem1(target, 7512, AiUtils.OwnItemCount(target, 7512));
							Say(AiUtils.MakeFString(9754, "", "", "", "", ""));
						}
						else
						{
							Say(AiUtils.MakeFString(9755, "", "", "", "", ""));
						}
					}
				}
			}
	...
	protected void onEvtSpawn()
	{
		...
		if (c0 !=null)
		{
			...
			if (c0.isPlayer())
			{
				Say(AiUtils.MakeFString(9753, c0.getName(), "", "", "", ""));
			}
		}
		super.onEvtSpawn();
	}
