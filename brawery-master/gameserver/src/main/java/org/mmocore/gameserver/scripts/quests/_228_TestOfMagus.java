...
public class _228_TestOfMagus extends Quest {
    ...
    public void checkBooks(QuestState st) {
        if (st.ownItemCount(ToneOfWater) != 0 && st.ownItemCount(ToneOfFire) != 0 && st.ownItemCount(ToneOfWind) != 0 && st.ownItemCount(ToneOfEarth) != 0) {
            st.setCond(6);
            st.setState(STARTED);
        }
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        int cond = st.getCond();
        if (npcId == Rukal) {
            if (st.ownItemCount(MarkOfMagus) != 0) {
                htmltext = "completed";
                st.exitQuest(true);
            } else if (cond == 0) {
                ...
                if (!st.getPlayer().getPlayerVariables().getBoolean(PlayerVariables.PROF2_3)) {
                    ...
                    st.giveReward(ADENA_ID, 372154, true);
            ...
            if (st.ownItemCount(ToneOfFire) == 0) {
                if (st.ownItemCount(SalamanderCharm) == 0) {
                    htmltext = "30411-01.htm";
                    st.giveItems(SalamanderCharm, 1);
                    st.soundEffect(SOUND_MIDDLE);
                } else if (st.ownItemCount(FlameCrystal) < 5) {
            ...
            if (st.ownItemCount(ToneOfWind) == 0) {
                if (st.ownItemCount(SylphCharm) == 0) {
                    htmltext = "30412-01.htm";
                } else if (st.ownItemCount(HarpysFeather) < 20 || st.ownItemCount(WyrmsWingbone) < 10 || st.ownItemCount(WindsusMane) < 10) {
            ...
            if (st.ownItemCount(ToneOfEarth) == 0) {
                if (st.ownItemCount(SerpentCharm) == 0) {
                    htmltext = "30409-01.htm";
                } else if (st.ownItemCount(EnchantedMonsterEyeShell) < 10 || st.ownItemCount(EnchantedStoneGolemPowder) < 10 || st.ownItemCount(EnchantedIronGolemScrap) < 10) {
            ...
            if (st.ownItemCount(ToneOfWater) == 0) {
                if (st.ownItemCount(UndineCharm) == 0) {
                    htmltext = "30413-01.htm";
                    st.giveItems(UndineCharm, 1);
                    st.soundEffect(SOUND_MIDDLE);
                } else if (st.ownItemCount(DazzlingDrop) < 20) {
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 3 && st.ownItemCount(GoldenSeed1st) != 0 && st.ownItemCount(GoldenSeed2st) != 0 && st.ownItemCount(GoldenSeed3st) != 0) {
            st.setCond(4);
            st.setState(STARTED);
        }
        return null;
