...
public final class QuestState {
     ...
     * @param count
     */
    public void giveReward(final int itemId, final long count, final boolean rate) {
        if(rate)
            giveItems(itemId, (long)(count * getRateQuestsReward()), false);
        else
            giveItems(itemId, count, false);
    }
    /**
    ...
    public double getRateQuestsDrop() {
        ...
        return _quest.getRateDrop() * Bonus;
    }
    ...
    public double getRateQuestsReward() {
        ...
        return _quest.getRateReward() * Bonus;
    }
            ...
            switch (_quest.getParty()) {
                ...
                case Quest.PARTY_ALL:
                    if (actorPlayer.getParty() == null) {
                        ...
                        players.addAll(actorPlayer.getParty().getPartyMembers().stream().filter($member -> $member.isInRange(actorPlayer, AllSettingsConfig.ALT_PARTY_DISTRIBUTION_RANGE) || _quest.questId >=734 && _quest.questId<=738 && $member.getReflection() == actorPlayer.getReflection()).collect(Collectors.toList()));
                    }
                    break;
                case Quest.PARTY_CC:
