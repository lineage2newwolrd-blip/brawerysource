...
public class _368_TrespassingIntoTheSacredArea extends Quest {
                ...
                if (npcId == priestess_restina) {
                    if (st.ownItemCount(q_blade_stakatos_fang) < 1)
                        ...
                        if (st.ownItemCount(q_blade_stakatos_fang) >= 10)
                            st.giveReward(ADENA_ID, st.ownItemCount(q_blade_stakatos_fang) * 250 + 9450, true);
                        else
                            st.giveReward(ADENA_ID, st.ownItemCount(q_blade_stakatos_fang) * 250 + 2000, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == blade_stakato) {
            if (Rnd.get(100) < 60) {
                st.giveItems(q_blade_stakatos_fang, 1, true);
