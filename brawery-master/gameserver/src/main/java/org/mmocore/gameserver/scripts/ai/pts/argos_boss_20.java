...
{
	public argos_boss_20(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (target.isPlayer())
					{
						...
						{
							GiveItem1(target, 7411, 1);
							SetFlagJournal(target, 89, 16);
							ShowQuestMark(target, 89);
							SoundEffect(target, "ItemSound.quest_middle");
							DeleteItem1(target, 7504, AiUtils.OwnItemCount(target, 7504));
							Say(AiUtils.MakeFString(8954, "", "", "", "", ""));
						}
						else
						{
							Say(AiUtils.MakeFString(8955, "", "", "", "", ""));
						}
					}
				}
			}
	...
	protected void onEvtSpawn()
	{
		...
		if (c0 !=null)
		{
			...
			if (c0.isPlayer())
			{
				Say(AiUtils.MakeFString(8953, c0.getName(), "", "", "", ""));
			}
		}
		super.onEvtSpawn();
	}
