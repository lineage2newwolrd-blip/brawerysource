...
public class _206_DwarfTutorial extends Quest {
    // npc
    private static final int foreman_laferon = 30528; // Grand Master
    private static final int miner_mai = 30530; // Newbie Helper
    // mobs
    private static final int tutorial_gremlin = 18342;
    // questitem
    private static final int license_of_miner = 1496;
    ...
    public _206_DwarfTutorial() {
        super(false);
        addTalkId(foreman_laferon, miner_mai);
        addFirstTalkId(foreman_laferon, miner_mai);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("timer_newbie_helper")) {
            ...
            if (GetMemoStateEx == 3)
                st.playTutorialVoice("tutorial_voice_010f");
            return null;
            ...
            return null;
        } else if (event.equalsIgnoreCase("reply_31") && st.ownItemCount(license_of_miner) > 0) {
            htmltext = "foreman_laferon002.htm";
            st.takeItems(license_of_miner, 1);
            st.startQuestTimer("timer_grand_master", 60000, npc);
            if (GetMemoStateEx <= 3)
                qs.setMemoState("tutorial_quest_ex", String.valueOf(4), true);
            if (st.ownItemCount(soulshot_none_for_rookie) <= 200) {
                st.giveItems(soulshot_none_for_rookie, 200);
                st.playTutorialVoice("tutorial_voice_026");
                st.addExpAndSp(0, 50);
            }
        } else if (event.equalsIgnoreCase("reply_41")) {
            htmltext = "foreman_laferon005.htm";
            ...
            htmltext = "foreman_laferon006.htm";
            ThreadPoolManager.getInstance().schedule(new RadarTask(115632, -177996, -905, st.getPlayer()), 200L);
        }
        return htmltext;
    }
    ...
    public String onFirstTalk(NpcInstance npc, Player player) {
        ...
        // [dwarf_tutorial] 206 Dwarf's Tutorial
        ...
        switch (npcId) {
            case miner_mai:
                ...
                if (GetMemoStateEx < 0) {
                    ...
                    htmltext = "miner_mai004.htm";
                else if (GetMemoStateEx > 3)
                    htmltext = "carl005.htm";
                break;
            case foreman_laferon:
                if (st.ownItemCount(license_of_miner) > 0)
                    htmltext = "foreman_laferon001.htm";
                else if (st.ownItemCount(license_of_miner) == 0 && GetMemoStateEx > 3)
                    htmltext = "foreman_laferon004.htm";
                else if (st.ownItemCount(license_of_miner) == 0 && GetMemoStateEx <= 3)
                    htmltext = "foreman_laferon003.htm";
                break;
        }
        return htmltext;
    }
