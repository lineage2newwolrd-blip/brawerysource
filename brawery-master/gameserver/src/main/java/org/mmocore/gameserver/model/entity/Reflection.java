...
public class Reflection {
    ...
    protected List<Location>  _excluded = new ArrayList<>();
    ...
    public void collapse() {
        ...
        try {
            ...
            for(ObservePoint o : observers)
            {
                ...
                if (observer != null)
                {
                    if (observer.getObservableEvent()!=null)
                        observer.leaveMatchObserverMode();
                    else if (observer.getOlympiadObserveGame()!=null)
        ...
        for (final InstantZone.SpawnInfo s : si) {
            ...
            switch (s.getSpawnType()) {
                ...
                case 2: // локационный спаун
                    ...
                    addSpawn(c);
                    break;
                case 3: // исключащий спаун других нпц в рандомной точке(для либиринта)
                    c = new SimpleSpawner(s.getNpcId());
                    c.setReflection(this);
                    c.setRespawnDelay(s.getRespawnDelay(), s.getRespawnRnd());
                    c.setAmount(1);
    ...
    public boolean isExcludedLoc(Location loc)
    {
        if(loc == null)
            return false;
        for(Location l : _excluded)
            if(loc.distance(l)<10) {
                return true;
            }
        return false;
    }

    public void excludeLoc(Location loc)
    {
        if(loc != null)
            _excluded.add(loc);
        for(NpcInstance n : getNpcs())
            if(n.getLoc().distance(loc)<10) {
    ...
    public NpcInstance addDelayedSpawn(int npcId, Location loc, int randomOffset, int spawnDelay)
    {
        if(_isCollapseStarted)
            return null;
        if(spawnDelay < 25)
            spawnDelay = 25;
        int _delay =  spawnDelay + (int)(long)(System.currentTimeMillis() / 1000);
        SimpleSpawner sp = new SimpleSpawner(NpcHolder.getInstance().getTemplate(npcId));
        sp.setLoc(randomOffset > 0 ? Location.findPointToStay(loc, 0, randomOffset, getGeoIndex()) : loc);
        sp.setReflection(this);
        sp.setAmount(1);
        sp.setRespawnTime(_delay);
        sp.doSpawn(false);
        sp.startRespawn();
        return sp.getLastSpawn();
    }
    ...
    public void spawnByGroup(final String name, boolean instant) {
        final List<Spawner> list = _spawners.get(name);
        if (list == null) {
            throw new IllegalArgumentException();
        }
