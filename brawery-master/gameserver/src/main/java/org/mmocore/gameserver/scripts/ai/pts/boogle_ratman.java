...
public class boogle_ratman extends warrior_passive
{
	public boogle_ratman(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			if (code_info.getCode() == 0)
			{
				...
				{
					c1 = GetLastAttacker();
					if (AiUtils.HaveMemo(c_quest0, 418) == 1 && AiUtils.OwnItemCount(c_quest0, 1632) == 1 && AiUtils.OwnItemCount(c_quest0, 1636) < 10 && i_quest0 == 1)
					{
						if (AiUtils.Rand(10) < 7)
						{
							if (AiUtils.OwnItemCount(c_quest0, 1636) == 9)
							{
								GiveItem1(c_quest0, 1636, 1);
								if (AiUtils.OwnItemCount(c_quest0, 1637) >= 2)
								{
									SetFlagJournal(c_quest0, 418, 2);
