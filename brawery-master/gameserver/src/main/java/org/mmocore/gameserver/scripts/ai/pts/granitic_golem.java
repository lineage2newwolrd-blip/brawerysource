...
{
	public granitic_golem(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 216) == 1 && AiUtils.OwnItemCount(target, 3122) == 1 && AiUtils.OwnItemCount(target, 3129) == 1 && AiUtils.OwnItemCount(target, 3131) < 70)
		{
			...
			switch (code_info.getCode())
			{
				case 1:
				{
					...
					while (!AiUtils.IsNull(target = code_info.Next()))
					{
						...
						{
							SetCurrentQuestID(216);
							GiveItem1(target, 3131, 7);
							if (AiUtils.OwnItemCount(target, 3131) >= 63)
							{
