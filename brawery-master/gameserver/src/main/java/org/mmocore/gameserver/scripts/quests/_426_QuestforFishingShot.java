    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (id == CREATED) {
            switch (isAvailableFor(st.getPlayer())) {
                case LEVEL:
                    htmltext = st.getPlayer().getLanguage() == Language.ENGLISH ? "This quest available for 1 and from 82 level." : "Данный квест доступен игрокам с 1 и до 80 уровня.";
                    break;
                default:
                    htmltext = "1.htm";
                    break;
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (Rnd.chance(30)) {
            if (Arrays.binarySearch(HMOBS, npcId) >= 0) {
                st.giveItems(SWEET_FLUID, Rnd.get(5) + 1, true);
            } else {
                st.giveItems(SWEET_FLUID, Rnd.get(3) + 1, true);
            }
