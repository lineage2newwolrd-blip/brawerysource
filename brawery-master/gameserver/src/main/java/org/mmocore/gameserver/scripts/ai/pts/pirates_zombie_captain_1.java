...
public class pirates_zombie_captain_1 extends warrior_aggressive_physicalspecial
{
	public pirates_zombie_captain_1(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 365)!=0)
		{
		...
		if (!AiUtils.IsNull(code_info))
		{
			switch (code_info.getCode())
			{
				case 0:
				{
					SetCurrentQuestID(365);
					target = code_info.RandomSelectOne();
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						if (AiUtils.Rand(100) < 40)
						{
							GiveItem1(target, 5873, 1);
							SoundEffect(target, "ItemSound.quest_itemget");
							SetMemoState(target, 365, 0);
						}
					}
					break;
				}
