...
public class CubicComponent {
    ...
    public CubicComponent(final Player owner, final DefaultCubicData template) {
        ...
        _time_init = System.currentTimeMillis();
        initAction();
        startTask();
    ...
    private void startTask() {
        if (_delay > 0) {
            ...
            _scheduledActionTask = Optional.of(ThreadPoolManager.getInstance().scheduleAtFixedRate(_actionTask.get(), 1000, _delay * 1000));
        }
        if (_duration > 0) {
