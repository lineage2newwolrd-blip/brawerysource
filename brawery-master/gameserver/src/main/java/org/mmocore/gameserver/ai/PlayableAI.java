...
public class PlayableAI extends CharacterAI {
    ...
    protected void thinkFollow() {
        ...
        if (actor.isInRange(target, offset + 75) || actor.isMovementDisabled()) {
            clientActionFailed();
        }
    ...
    protected void thinkInteract() {
        ...
        if (actor.isInRangeZ(target, range)) {
            ...
            actor.moveToLocation(target.getLoc(), 40, false);
    ...
    protected void thinkAttack(final boolean checkRange) {
        ...
        if (attack_target == null || !isPosessed && !(_forceUse ? attack_target.isAttackable(actor) : attack_target.isAutoAttackable(actor))) {
            setIntention(AI_INTENTION_ACTIVE);
            actor.sendActionFailed();
            return;
        }
        ...
        if (attackTarget instanceof DoorInstance) {
        ...
        if (actor.isInRangeZ(attack_target, range + 12)) {
            if (!canSee) {
                actor.sendPacket(SystemMsg.CANNOT_SEE_TARGET);
                setIntention(AI_INTENTION_ACTIVE);
                actor.sendActionFailed();
...
*/
    ...
    public void lockTarget(final Creature target) {
        ...
        if (target == null || target.isDead()) {
            ...
            actor.setLockedTarget(true);//fixed: на оффе нельзя отменять таргет
    ...
    protected class ThinkFollow extends RunnableImpl {
        ...
        protected void runImpl() throws Exception {
            ...
            if (!actor.isAfraid() && !actor.isInRange(target, offset + 75) && (!actor.isFollow || actor.getFollowTarget() != target)) {
    ...
    protected class ExecuteFollow extends RunnableImpl {
        ...
        protected void runImpl() {
            if (_target.isDoor()) {
                ...
                _actor.followToCharacter(_target, _range, _actor.isSummon() || _actor.isPet());
            }
        }
    }
}