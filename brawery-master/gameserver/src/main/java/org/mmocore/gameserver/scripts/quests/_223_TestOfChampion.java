...
public class _223_TestOfChampion extends Quest {
    ...
    public _223_TestOfChampion() {
        ...
        addLevelCheck(39);
        addClassIdCheck(ClassId.warrior, ClassId.orc_raider);
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        ClassId class_id = st.getPlayer().getPlayerClassComponent().getClassId();
        if (npcId == Ascalon) {
            if (cond == 0) {
        ...
        for (int[] drop : DROPLIST) {
            if (drop[2] == npcId && drop[0] == cond) {
                ...
                for (int[] drop2 : DROPLIST) {
                    if (drop2[0] == cond && st.ownItemCount(drop2[3]) < drop2[5]) {
                        return null;
                    }
                }
