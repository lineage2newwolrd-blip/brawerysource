...
public class _296_TarantulasSpiderSilk extends Quest {
                ...
                if (npcId == trader_mion) {
                    if (st.ownItemCount(tarantula_spider_silk) < 1)
                        ...
                        if (st.ownItemCount(tarantula_spider_silk) >= 10)
                            st.giveReward(ADENA_ID, st.ownItemCount(tarantula_spider_silk) * 30 + 2000, true);
                        else
                            st.giveReward(ADENA_ID, st.ownItemCount(tarantula_spider_silk) * 30, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == crimson_tarantula || npcId == hunter_tarantula || npcId == plunder_tarantula) {
            int i0 = Rnd.get(100);
            if (i0 > 95) {
                st.giveItems(tarantula_spinnerette, 1, true);
                st.soundEffect(SOUND_ITEMGET);
            } else if (i0 > 45) {
                st.giveItems(tarantula_spider_silk, 1, true);
                st.soundEffect(SOUND_ITEMGET);
            }
        }
        return null;
