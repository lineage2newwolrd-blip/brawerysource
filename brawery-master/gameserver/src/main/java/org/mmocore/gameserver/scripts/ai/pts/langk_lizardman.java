...
{
	public langk_lizardman(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		target = attacker;
		if (AiUtils.HaveMemo(target, 340)!=0 && AiUtils.GetMemoState(target, 340) == 3)
		{
		...
		if (!AiUtils.IsNull(code_info))
		{
			switch (code_info.getCode())
			{
				case 1:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						if (AiUtils.OwnItemCount(target, 4256) == 0)
						{
							if (AiUtils.Rand(100) <= 18)
							{
								GiveItem1(target, 4256, 1);
								SoundEffect(target, "ItemSound.quest_itemget");
							}
						}
						else if (AiUtils.OwnItemCount(target, 4256) >= 1 && AiUtils.OwnItemCount(target, 4257) == 0)
						{
							if (AiUtils.Rand(100) <= 18)
							{
								GiveItem1(target, 4257, 1);
								SoundEffect(target, "ItemSound.quest_itemget");
							}
						}
					}
