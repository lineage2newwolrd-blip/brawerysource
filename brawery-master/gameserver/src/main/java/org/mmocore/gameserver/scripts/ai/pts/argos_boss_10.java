...
{
	public argos_boss_10(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (target.isPlayer())
					{
						...
						{
							GiveItem1(target, 7401, 1);
							SetFlagJournal(target, 79, 16);
							ShowQuestMark(target, 79);
							SoundEffect(target, "ItemSound.quest_middle");
							DeleteItem1(target, 7494, AiUtils.OwnItemCount(target, 7494));
							Say(AiUtils.MakeFString(7954, "", "", "", "", ""));
						}
						else
						{
							Say(AiUtils.MakeFString(7955, "", "", "", "", ""));
						}
					}
				}
			}
	...
	protected void onEvtSpawn()
	{
		...
		if (c0 !=null)
		{
			...
			if (c0.isPlayer())
			{
				Say(AiUtils.MakeFString(7953, c0.getName(), "", "", "", ""));
			}
		}
		super.onEvtSpawn();
	}
