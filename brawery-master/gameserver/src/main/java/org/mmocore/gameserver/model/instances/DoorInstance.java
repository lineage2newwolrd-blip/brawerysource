...
public final class DoorInstance extends Creature implements GeoCollision {
    ...
    private int lastDamageGradeUpdate = -1;
    ...
    protected boolean needStatusUpdate() {
        ...
        int stateMask = (getTemplate().isTargetable() ? 1 : 0)
                | (isOpen() ? 0 : 1) << 1
                | (isHPVisible() ? 1 : 0) << 2
                | (isInvul() ? 1 : 0) << 3
                | (getEvent(SiegeEvent.class) != null ? 1 : 0) << 4;
        if (stateMask != lastStateMask) {
            lastStateMask = stateMask;
            result = true;
        }

        int currentHpBar = (int) (getCurrentHp() * CLIENT_BAR_SIZE / getMaxHp());
        if (currentHpBar != _lastHpBarUpdate) {
            _lastHpBarUpdate = currentHpBar;
            result = true;
        }

        int currentDamageGrade = getDamage();
        if (currentDamageGrade != lastDamageGradeUpdate) {
            lastDamageGradeUpdate = currentDamageGrade;
            result = true;
        }
