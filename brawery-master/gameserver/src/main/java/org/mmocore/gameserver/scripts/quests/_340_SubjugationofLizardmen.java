...
public class _340_SubjugationofLizardmen extends Quest {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("30385-4.htm") && _state == CREATED) {
        ...
        } else if (event.equalsIgnoreCase("30385-6.htm") && _state == STARTED && cond == 1 && st.ownItemCount(TRADE_CARGO) >= 30) {
            st.setCond(2);
            ...
            st.soundEffect(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("30037-4.htm") && _state == STARTED && cond == 6 && st.ownItemCount(EVIL_SPIRIT_OF_DARKNESS) > 0) {
            st.setCond(7);
            st.takeItems(EVIL_SPIRIT_OF_DARKNESS, -1);
            st.soundEffect(SOUND_MIDDLE);
        } else if (event.equalsIgnoreCase("30385-10.htm") && _state == STARTED && cond == 7) {
            st.giveReward(ADENA_ID, 14700, true);
            st.soundEffect(SOUND_FINISH);
            st.exitQuest(false);
        } else if (event.equalsIgnoreCase("30385-7.htm") && _state == STARTED && cond == 1 && st.ownItemCount(TRADE_CARGO) >= 30) {
            st.takeItems(TRADE_CARGO, -1);
            st.giveReward(ADENA_ID, 4090, true);
            st.soundEffect(SOUND_FINISH);
            st.exitQuest(false);
        }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (npcId == WEITSZ && cond == 1) {
            return st.ownItemCount(TRADE_CARGO) < 30 ? "30385-8.htm" : "30385-5.htm";
        }
        ...
        if (npcId == ADONIUS && cond == 3) {
            if (st.ownItemCount(ROSARY) == 0 || st.ownItemCount(HOLY_SYMBOL) == 0) {
                return "30375-4.htm";
            }
        ...
        if (npcId == LEVIAN && cond == 6 && st.ownItemCount(EVIL_SPIRIT_OF_DARKNESS) > 0) {
            return "30037-3.htm";
        }
