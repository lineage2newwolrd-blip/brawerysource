...
public class ServitorAI extends PlayableAI {
    ...
    private long _checkRunTimestamp = 0;
    ...
    protected void onEvtAttacked(final Creature attacker, final SkillEntry skill, final int damage) {
        ...
        if (attacker != null) {
            ...
            if (now - _checkRunTimestamp > 3000L && actor.isFollowMode() && !actor.isMoving && !actor.getPlayer().isDead() && actor.getDistance(actor.getPlayer()) < 1000) // additional check that grants safety move
            {
                _checkRunTimestamp = now;
                storeIntention();
                if(actor.getPlayer() != attacker)
                    actor.moveToLocation(Location.findAroundPosition(actor.getPlayer(), 100), 0, false);
                else if(actor.getPlayer() == attacker)
                    actor.moveToLocation(actor.getPlayer().getLoc(), 0, false);
