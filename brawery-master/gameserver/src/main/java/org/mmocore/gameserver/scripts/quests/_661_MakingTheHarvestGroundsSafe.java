        ...
        if (npcId == warehouse_keeper_norman)
            if (event.equalsIgnoreCase("quest_accept")) {
                ...
                if (st.ownItemCount(q_big_hornet_sting) + st.ownItemCount(q_cloud_gem) + st.ownItemCount(q_young_araneid_claw) >= 10)
                    st.giveReward(ADENA_ID, 5773 + 57 * st.ownItemCount(q_big_hornet_sting) + 56 * st.ownItemCount(q_cloud_gem) + 60 * st.ownItemCount(q_young_araneid_claw), true);
                else
                    st.giveReward(ADENA_ID, 57 * st.ownItemCount(q_big_hornet_sting) + 56 * st.ownItemCount(q_cloud_gem) + 60 * st.ownItemCount(q_young_araneid_claw), true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (GetMemoState == 1 * 10 + 1)
            if (npcId == giant_poison_bee) {
                int i4 = Rnd.get(1000);
                if (i4 < 508) {
                    st.giveItems(q_big_hornet_sting, 1, true);
                    st.soundEffect(SOUND_ITEMGET);
                }
            } else if (npcId == cloudy_beast) {
                int i4 = Rnd.get(1000);
                if (i4 < 500) {
                    st.giveItems(q_cloud_gem, 1, true);
                    st.soundEffect(SOUND_ITEMGET);
                }
            } else if (npcId == young_araneid) {
                int i4 = Rnd.get(1000);
                if (i4 < 516) {
                    st.giveItems(q_young_araneid_claw, 1, true);
                    st.soundEffect(SOUND_ITEMGET);
                }
            }
        return null;
