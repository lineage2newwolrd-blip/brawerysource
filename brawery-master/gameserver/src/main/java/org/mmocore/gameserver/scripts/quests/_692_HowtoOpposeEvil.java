...
public class _692_HowtoOpposeEvil extends Quest {
    ...
    private static final int[] SOD = {
            22552,//mutant_drake
            22541,//draconian_legionarii
            22550,//ravager_silen_d
            22551,//disciple_silen_d
            22596,//draconian_centurion_fix
            22544,//draconian_mage
            22540,//draconian_centurion
            22547,//draconian_healer
            22542,//draconian_officer_leader
            22543,//draconian_officer
            22539,//draconian_primus_plius
            22546,//draconian_berserker
            22548,//draconian_peltast
            22536,//draconian_eliteguard
            22538,//draconian_legatus
            22537//draconian_archmage
    };
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("take_test") && cond == 0) {
            ...
            if (st.ownItemCount(BreathofTiat) == 0) {
                ...
                st.giveReward(ADENA_ID, st.ownItemCount(BreathofTiat) * 2500, true);
            ...
            if (st.ownItemCount(PortionofaSoul) == 0) {
                ...
                st.giveReward(ADENA_ID, st.ownItemCount(PortionofaSoul) * 2500, true);
            ...
            if (st.ownItemCount(ConcentratedSpiritEnergy) == 0) {
                ...
                st.giveReward(ADENA_ID, st.ownItemCount(ConcentratedSpiritEnergy) * 25000, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (cond == 3) {
            if (ArrayUtils.contains(SOD, npcId)) {
                st.rollAndGive(FleetSteedTroupsTotem, 1, 27); //Шанс на глаз и с потолка. Умножается на рейт квестов и делится на кол-во членов группы. Для нас это (nRate*3)/9
            } else if (ArrayUtils.contains(SOI, npcId)) {
                st.rollAndGive(NucleusofanIncompleteSoul, 1, 69); //Шанс на глаз и с потолка. Умножается на рейт квестов и делится на кол-во членов группы. Для нас это (nRate*3)/9
            } else if (ArrayUtils.contains(SOA, npcId)) {
                st.rollAndGive(SpiritStoneDust, 1, 16); //Шанс на глаз и с потолка. Умножается на рейт квестов и делится на кол-во членов группы. Для нас это (nRate*3)/9
            }
        }
        return null;
    }
