...
{
	public boogle_ratman_leader(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			if (code_info.getCode() == 0)
			{
				...
				{
					c1 = GetLastAttacker();
					if (AiUtils.HaveMemo(c_quest0, 418) == 1 && AiUtils.OwnItemCount(c_quest0, 1632) == 1 && AiUtils.OwnItemCount(c_quest0, 1637) < 2 && i_quest0 == 1)
					{
						if (AiUtils.Rand(10) < 5)
						{
							if (AiUtils.OwnItemCount(c_quest0, 1637) == 1)
							{
								GiveItem1(c_quest0, 1637, 1);
								if (AiUtils.OwnItemCount(c_quest0, 1636) >= 10)
								{
