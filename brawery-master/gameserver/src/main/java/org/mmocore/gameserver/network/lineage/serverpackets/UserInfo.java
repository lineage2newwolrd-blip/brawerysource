...
public class UserInfo extends GameServerPacket {
    ...
    public UserInfo(final Player player) {
        ...
        int setId=player.getInventory().getPaperdollVisualItemId(Inventory.PAPERDOLL_CHEST);
        if(!player.isInOlympiadMode() && setId!=0){
            DressArmorData dress = DressArmorHolder.getInstance().getArmor(setId);
            inv[Inventory.PAPERDOLL_GLOVES][1] = dress.getGloves();
            inv[Inventory.PAPERDOLL_CHEST][1] = dress.getChest();
            inv[Inventory.PAPERDOLL_LEGS][1] = dress.getLegs();
            inv[Inventory.PAPERDOLL_FEET][1] = dress.getFeet();
            if(dress.getHelmet()!=0) {
                inv[Inventory.PAPERDOLL_HAIR][1] = dress.getHelmet();
                inv[Inventory.PAPERDOLL_DHAIR][1] = dress.getHelmet();
            }
            if(dress.getCloak()!=0) {
                inv[Inventory.PAPERDOLL_BACK][1] = dress.getCloak();
            }
        }
        int weaponR=player.getInventory().getPaperdollVisualItemId(Inventory.PAPERDOLL_RHAND);
        if(!player.isInOlympiadMode() && weaponR!=0){
            DressWeaponData weapon = DressWeaponHolder.getInstance().getWeapon(weaponR);
            inv[Inventory.PAPERDOLL_RHAND][1] = weapon.getWeaponId();
        }
        int weaponLR=player.getInventory().getPaperdollVisualItemId(Inventory.PAPERDOLL_LRHAND);
        if(!player.isInOlympiadMode() && weaponLR!=0){
            DressWeaponData weapon = DressWeaponHolder.getInstance().getWeapon(weaponLR);
            inv[Inventory.PAPERDOLL_LRHAND][1] = weapon.getWeaponId();
        }
        ...
        if(player.isInFightClub())
        {
            ...
            name = fightClubEvent.getVisibleName(player, name, true);
