...
public class NevitComponent {
    ...
    private static final int MIN_BONUS_EFFECT_TIME = 40; // 40 секунд минимум длится эффект бонуса нэвита.
    ...
    public void addPoints(int val) {
        ...
        if (_points > MAX_POINTS) {
            _percent = 0;
            _points = 0;
            int bonusTime = _time/60;
            startNevitEffect(bonusTime < MIN_BONUS_EFFECT_TIME ? MIN_BONUS_EFFECT_TIME : bonusTime);
        }
        player.sendPacket(new ExNavitAdventPointInfo(_points));
    }
