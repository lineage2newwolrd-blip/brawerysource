...
public class CharStatsChangeRecorder<T extends Creature> {
    ...
    protected boolean set(int flag, boolean oldValue, boolean newValue) {
        if (oldValue != newValue)
            _changes |= flag;
        return newValue;
    }
    ...
    protected void refreshStats() {
        ...
        _runSpeed = set(BROADCAST_CHAR_INFO, _runSpeed, _activeChar.getMoveSpeed());
        ...
        //_curHp = set(SEND_STATUS_INFO, _curHp, (int) _activeChar.getCurrentHp());
        _maxHp = set(SEND_STATUS_INFO, _maxHp, (int) _activeChar.getMaxHp());
        //_curMp = set(SEND_STATUS_INFO, _curMp, (int) _activeChar.getCurrentMp());
        _maxMp = set(SEND_STATUS_INFO, _maxMp, _activeChar.getMaxMp());

        _level = set(SEND_CHAR_INFO, _level, _activeChar.getLevel());

        _abnormalEffects = set(BROADCAST_CHAR_INFO, _abnormalEffects, _activeChar.getAbnormalEffect(AbnormalEffectType.FIRST));
        _abnormalEffects2 = set(BROADCAST_CHAR_INFO, _abnormalEffects2, _activeChar.getAbnormalEffect(AbnormalEffectType.SECOND));
        _abnormalEffects3 = set(BROADCAST_CHAR_INFO, _abnormalEffects3, _activeChar.getAbnormalEffect(AbnormalEffectType.BRANCH));
