...
public class Quest implements OnInitScriptListener {
    ...
    private double customRateDrop = 0;
    private double customRateReward = 0;
    ...
    public Quest(final int party) {
        ...
        if (param.isPresent()) {
            customRateDrop = param.get().getRateDrop();
            customRateReward = param.get().getRateReward();
        }
    }
    ...
    public double getRateDrop() {
        if (customRateDrop != 0) {
            return customRateDrop;
        }
        return ServerConfig.RATE_QUESTS_DROP;
    }

    public double getRateReward() {
        if (customRateReward != 0) {
            return customRateReward;
        }
        return ServerConfig.RATE_QUESTS_REWARD;
    }
