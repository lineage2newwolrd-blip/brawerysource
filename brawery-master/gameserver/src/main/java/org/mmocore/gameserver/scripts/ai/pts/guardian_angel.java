...
public class guardian_angel extends warrior_aggressive_casting_3skill_approach_revival
{
	public guardian_angel(final NpcInstance actor){super(actor);}
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			if (code_info.getCode() == 2)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					...
					if (i1 == 0)
					{
						if (AiUtils.HaveMemo(c1, 348)!=0 && AiUtils.GetMemoStateEx(c1, 348, 0) == 17 && AiUtils.OwnItemCount(c1, 4294) > 0)
						{
							c2 = c1;
						}
					}
					else
					{
						for (i0 = 0; i0 < i1; i0++)
						{
							...
							if (AiUtils.HaveMemo(c0, 348)!=0 && AiUtils.GetMemoStateEx(c0, 348, 0) == 17 && AiUtils.OwnItemCount(c0, 4294) > 0)
							{
					...
					if (!IsNullCreature(c2) && DistFromMe(c2) <= 1500)
					{
						if (GetCurrentTick() - c2.quest_last_reward_time > 1)
						{
							...
							i0 = i0 + AiUtils.Rand(100) + 100;
							SetMemoStateEx(c2, 348, 1, i0);
							if (AiUtils.GetMemoStateEx(c2, 348, 1) + i0 > 750)
							{
								GiveItem1(c2, 4295, 1);
								DeleteItem1(c2, 4294, 1);
								SoundEffect(c2, "ItemSound.quest_middle");
								SetMemoStateEx(c2, 348, 1, 0);
							}
						}
					}
				}
