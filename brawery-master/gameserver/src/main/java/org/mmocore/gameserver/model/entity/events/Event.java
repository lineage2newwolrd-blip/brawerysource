...
public abstract class Event extends LoggerObject {
    ...
    protected Instant _startTime, _pausedTime;
    ...
    public synchronized void registerActions(final boolean ignoreStartTime) {
        if (!ignoreStartTime) {
            ...
            if (_startTime == null || _startTime.equals(Instant.EPOCH))
                return;
        } else {
            //correct -3 second
            _startTime = Instant.now().plusSeconds(180);
        }
        ...
        for (final int key : onTimeActions.keySet()) {
            final Instant time = _startTime.plusSeconds(key);
    ...
    public synchronized void resumeActions()
    {
        if (_startTime == null || _startTime.equals(Instant.EPOCH))
            return;
        if (_pausedTime == null || _pausedTime.equals(Instant.EPOCH))
            return;

        final Instant now = Instant.now();
        _startTime = _startTime.plusMillis(Duration.between(now, _pausedTime).toMillis());//start + pause delay
        _pausedTime = null;
