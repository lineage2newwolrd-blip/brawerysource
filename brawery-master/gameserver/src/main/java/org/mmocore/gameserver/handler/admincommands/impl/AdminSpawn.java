    ...
    public boolean useAdminCommand(final Enum<?> comm, final String[] wordList, final String fullString, final Player activeChar) {
        ...
        switch (command) {
            ...
            case admin_spawn_tasks:
                try {
                    try {
                        ...
                        final FileWriter writer = new FileWriter(f, true);
                        writer.write(SpawnTaskManager.getInstance().toString());
                        writer.close();
                    } catch (Exception e) {
            ...
            case admin_mob_locs:
                ...
                try {
                    final int npcId = Integer.parseInt(wordList[1]);
                    List<NpcInstance> mobs = GameObjectsStorage.getAllByNpcId(npcId, false);
                    new File("dumps").mkdir();
                    final File f = new File("dumps/mobdump.txt");
                    if (!f.exists()) {
                        f.createNewFile();
                    }
                    try (FileWriter writer = new FileWriter(f, true)) {
                        mobs.stream()
                                .map(mob -> mob.toString() + " alive:" + mob.isAlive() + " loc:" + mob.getLoc().toXYZString())
                                .forEach(s -> {
                                            try {
                                                writer.write(s);
                                            } catch (Exception e) {
