...
public class HardSpawner extends Spawner {
    ...
    protected NpcInstance initNpc(final NpcInstance mob, final boolean spawn, final MultiValueSet<String> set) {
        ...
        final SpawnRange range = template.getSpawnRange(template.isRndPos()? getRndRangeId() : getNextRangeId());
        mob.setSpawnRange(range);
        if(range instanceof Territory)
            mob.setTerritory((Territory) range);
    ...
    private int getRndRangeId() {
        final int newId = Rnd.get(template.getSpawnRangeSize());
        pointIndex.set(newId);
        return newId;
    }
