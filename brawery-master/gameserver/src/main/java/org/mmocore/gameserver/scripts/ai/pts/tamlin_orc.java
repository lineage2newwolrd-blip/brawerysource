...
public class tamlin_orc extends warrior_pa_berserker
{
	public tamlin_orc(final NpcInstance actor){super(actor);}
			...
			switch (code_info.getCode())
			{
				...
				case 2:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						else if (AiUtils.HaveMemo(c1, 233)!=0 && AiUtils.OwnItemCount(c1, 2880) == 1 && AiUtils.OwnItemCount(c1, 2881) >= 13)
						{
							SetFlagJournal(c1, 233, 4);
							ShowQuestMark(c1, 233);
						}
					}
					break;
				}
				case 3:
				{
					...
					if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
					{
						...
						if (AiUtils.HaveMemo(c1, 222)!=0 && AiUtils.OwnItemCount(c1, 2767) > 0 && AiUtils.OwnItemCount(c1, 2776) < 10 && AiUtils.GetMemoState(c1, 222) == 1)
						{
							i0 = AiUtils.GetMemoStateEx(c1, 222, 1);
							SetMemoStateEx(c1, 222, 1, i0 + 1);
							c1.quest_last_reward_time = GetCurrentTick();
							GiveItem1(c1, 2776, 1);
							if (AiUtils.OwnItemCount(c1, 2776) >= 10)
							{
								SoundEffect(c1, "ItemSound.quest_middle");
								if (i0 >= 9)
								{
