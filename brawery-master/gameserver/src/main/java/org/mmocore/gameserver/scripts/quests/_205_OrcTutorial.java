...
public class _205_OrcTutorial extends Quest {
    // npc
    private static final int guardian_vullkus = 30573; // Grand Master
    private static final int shela_priestess_of_fire = 30575; // Newbie Helper
    // mobs
    ...
    public _205_OrcTutorial() {
        super(false);
        addTalkId(guardian_vullkus, shela_priestess_of_fire);
        addFirstTalkId(guardian_vullkus, shela_priestess_of_fire);
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("timer_newbie_helper")) {
            ...
            if (GetMemoStateEx == 3)
                st.playTutorialVoice("tutorial_voice_010e");
            return null;
            ...
            return null;
        } else if (event.equalsIgnoreCase("reply_31") && st.ownItemCount(voucher_of_flame) > 0) {
            htmltext = "guardian_vullkus002.htm";
            st.takeItems(voucher_of_flame, 1);
            st.startQuestTimer("timer_grand_master", 60000, npc);
            if (GetMemoStateEx <= 3)
                qs.setMemoState("tutorial_quest_ex", String.valueOf(4), true);
            if (talker_occupation != orc_mage && st.ownItemCount(soulshot_none_for_rookie) <= 200) {
                st.giveItems(soulshot_none_for_rookie, 200);
                st.playTutorialVoice("tutorial_voice_026");
                st.addExpAndSp(0, 50);
            } else if (talker_occupation == orc_mage && st.ownItemCount(spiritshot_none_for_rookie) <= 100) {
                st.giveItems(spiritshot_none_for_rookie, 100);
                st.playTutorialVoice("tutorial_voice_027");
                st.addExpAndSp(0, 50);
            }
        } else if (event.equalsIgnoreCase("reply_41")) {
            htmltext = "guardian_vullkus005.htm";
            ...
            htmltext = "guardian_vullkus006.htm";
            ThreadPoolManager.getInstance().schedule(new RadarTask(-45032, -113598, -192, st.getPlayer()), 200L);
        }
        return htmltext;
    }
    ...
    public String onFirstTalk(NpcInstance npc, Player player) {
        ...
        // [orc_tutorial] 205 Orc's Tutorial
        ...
        switch (npcId) {
            case shela_priestess_of_fire:
                ...
                if (GetMemoStateEx < 0) {
                    ...
                    if (talker_occupation == orc_mage)
                        htmltext = "shela_priestess_of_fire002.htm";
                    else
                    ...
                    st.giveItems(voucher_of_flame, 1);
                    st.startQuestTimer("timer_newbie_helper", 30000, npc);
                    qs.setMemoState("tutorial_quest", String.valueOf(i0 | 4), true);
                    if (talker_occupation != orc_mage) {
                        if (st.ownItemCount(soulshot_none_for_rookie) <= 0)
                            st.giveItems(soulshot_none_for_rookie, 200);
                        st.playTutorialVoice("tutorial_voice_026");
                        htmltext = "shela_priestess_of_fire003f.htm";
                    }
                    if (talker_occupation == orc_mage) {
                        if (st.ownItemCount(spiritshot_none_for_rookie) <= 0)
                            st.giveItems(spiritshot_none_for_rookie, 100);
                        st.playTutorialVoice("tutorial_voice_027");
                        htmltext = "shela_priestess_of_fire003m.htm"; // FIXME: корректный диалог нужен.
                    }
                } else if (GetMemoStateEx == 3)
                    htmltext = "shela_priestess_of_fire004.htm";
                else if (GetMemoStateEx > 3)
                    htmltext = "carl005.htm";
                break;
            case guardian_vullkus:
                if (st.ownItemCount(voucher_of_flame) > 0)
                    htmltext = "guardian_vullkus001.htm";
                else if (st.ownItemCount(voucher_of_flame) == 0 && GetMemoStateEx > 3)
                    htmltext = "guardian_vullkus004.htm";
                else if (st.ownItemCount(voucher_of_flame) == 0 && GetMemoStateEx <= 3)
                    htmltext = "guardian_vullkus003.htm";
                break;
        }
        return htmltext;
    }
