...
public class warrior_physicalspecial_lab_random extends warrior_casting_enchant_lab_clan1
{
	...
	public warrior_physicalspecial_lab_random(final NpcInstance actor){super(actor);}
	...
	protected void onEvtAttacked(Creature attacker, int skill_name_id, int skill_id, int damage)
	{
		...
		if (actor.currentHp < actor.getMaxHp() / 3 && AiUtils.Rand(100) < 33)
		{
		...
		else if (actor.currentHp < actor.getMaxHp() / 0.600000f && AiUtils.Rand(100) < 25)
		{
			if (Skill_GetConsumeMP(SelfRangePhysicalSpecial) < actor._currentMp && Skill_GetConsumeHP(SelfRangePhysicalSpecial) < actor.currentHp && Skill_InReuseDelay(SelfRangePhysicalSpecial) == 0)
			{
				AddUseSkillDesire(attacker, SelfRangePhysicalSpecial, DesireType.ATTACK, DesireMove.MOVE_TO_TARGET, 1000000);
		...
		else if (actor.currentHp < actor.getMaxHp() / 0.900000f && AiUtils.Rand(100) < 20)
		{
			if (Skill_GetConsumeMP(SelfRangePhysicalSpecial) < actor._currentMp && Skill_GetConsumeHP(SelfRangePhysicalSpecial) < actor.currentHp && Skill_InReuseDelay(SelfRangePhysicalSpecial) == 0)
			{
				AddUseSkillDesire(attacker, SelfRangePhysicalSpecial, DesireType.ATTACK, DesireMove.MOVE_TO_TARGET, 1000000);
