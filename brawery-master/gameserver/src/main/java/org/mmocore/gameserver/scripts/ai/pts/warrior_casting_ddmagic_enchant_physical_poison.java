...
public class warrior_casting_ddmagic_enchant_physical_poison extends warrior_casting_enchant_physical_poison
{
	...
	public warrior_casting_ddmagic_enchant_physical_poison(final NpcInstance actor){super(actor);}
	...
	protected void onEvtAttacked(Creature attacker, int skill_name_id, int skill_id, int damage)
	{
		...
		if (attacker.isPlayer() || IsInCategory(Category.summon_npc_group, attacker.getClassId()))
		{
			...
			if (DistFromMe(attacker) > 100)
			{
				...
				{
					if ((top_desire_target == attacker && i0 < 33))
					{
	...
	protected void onEvtClanAttacked(Creature attacker, Creature victim, int damage, int skill_name_id, int skill_id)
	{
		...
		{
			i0 = AiUtils.Rand(100);
			if (DistFromMe(attacker) > 100 && i0 < 33)
			{
				if (Skill_GetConsumeMP(DDMagic) < actor._currentMp && Skill_GetConsumeHP(DDMagic) < actor.currentHp && Skill_InReuseDelay(DDMagic) == 0)
				{
					AddUseSkillDesire(attacker, DDMagic, DesireType.ATTACK, DesireMove.MOVE_TO_TARGET, 1000000);
