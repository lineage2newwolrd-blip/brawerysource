...
import org.jts.dataparser.data.holder.npcdata.NpcData;
...
public final class NpcTemplate extends CharTemplate {
    ...
    public NpcTemplate(final StatsSet set) {
        ...
        NpcData data = NpcDataHolder.getInstance().getNpcInfo(npcId);
        name = set.getString("name");
        title = set.getString("title");
        // sex = set.getString("sex");
        level = data!=null? data.level : set.getInteger("level");
        rewardExp = data!=null? (long)(data.acquire_exp_rate * level * level): set.getLong("rewardExp");
        rewardSp = data!=null? data.acquire_sp: set.getInteger("rewardSp");
        rewardRp = (int) set.getDouble("rewardRp");
        event_flag = data!=null ? data.event_flag.mask : 0;
        ...
        if(data==null || !setPTSAI(data.npc_ai.ai_name)) {
            setAI(set.getString("ai_type", null));
            aggroRange = set.getInteger("aggroRange");
        } else {
            aggroRange = data.agro_range;
        }
    }

    @SuppressWarnings("unchecked")
    private boolean setPTSAI(final String ai) {//TO DELETE
        Class<CharacterAI> classAI;
        try {
            classAI = (Class<CharacterAI>) Class.forName("org.mmocore.gameserver.scripts.ai.test." + ai);
        } catch (ClassNotFoundException e) {
            classAI = null;
        }
        if(classAI==null)
            try {
                classAI = (Class<CharacterAI>) Class.forName("org.mmocore.gameserver.scripts.ai.pts." + ai);
            } catch (ClassNotFoundException e) {
                classAI = null;
            }
    ...
    public CharacterAI getNewAI(final NpcInstance npc) {
        try {
            CharacterAI newAI = aiConstructor.newInstance(npc);

            if(newAI instanceof NpcAI) {
                for (final Field field : aiClass.getFields()) {
                    if (field.isAnnotationPresent(SkillInfo.class)) {
                        final SkillInfo skill = field.getAnnotation(SkillInfo.class);
                        field.setAccessible(true);
                        try {
                            field.set(newAI, skill.id() << 16 | skill.level());
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
                }
                NpcData npcdata = NpcDataHolder.getInstance().getNpcInfo(npc.getNpcId());
                if (npcdata != null) {
                    Map<String, Object> ai_params = npcdata.npc_ai.params;
                    for (final String param : ai_params.keySet()) {
                        try {
                            setParam(aiClass, newAI, param, ai_params.get(param));
                        } catch (NoSuchFieldException e) {
                            LOGGER.error("No such AI param: " + param + " for AI: " + npcdata.npc_ai.ai_name);
                        } catch (IllegalArgumentException e) {
                            LOGGER.error("Cannot cast AI param: " + param + " value: " + ai_params.get(param) +  " for AI: " + npcdata.npc_ai.ai_name);
                        } catch (Exception e) {
                            LOGGER.error("Access error, AI param: " + param + " for AI: " + npcdata.npc_ai.ai_name);
                        }
                    }
                }
                MultiValueSet<String> spawn_set = npc.getSpawnParameters();
                if(!spawn_set.isEmpty())
                    for(String param: spawn_set.keySet()){
                        try {
                            setParam(aiClass, newAI, param, spawn_set.get(param));
                        } catch (NoSuchFieldException e) {
                            LOGGER.error("No such AI param: " + param + " for Npc: " + npc.getName());
                        } catch (IllegalArgumentException e) {
                            LOGGER.error("Cannot cast AI param: " + param + " value: " + spawn_set.get(param) +  " for Npc: " + npc.getName());
                        } catch (Exception e) {
                            LOGGER.error("Access error, AI param: " + param + " for Npc: " + npc.getName());
                        }
                    }
            }
    ...
    public <V> void setParam(Class clz, CharacterAI ai_instance, String param, V value) throws Exception{
        //Class clz = this.getClass();
        //if(ai_instance.toString().startsWith("vuku_orc"))
        //    LOGGER.info("Setting param: " + param + " value: " + value.toString() + " isString: "  + String.class.isInstance(value));
        ((NpcAI) ai_instance).onShiftMap.put(param, value.toString());
        Field field = clz.getField(param);
        field.setAccessible(true);
        if(String.class.isInstance(value)) {
            Class type = field.getType();
            if (type.isAssignableFrom(Integer.TYPE))
                field.set(ai_instance, Integer.parseInt((String) value));
            else if (type.isAssignableFrom(Float.TYPE))
                field.set(ai_instance, Float.parseFloat((String) value));
            else if (type.isAssignableFrom(Long.TYPE))
                field.set(ai_instance, Long.parseLong((String) value));
            else
                field.set(ai_instance, value);
            return;
        }
        field.set(ai_instance, value);
    }
    ...
    public boolean isPTSAI(){
        return aiClass.isAssignableFrom(NpcAI.class);
    }
}
