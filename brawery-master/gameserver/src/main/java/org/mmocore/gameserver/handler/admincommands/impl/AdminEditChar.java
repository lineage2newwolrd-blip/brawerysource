...
import static org.jts.dataparser.data.holder.setting.common.PlayerSex.FEMALE;
import static org.jts.dataparser.data.holder.setting.common.PlayerSex.MALE;
    ...
    private Set<PlayerClass> getAvailableSubClasses(final Player player) {
        ...
        for (final PlayerClass availSub : availSubs) {
            ...
            for (final SubClass subClass : player.getPlayerClassComponent().getSubClasses().values()) {
                ...
                final ClassId parent = ClassId.VALUES[availSub.ordinal()].getParent(player.getSex().ordinal());
                ...
                final ClassId subParent = ClassId.VALUES[subClass.getClassId()].getParent(player.getSex().ordinal());
            ...
            if (availSub.isOfRace(PlayerRace.kamael)) {
                ...
                if (currClass == PlayerClass.Berserker || currClass == PlayerClass.Doombringer || currClass == PlayerClass.Arbalester || currClass == PlayerClass.Trickster) {
                    if (player.getSex() == FEMALE && availSub == PlayerClass.MaleSoulbreaker || player.getSex() == MALE && availSub == PlayerClass.FemaleSoulbreaker) {
                        availSubs.remove(availSub);
                    }
                }
