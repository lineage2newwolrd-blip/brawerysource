            ...
            case STARTED:
                if (npcId == kintaijin) {
                    if (GetMemoState == 1 && st.ownItemCount(q240_claw_of_stakato) == 0)
                        ...
                        st.giveReward(ADENA_ID, 147200, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == n_spike_stakato) {
            if (GetMemoState == 1) {
                int i0 = Rnd.get(1000);
                if (i0 < 804)
                    if (st.ownItemCount(q240_claw_of_stakato) +1*st.getRateQuestsDrop() >= 25) {
                        st.giveItems(q240_claw_of_stakato, 1, true);
                        st.setCond(2);
            ...
            if (GetMemoState == 1) {
                ...
                if (i0 < 170) {
                    if (st.ownItemCount(q240_claw_of_stakato) +1*st.getRateQuestsDrop() >= 25) {
                        ...
                        st.setMemoState("cannot_believe_anyone", String.valueOf(2), true);
                        st.soundEffect(SOUND_MIDDLE);
                    } else if (st.ownItemCount(q240_claw_of_stakato) +2*st.getRateQuestsDrop() >= 25) {
                        st.giveItems(q240_claw_of_stakato, 2, true);
                        st.setCond(2);
                        st.setMemoState("cannot_believe_anyone", String.valueOf(2), true);
                        st.soundEffect(SOUND_MIDDLE);
                    } else {
                ...
                } else if (st.ownItemCount(q240_claw_of_stakato) +1*st.getRateQuestsDrop() >= 25) {
                    st.giveItems(q240_claw_of_stakato, 1, true);
                    st.setCond(2);
                    st.setMemoState("cannot_believe_anyone", String.valueOf(2), true);
                    st.soundEffect(SOUND_MIDDLE);
