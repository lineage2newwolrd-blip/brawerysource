        ...
        if (player.getActiveReflection().getInstancedZoneId() == INSTANCE_ID) {
            ...
            if (quest != null && quest.isCompleted()) {
                if (getNpcId() == NPC_ID_MOTHER_NORNIL) {
                    showChatWindow(player, "quests/_236_SeedsOfChaos/mother_nornil_q0236002.htm");
                } else if (getNpcId() == NPC_ID_RODEN) {
                    showChatWindow(player, "quests/_236_SeedsOfChaos/roden_q0236002.htm");
                }
            } else {
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        if (command.equalsIgnoreCase("request_exitInstance")) {
            if (player.getActiveReflection().getInstancedZoneId() == INSTANCE_ID) {
                ...
                if (quest != null && quest.isCompleted()) {
                    reflection.collapse();
                    showChatWindow(player, "quests/_236_SeedsOfChaos/mother_nornil_q0236003.htm");
                }
            }
        } else {
