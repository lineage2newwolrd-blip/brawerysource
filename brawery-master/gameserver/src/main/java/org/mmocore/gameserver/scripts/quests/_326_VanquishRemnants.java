...
public class _326_VanquishRemnants extends Quest {
                    ...
                    if (GetMemoState == 1) {
                        if (st.ownItemCount(red_cross_badge) + st.ownItemCount(blue_cross_badge) + st.ownItemCount(black_cross_badge) == 0)
                            ...
                            if (st.ownItemCount(red_cross_badge) + st.ownItemCount(blue_cross_badge) + st.ownItemCount(black_cross_badge) >= 10)
                                st.giveReward(ADENA_ID, 4320 + 46 * st.ownItemCount(red_cross_badge) + 52 * st.ownItemCount(blue_cross_badge) + 58 * st.ownItemCount(black_cross_badge), true);
                            else
                                st.giveReward(ADENA_ID, 46 * st.ownItemCount(red_cross_badge) + 52 * st.ownItemCount(blue_cross_badge) + 58 * st.ownItemCount(black_cross_badge), true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (npcId == ol_mahum_patrol || npcId == ol_mahum_guard) {
            if (GetMemoState == 1) {
                if (Rnd.get(100) < 61) {
                    st.giveItems(red_cross_badge, 1, true);
            ...
            if (GetMemoState == 1) {
                if (Rnd.get(100) < 57) {
                    st.giveItems(blue_cross_badge, 1, true);
                    st.soundEffect(SOUND_ITEMGET);
                }
            }
            ...
            if (GetMemoState == 1) {
                if (Rnd.get(100) < 59) {
                    st.giveItems(black_cross_badge, 1, true);
                    st.soundEffect(SOUND_ITEMGET);
                }
            }
