...
{
	public serpent_demon_kadesh(final NpcInstance actor){super(actor);}
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					...
					if (AiUtils.HaveMemo(c1, 224)!=0 && AiUtils.GetMemoState(c1, 224) == 13 && AiUtils.OwnItemCount(c1, 3300) == 0)
					{
						if (actor.last_blow_weapon_class_id == 3028)
						{
							GiveItem1(c1, 3300, 1);
							SetMemoState(c1, 224, 14);
							SoundEffect(c1, "Itemsound.quest_middle");
							SetFlagJournal(c1, 224, 14);
