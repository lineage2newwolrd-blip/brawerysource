...
public class blade_stakato_worker extends warrior_passive_casting_curse
{
	public blade_stakato_worker(final NpcInstance actor){super(actor);}
	...
	protected void onEvtDead(Creature attacker)
	{
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			...
			{
				SetCurrentQuestID(368);
				target = code_info.RandomSelectOne();
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (AiUtils.Rand(100) < 57)
					{
						c1 = GetLastAttacker();
						...
						i2 = 0;
						if (i1 == 0 && AiUtils.HaveMemo(c1, 368)!=0)
						{
							c2 = c1;
						}
						for (i0 = 0; i0 < i1; i0++)
