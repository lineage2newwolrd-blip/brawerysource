...
import org.mmocore.gameserver.Broadcaster;
...
import static org.mmocore.gameserver.configuration.config.GeodataConfig.CLIENT_Z_SHIFT;
...
public abstract class Creature extends GameObject {
	...
	private static final double[] POLE_VAMPIRIC_MOD = {1., 0.75, 0.56, 0.42, 0.31, 0.24, 0.18, 0.13};
	...
	public boolean isOnlyPathFind;
	public double _currentCp;
	public double currentHp = 1;
	public double _currentMp = 1;
	...
	protected int _lastHpBarUpdate = -1;
	...
	//Todo: Quest Reward pts scripts
	public int flag;
	public int quest_last_reward_time = 0;
	public int param1;
	public int i_quest0;
	public int last_blow_weapon_class_id = 0;
	...
	public final void abortCast(boolean force, boolean message) {
		if (isCastingNow() && (force || canAbortCast())) {
			...
			clearCastVars();
			ThreadPoolManager.getInstance().execute(new NotifyAITask(this, CtrlEvent.EVT_FINISH_CASTING, castingSkill, getTarget(), false));
	...
	private double absorbAndReflect(Creature target, SkillEntry skillEntry, double damage) {
		...
		if (isPlayer() && bow)
			bow = !((Player) this).isInMeleeTransform();
	...
	public void altOnMagicUseTimer(Creature aimingTarget, SkillEntry skillEntry) {
		...
		int magicId = getSkillDisplayId(skill);
	...
	public void altUseSkill(Creature aimingTarget, List<Creature> targets, SkillEntry skillEntry) {
		if (isDead()) {
			return;
		}
		...
		if (!skill.checkPreConditions(skillEntry, this, aimingTarget)) {
			return;
		}

		getListeners().onMagicUse(skillEntry, aimingTarget, true);
		...
		if (!skill.isToggle()) {
			broadcastPacket(new MagicSkillUse(this, aimingTarget, getSkillDisplayId(skill), level, skill.getHitTime(), reuseDelay));
		}
	...
	public void altUseSkill(SkillEntry skillEntry, Creature target, final boolean reuse) {
		...
		long reuseDelay = Formulas.calcSkillReuseDelay(this, skillEntry);
		if (!skill.isToggle()) {
			broadcastPacket(new MagicSkillUse(this, target, getSkillDisplayId(skill), level, skill.getHitTime(), reuseDelay));
		}
		// Не показывать сообщение для хербов и кубиков
		if (!skill.isHideUseMessage()) {
			if (skill.getSkillType() == SkillType.PET_SUMMON) {
	...
	public void broadcastPacketToOthers(IBroadcastPacket... packets) {
		Broadcaster.broadcast(this, packets);
	}
	...
	public void useTriggerSkill(GameObject target, List<Creature> targets, TriggerInfo trigger, SkillEntry owner, double damage) {
		...
		if (Rnd.chance(trigger.getChance()) && trigger.checkCondition(this, realTarget, aimTarget, owner, damage) && skillEntry.checkCondition(this, aimTarget, false, true, true)) {
			...
			if (displayId == 0) {
				displayId = getSkillDisplayId(skill);
			}
			if (displayLevel == 0) {
	...
	public void doAttack(Creature target) {
		...
		if (weaponItem != null) {
			if (isPlayer() && weaponItem.getAttackReuseDelay() > 0) {
				int reuse = Formulas.calcBowReuseTime((Player) this, target, weaponItem);
				if (reuse > 0) {
					sendPacket(new SetupGauge(this, SetupGauge.RED, reuse));
					_attackReuseEndTime = reuse + System.currentTimeMillis() - 75;
					if (reuse > sAtk) {
						ThreadPoolManager.getInstance().schedule(new NotifyAITask(this, CtrlEvent.EVT_HIT_END, null, null), reuse);
					}
				}
			}
	...
	private void doAttackHitByPole(Attack attack, Creature target, int sAtk) {
		int angle = (int) calcStat(Stats.POLE_ATTACK_ANGLE, 120, target, null);
		...
		if (!isInZonePeace())// Гварды с пикой, будут атаковать только одиночные цели в городе
		{
			for (Creature t : getAroundCharacters(range, 200)) {
				if (_poleAttackCount <= attackcountmax) {
					if (t == target || t.isDead() || !PositionUtils.isInFrontOf(this, t, angle)) {
						continue;
					}
	...
	public void doCast(SkillEntry skillEntry, Creature target, boolean forceUse, boolean regular) {
		...
		if (skill.isMagic() && !skill.isSkillTimePermanent()
				&& (!isPlayer() || getPlayer().canChargeShot())
				&& getChargedSpiritShot() > 0
		...
		//invalidate client position for melee skills
		if (isPlayer() && skill.getCastRange() <= 40) {
			ValidatePosition.correctPosition(getPlayer(), ValidatePosition.CorrectType.values()[GeodataConfig.correctType]);
		}
		broadcastPacket(new MagicSkillUse(this, target, getSkillDisplayId(skill), level, skillTime, reuseDelay));
		...
		if (skill.getTargetType() == SkillTargetType.TARGET_HOLY) {
			ThreadPoolManager.getInstance().execute(new NotifyAITask(target, CtrlEvent.EVT_AGGRESSION, this, 1));
		}
		...
		int delay = skillTime - skillInterruptTime;
		...
		delay -= 250;
		if (delay > 0) {
			if (skill.getCastRange() < 32767 && skill.getSkillType() != SkillType.TAKECASTLE && skill.getSkillType() != SkillType.TAKEFORTRESS)
				_skillGeoCheckTask = ThreadPoolManager.getInstance().schedule(new MagicGeoCheckTask(this), delay);// Create a task MagicUseTask with Medium priority to launch the MagicSkill at the end of the casting time
			_castInterruptTime = System.currentTimeMillis() + delay;
		} else {
	...
	public Location getFlyLocation(GameObject target, SkillEntry skillEntry) {
		...
		if (target != null && target != this) {
			...
			if (skill.isFlyToBack()) {
				loc = new Location(target.getX() + (int) (Math.sin(radian) * 30), target.getY() - (int) (Math.cos(radian) * 30), target.getZ());
			} else {
				loc = new Location(target.getX() - (int) (Math.sin(radian) * 30), target.getY() + (int) (Math.cos(radian) * 30), target.getZ());
			}
			...
			if (isFlying()) {
				...
				if (!GeoEngine.canMoveToCoord(getX(), getY(), getZ(), loc.x, loc.y, loc.z, getGeoIndex())) {
					loc = Location.findPointToStay(target, 30); // Если не получается встать рядом с объектом, пробуем встать прямо в него
					if (!GeoEngine.canMoveToCoord(getX(), getY(), getZ(), loc.x, loc.y, loc.z, getGeoIndex())) {
		...
		if (isFlying())
			...
			return GeoEngine.moveCheck(getX(), getY(), getZ(), getX() + x1, getY() + y1, true, getGeoIndex());
	}
	...
	public final int getSkillDisplayId(Skill skill) {
		return isNpc() ? skill.getId() : skill.getDisplayId();
	}
	...
	public boolean isSkillDisabled(int pts_skill_id) {
		TimeStamp sts = skillReuses.get(pts_skill_id);
		if (sts == null) {
			return false;
		}
		if (sts.hasNotPassed()) {
		...
		skillReuses.remove(pts_skill_id);
		return false;
	}
	...
	public Location getIntersectionPoint(Creature target) {
		if (PositionUtils.isInFrontOf(this, target, 90)) {
	...
	private boolean buildPathTo(int x, int y, int z, int offset, boolean pathFind, boolean keyboard) {
		return buildPathTo(x, y, z, offset, null, false, pathFind, keyboard);
	}

	private boolean buildPathTo(int x, int y, int z, int offset, Creature follow, boolean forestalling, boolean pathFind, boolean keyboard) {
		...
		if (isFlying() || isInWater()) {
			...
			if (isFlying()) {
				...
				// считаем точку встречи с водной поверхностью
				nextloc = GeoEngine.moveInWaterCheck(getX(), getY(), getZ(), dest.x, dest.y, dest.z, waterZ, geoIndex);
				if (nextloc == null)
					return false;

				int waterBottomZ = getWaterBottomZ();
				if (nextloc.z < waterBottomZ) // ниже дна не ныряем
					return false;

				_targetRecorder.clear();
				// считаем путь до водной поверхности
				List<Location> moveList = GeoMove.constructMoveList(getLoc(), nextloc);
				if (moveList.size() > 2)
					_targetRecorder.add(moveList);
				else
					nextloc = getLoc();
				...
				// плывем по водной поверхности, если необходимо
				if (dz > 0) {
					waterloc = GeoEngine.MoveWaterCheck(nextloc.x, nextloc.y, waterZ, dest.x, dest.y, geoIndex);
					if (waterloc == null)
						return false;
					moveList = GeoMove.constructMoveList(nextloc, waterloc);
					if (moveList.size() > 2)
						_targetRecorder.add(moveList);
					else
						nextloc = waterloc;
				...
				if (earthZ > waterloc.z || Math.abs(dest.z - earthZ) < 80) {
					moveList = GeoEngine.MoveList(waterloc.x, waterloc.y, waterloc.z, dest.x, dest.y, geoIndex, false);
					if (moveList != null && moveList.size() > 1) {
						_targetRecorder.add(moveList);
					}
				}
		...
		if (isPlayer()) {
			Zone waterZone = World.getWater(dest, getReflection());
			// если есть вода, мы выше нее и зайдем в воду
			if (waterZone != null && waterZone.getTerritory().getZmax() < getZ()) {
				// сначала считаем путь до поверхности воды
				List<Location> moveList = GeoEngine.MoveListForWater(getX(), getY(), getZ(), dest.x, dest.y, waterZone.getTerritory().getZmax(), geoIndex);
				if (moveList != null) {
					_targetRecorder.clear();
					Location nextLoc;
					if (moveList.size() > 2) {
						_targetRecorder.add(moveList);
						nextLoc = moveList.get(moveList.size() - 1).clone().geo2world();
					} else {
						nextLoc = getLoc();
					}
					// потом добавляем путь в воде до конца
					Location waterLoc = GeoEngine.moveInWaterCheck(nextLoc.x, nextLoc.y, nextLoc.z, dest.x, dest.y, dest.z, waterZone.getTerritory().getZmax(), geoIndex);
					if (waterLoc == null)
						return true;
			...
			if (keyboard) {
				List<Location> moveList = GeoEngine.MoveList(getX(), getY(), getZ(), dest.x, dest.y, geoIndex, false); // onlyFullPath = false - идем до куда можем
				if (moveList != null && !moveList.isEmpty()) // null - нет геодаты, empty - уже стоим на нужной клетке
				{
					_targetRecorder.clear();
					_targetRecorder.add(moveList);
					_endLoc = moveList.get(moveList.size() - 1).clone().setXY(dest);
					return true;
				}
				return false;
			}
		...
		if (moveList != null && (!isOnlyPathFind || !pathFind)) // null - до конца пути дойти нельзя
		{
			...
			Location lastMovePoint = moveList.get(moveList.size() - 1).clone();
			_targetRecorder.clear();
			_targetRecorder.add(moveList);
			if (isPlayer()) {
				_endLoc = offset > 0
						? lastMovePoint.shiftInBlock(dest, getLoc(), offset)
						: lastMovePoint.setXY(dest);
			}
			return true;
		}
...
//			validateLocation(0); // при каждой остановке сверяем координаты с клиентом
			...
			if (buildPathTo(loc.x, loc.y, loc.z, offset, target, forestalling, !isPlayer() && !target.isDoor(), false))
				movingDestTempPos.set(loc.x, loc.y, loc.z);
			else if (isNpc() && notFullPath(loc.x, loc.y, loc.z, offset)) {
				movingDestTempPos.set(loc.x, loc.y, loc.z);
				isMoving = true;
				moveNext(true);
				return false;
	...
	public void moveWithKeyboard(Location loc) {
		moveToLocation(loc.x, loc.y, loc.z, 0, false, true);
	}
	...
	public boolean moveToLocation(int x_dest, int y_dest, int z_dest, int offset, boolean pathfinding, boolean keyboard) {
		moveLock.lock();
		try {
			...
			if (isPlayer()) {
				getPlayer().shotsReady();
				getAI().changeIntention(AI_INTENTION_ACTIVE, null, null);
			}

			stopMove(false, false);

			if (buildPathTo(x_dest, y_dest, z_dest, offset, pathfinding, keyboard)) {
				movingDestTempPos.set(dst_geoloc);
			} else if (isNpc() && notFullPath(x_dest, y_dest, z_dest, offset)) {
				movingDestTempPos.set(dst_geoloc);
				isMoving = true;
				moveNext(true);
				return false;
	...
	private void moveNext(boolean firstMove) {
		...
		if (_targetRecorder.isEmpty()) {
			...
			if (destination != null)
				ThreadPoolManager.getInstance().execute(new NotifyAITask(this, ctrlEvent, destination.x, destination.y, destination.z));
			else
				ThreadPoolManager.getInstance().execute(new NotifyAITask(this, ctrlEvent));
			stopMove(false, isPlayer()); // рассылка StopMove для предотвращения "залипания" при преследовании
			return;
		}
		...
		destination = GeoEngine.isTheSameBlock(_endLoc, end) ? _endLoc : end;
		double distance = (isFlying() || isInWater()) ? begin.distance3D(destination) : begin.distance(destination); //клиент при передвижении не учитывает поверхность

		if (distance != 0) {
			if (isFollow && followTarget.get() != null)
				setHeading(PositionUtils.calculateHeadingFrom(this, followTarget.get()));
			else
		...
		_moveTask = ThreadPoolManager.getInstance().schedule(_moveTaskRunnable.setDist(distance, begin), getMoveTickInterval());
	}

	public int getMoveTickInterval() {
		return isPlayer() ? Math.min(12000 / Math.max(getMoveSpeed(), 1), GeodataConfig.coordSyncTime) : 150;
	}
	...
	public int getWaterZ() {
		...
		return waterZ > Integer.MIN_VALUE ? waterZ - CLIENT_Z_SHIFT : waterZ;
	}
	...
	public void onHitTimer(Creature target, AttackType attackType, int damage, boolean crit, boolean miss, boolean soulshot,
		...
		if (target.getEffectList().getEffectByIndexAndType(522, EffectType.Debuff) != null && Formulas.calcRTBreak(this, target, crit)) {
		...
		if (!miss && damage > 0) {
			...
			last_blow_weapon_class_id = getWeaponClass();
	...
	public void onMagicUseTimer(Creature aimingTarget, SkillEntry skillEntry, boolean forceUse) {
		...
		if (isPlayable() && getPlayer().isInFightClub())
			getPlayer().getFightClubEvent().onMagicUse(getPlayer(), aimingTarget, skill);
	...
	private void finishFly(boolean aborted) {
		...
		if (flyLoc != null) {
			...
			if (aborted && skill != null && skill.getTemplate().getFlyType() != FlyToLocation.FlyType.CHARGE)
				broadcastPacket(new FlyToLocation(this, getLoc(), FlyToLocation.FlyType.NONE));
			else
				setLoc(flyLoc);
			validateLocation(1);
		}
	...
	public synchronized void setCurrentHp(double newHp, boolean canRessurect, boolean sendInfo) {
	...
	public void setCurrentHpMp(double newHp, double newMp, boolean canRessurect) {
		double maxHp = getMaxHp();
		setCurrentHp(newHp, false, canRessurect);
		int maxMp = getMaxMp();
	...
	public void startAbnormalEffect(AbnormalEffect ae, Creature speller, SkillEntry se) {
	...
	public boolean isFlashed() {
		return _flashed > System.currentTimeMillis();
	}
	...
	public void setFlashed(long flashed) {
		_flashed = System.currentTimeMillis() + flashed;
	}
	...
	public boolean isActionsDisabled() {
		return isBlocked() || isAlikeDead() || isFlashed() || isStunned() || isSleeping() || isParalyzed() || isAttackingNow() || isCastingNow() || isFrozen();
	}
	...
	public boolean paralizeOnAttack(Creature attacker) {
		...
		if (isNpc()) {
			...
			} else if (isMinion() && (leader = this.getLeader()) != null && leader.isRaid())
				max_attacker_level = leader.getLevel() + ServerConfig.RAID_MAX_LEVEL_DIFF;
		}
	...
	public int getWeaponClass() {
		if (getActiveWeaponItem() != null) {
			return getActiveWeaponItem().getItemId();
		}
		return 0;
	}
	...
	public int getWeaponType() {
		if (getActiveWeaponItem() != null) {
			return getActiveWeaponItem().getItemType().type();
		}
		return 0;
	}
	...
	public class MoveNextTask extends RunnableImpl {
		...
		public MoveNextTask setDist(final double dist, Location startLoc) {
			allDist = dist;
			doneDist = 0.D;
			begin = startLoc;
			return this;
		...
		public void runImpl() throws Exception {
			...
			try {
				...
				doneDist += (now - _startMoveTime) * (double) _previousSpeed / 1000.D;
				...
				Location loc;
				double doneSize = (double) (moveList.size() - 1) * done;
				...
				loc = index == 0 ? begin.clone() : moveList.get(index).clone().geo2world();
				if (isPlayer()) {
					if (index >= moveList.size() - 2)
						loc.correctInBlock(destination, doneSize);
					else if (index < moveList.size() - 1)
						loc.correctInBlock(moveList.get(index + 1).clone().geo2world(), doneSize);
				}
				...
				if (isFollow) {
					...
					if (isInRangeZ(follow, _offset) && GeoEngine.canSeeTarget(Creature.this, follow, false)) {
						stopMove(true, isPlayer());
						ThreadPoolManager.getInstance().execute(new NotifyAITask(Creature.this, CtrlEvent.EVT_ARRIVED_TARGET));
						return;
					}
				}

				if (isFollow && now - _followTimestamp > (_forestalling ? 500 : 1000)
						&& !follow.isInRange(movingDestTempPos, Math.max(50, _offset))) {
					if (Math.abs(getZ() - loc.z) > 1000 && !isFlying()) {
						sendPacket(SystemMsg.CANNOT_SEE_TARGET);
						stopMove();
						return;
					}
					if (buildPathTo(follow.getX(), follow.getY(), follow.getZ(), _offset, follow, true, !isPlayer(), false)) {
				...
				//Последник тик считаем из оставшегося пути + учет нелинейного ускорения персонажа в начале пути для совпадения с клиентом
				int delay = getMoveTickInterval();
				if (isPlayer()) {
					int tail = (int) ((allDist - doneDist) * 1000. / speed);
					if (tail < delay)
						delay = tail + Math.min(speed / 5, GeodataConfig.coordSyncTime);
				}

				_moveTask = ThreadPoolManager.getInstance().schedule(this, delay);
