...
public class FlagToCaptureInstance extends NpcInstance
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2876965120803048665L;
	...
	public void onAction(Player player, boolean shift)
	{
		...
		if (player.isInFightClub())
		{
            if (player.getFightClubEvent() instanceof CaptureTheFlagEvent) {
                boolean shouldDissapear = ((CaptureTheFlagEvent) player.getFightClubEvent()).tryToTakeFlag(player, this);
                if (shouldDissapear)
                    deleteMe();
            }
		}
