...
public class _341_HuntingForWildBeasts extends Quest {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        if (_state == CREATED) {
            ...
            if (st.ownItemCount(BEAR_SKIN) >= 20) {
                ...
                st.giveReward(ADENA_ID, 3710, true);
    ...
    public String onKill(NpcInstance npc, QuestState qs) {
        ...
        if (qs.ownItemCount(BEAR_SKIN) < 20 && Rnd.chance(BEAR_SKIN_CHANCE)) {
            qs.giveItems(BEAR_SKIN, 1, true);
            if (qs.ownItemCount(BEAR_SKIN) >= 19) {
                qs.setCond(2);
