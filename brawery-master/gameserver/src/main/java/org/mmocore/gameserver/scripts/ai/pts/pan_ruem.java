...
{
	public pan_ruem(final NpcInstance actor){super(actor);}
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			if (code_info.getCode() == 1)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					...
					if (AiUtils.HaveMemo(c1, 335)!=0 && AiUtils.OwnItemCount(c1, 3756)!=0 && AiUtils.OwnItemCount(c1, 3798) < 40)
					{
						if (AiUtils.Rand(100) < 90)
						{
							if (GetCurrentTick() - c1.quest_last_reward_time > 1)
							{
								...
								GiveItem1(c1, 3798, 1);
								if (AiUtils.OwnItemCount(c1, 3798) >= 40)
								{
