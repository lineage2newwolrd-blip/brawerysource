            ...
            case STARTED:
                if (npcId == grandmaster_oltlin)
                    if (GetMemoState == 1 && st.ownItemCount(snake_scale) < 108)
                        ...
                        int i0 = Rnd.get(1000);
                        if (i0 < 125)
                            st.giveReward(rp_sealed_phoenixs_earing_i, 1, true);
                        else if (i0 < 250)
                            st.giveReward(rp_sealed_majestic_earing_i, 1, true);
                        else if (i0 < 375)
                            st.giveReward(rp_sealed_phoenixs_necklace_i, 1, true);
                        else if (i0 < 500)
                            st.giveReward(rp_sealed_majestic_necklace_i, 1, true);
                        else if (i0 < 625)
                            st.giveReward(rp_sealed_phoenixs_ring_i, 1, true);
                        else if (i0 < 750)
                            st.giveReward(rp_sealed_majestic_ring_i, 1, true);
                        else if (i0 < 875)
                            st.giveReward(rp_sealed_shield_of_nightmare_i, 1, true);
                        else
                            st.giveReward(rp_sealed_dark_crystal_shield_i, 1, true);
        ...
        if (GetMemoState == 1)
            if (npcId == trives) {
                if (st.ownItemCount(snake_scale) < 108 && Rnd.get(100) < 71) {
                    st.giveItems(snake_scale, 1, true);
                    st.soundEffect(SOUND_ITEMGET);
                    if (st.ownItemCount(snake_scale) >= 108) {
                        st.setCond(2);
