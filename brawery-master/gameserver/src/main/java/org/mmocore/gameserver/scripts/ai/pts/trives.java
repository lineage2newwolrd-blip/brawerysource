...
public class trives extends warrior_pa_casting_3skill_magical
{
	public trives(final NpcInstance actor){super(actor);}
		...
		while (!AiUtils.IsNull(code_info = always_list.Next()))
		{
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					...
					if (AiUtils.HaveMemo(c1, 358) == 1 && AiUtils.OwnItemCount(c1, 5868) < 108 && AiUtils.Rand(100) < 71)
					{
