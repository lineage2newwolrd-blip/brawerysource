...
{
	public argos_boss_17(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (target.isPlayer())
					{
						...
						{
							GiveItem1(target, 7408, 1);
							SetFlagJournal(target, 86, 16);
							ShowQuestMark(target, 86);
							SoundEffect(target, "ItemSound.quest_middle");
							DeleteItem1(target, 7501, AiUtils.OwnItemCount(target, 7501));
							Say(AiUtils.MakeFString(8654, "", "", "", "", ""));
						}
						else
						{
							Say(AiUtils.MakeFString(8655, "", "", "", "", ""));
						}
					}
				}
			}
	...
	protected void onEvtSpawn()
	{
		...
		if (c0 !=null)
		{
			...
			if (c0.isPlayer())
			{
				Say(AiUtils.MakeFString(8653, c0.getName(), "", "", "", ""));
			}
		}
		super.onEvtSpawn();
	}
