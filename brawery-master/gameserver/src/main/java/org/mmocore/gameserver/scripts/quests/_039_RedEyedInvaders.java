        ...
        if (npcId == guard_babenco) {
            ...
            if (event.equalsIgnoreCase("reply_1") && GetHTMLCookie == 2 - 1) {
                ...
                if (st.ownItemCount(q_mel_liz_perfume) >= 30 && st.ownItemCount(q_mel_liz_gem) >= 30) {
                    ...
                    st.giveReward(fp_babyduck_rod, 1, false);
                    st.giveReward(fishing_shot_none, 500, true);
                    st.giveReward(green_lure_high, 60, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (GetMemoState == 2 * 10 + 1) {
            if (npcId == male_lizardman || npcId == male_lizardman_scout) {
                int i4 = Rnd.get(1000);
                if (i4 < 500) {
                    if (st.ownItemCount(q_mel_liz_necklace_a) + 1*st.getRateQuestsDrop() >= 100) {
                        ...
                        st.giveItems(q_mel_liz_necklace_a, 1, true);
                ...
                int i4 = Rnd.get(1000);
                if (i4 < 500) {
                    if (st.ownItemCount(q_mel_liz_necklace_b) + 1*st.getRateQuestsDrop() >= 100) {
                        if (st.ownItemCount(q_mel_liz_necklace_b) < 100) {
                        ...
                        st.giveItems(q_mel_liz_necklace_b, 1, true);
                        st.soundEffect(SOUND_ITEMGET);
                    }
                }
            }
            ...
            if (npcId == giant_araneid) {
                int i4 = Rnd.get(1000);
                if (i4 < 500) {
                    if (st.ownItemCount(q_mel_liz_gem) + 1*st.getRateQuestsDrop() >= 30) {
                        ...
                        st.giveItems(q_mel_liz_gem, 1, true);
                        st.soundEffect(SOUND_ITEMGET);
                    }
                }
                ...
                int i4 = Rnd.get(1000);
                if (i4 < 500) {
                    if (st.ownItemCount(q_mel_liz_perfume) + 1*st.getRateQuestsDrop() >= 30) {
                        if (st.ownItemCount(q_mel_liz_perfume) < 30) {
                        ...
                        st.giveItems(q_mel_liz_perfume, 1, true);
                        st.soundEffect(SOUND_ITEMGET);
                    }
                }
            }
