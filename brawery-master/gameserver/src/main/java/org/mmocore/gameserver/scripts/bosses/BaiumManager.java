...
public class BaiumManager implements OnInitScriptListener, OnReloadScriptListener {
	...
	private final static int baium_boss = 29020;
    ...
    /*private synchronized static void checkAnnihilated() {
        if (isPlayersAnnihilated()) {
            ThreadPoolManager.getInstance().schedule(new Task(3), TIMER_IDLE_COMBAT);
        }
    }*/
	...
	public static void spawnBaium(NpcInstance npcBaium, Player awakeBy) {
		Dying = false;
		NpcInstance baium = NpcUtils.spawnSingle(baium_boss, new Location(116033, 17447, 10107, -36482));
		npcBaium.deleteMe();
		if (CustomBossSpawnConfig.activateCustomSpawn)
			StateBaium(pattern, CustomBossSpawnConfig.baiumRandomMinutes, State.ALIVE);
		else
			StateBaium(getRespawnInterval(), State.ALIVE);
		Log.add("Spawned Baium, awake by: " + awakeBy, "org/mmocore/gameserver/scripts/bosses");
		setLastAttackTime();
		ThreadPoolManager.getInstance().schedule(new StartBaiumTask(baium, awakeBy, 1), 2000);
	}
	...
	private void init() {
		_state = new EpicBossState(baium_boss);
		// Инитализируем зону
	...
	private static class OnDeathListenerImpl implements OnDeathListener {
		...
		public void onDeath(Creature self, Creature killer) {
			...
			if (self.isNpc() && self.getNpcId() == baium_boss) {
				onBaiumDie(self);
			}
		}
	}
	...
	public static class StartBaiumTask extends RunnableImpl {
		...
		private final NpcInstance _baium;
		private int _taskId;

		public StartBaiumTask(final NpcInstance baium, final Player player, final int taskId) {
			_baium = baium;
			_player = player;
			_taskId = taskId;
		}
		...
		public void runImpl() {
			switch (_taskId) {
				case 1:
					...
					ThreadPoolManager.getInstance().schedule(new StartBaiumTask(_baium, _player, 2), 11500);
					break;
				case 2:
				...
				case 3:
					...
					if (_player == null || _player.isDead() || !_player.isInRangeZ(_baium, 2000))
						_player = _baium.getAggroList().getMostHated().isPlayable() ? _baium.getAggroList().getMostHated().getPlayer() : null;
					...
					ThreadPoolManager.getInstance().schedule(new StartBaiumTask(_baium, _player, 4), 2000);
					break;
				case 4:
					if (_player != null) {
						_player.doDie(_baium);
					}
					_baium.broadcastPacket(new SocialAction(_baium.getObjectId(), 1));
					ThreadPoolManager.getInstance().schedule(new StartBaiumTask(_baium, _player, 5), 8000);
					break;
				// Спавн Ангелов
				case 5:
					_baium.unblock();
					...
					ThreadPoolManager.getInstance().schedule(new StartBaiumTask(_baium, _player, 6), 2000);
					ThreadPoolManager.getInstance().schedule(new Task(2), TIMER_IDLE_COMBAT);
					break;
				case 6:
