...
public abstract class Playable extends Creature {
    ...
    public void reduceCurrentHp(double damage, Creature attacker, SkillEntry skill, boolean awake, boolean standUp, boolean directHp,
        ...
        // 2901 = Ancient Herb - Immortal, работает только на мобов
        if(attacker.isMonster() && attacker.getNpcId() >= 22650 && attacker.getNpcId() <= 22659 && getEffectList().getEffectsBySkillId(2901) != null)
        {
            return;
        }
    ...
    public boolean isAttackable(Creature attacker, boolean force, boolean nextAttackCheck) {
        Player player = getPlayer();
        if (attacker == null || player == null || attacker == this || attacker.isAlikeDead()) {
            return false;
        }
            ...
            for (Creature target : targets) {
                if (target.isNpc()) {
                    if (skill.isOffensive()) {
                        ...
                        if (!skill.isAI()) {
                            int damage = skill.getEffectPoint() != 0 ? skill.getEffectPoint() : skill.isMagic() ? Math.max(1, (int) skill.getPower()* 6) : Math.max(1, (int) skill.getMagicLevel()* 8);
                            ThreadPoolManager.getInstance().execute(new NotifyAITask(target, CtrlEvent.EVT_ATTACKED, this, skillEntry, damage));
                        }
                    }

                    ThreadPoolManager.getInstance().execute(new NotifyAITask(target, CtrlEvent.EVT_SEE_SPELL, skillEntry, this));
                    ...
                    if (target.isPlayable() && target != getServitor() && !((this.isSummon() || this.isPet()) && target == player)) {
                        ...
                        if (aggro == 0) {
                            aggro = skill.isMagic()
                                    ? Math.max(1, (int) skill.getPower() * 6)
                                    : Math.max(1, skill.getMagicLevel() * 8);
                        } else if (aggro == 1) {
                            aggro = 1000;
                        }
                        ...
                        for (NpcInstance npc : npcs) {
                            ...
                            ThreadPoolManager.getInstance().execute(new NotifyAITask(npc, CtrlEvent.EVT_SEE_SPELL, skillEntry, this, target));
                            ...
                            if (GeoEngine.canSeeTarget(npc, target, false) && !npc.getAI().isPTSAI()) // Моб агрится только если видит цель, которую лечишь/бафаешь.
                            {
                                ThreadPoolManager.getInstance().execute(
                                        new NotifyAITask(npc,
                                                CtrlEvent.EVT_AGGRESSION,
                                                this,
                                                ai.damage == 0 ? aggro / 2 : aggro)
                                );
                            }
                        }
                    }
        ...
        if (!isTeleporting()) {
            ...
            if (isSalvation()|| isPlayer() && getPlayer().isInFightClub()) {
                ...
                if (isPlayer() && OtherConfig.RESPAWN_RESTORE_CP >= 0) {
                    setCurrentCp(getMaxCp() * OtherConfig.RESPAWN_RESTORE_CP/100.);
                }

                if(OtherConfig.RESPAWN_RESTORE_HP >= 0){
                    setCurrentHp(Math.max(1, getMaxHp() * OtherConfig.RESPAWN_RESTORE_HP/100.), true);
                }

                if (OtherConfig.RESPAWN_RESTORE_MP >= 0) {
                    setCurrentMp(getMaxMp() * OtherConfig.RESPAWN_RESTORE_MP/100.);
                }
            }
