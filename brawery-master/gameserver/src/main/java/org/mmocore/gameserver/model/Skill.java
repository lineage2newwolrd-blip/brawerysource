...
public abstract class Skill extends StatTemplate implements Cloneable {
    ...
    protected final boolean _isAbnormalInstant;
    ...
    private int[] _affectLimit = new int[]{0,0};
    ...
    protected Skill(final StatsSet set) {
        ...
        SkillData skillInfo = SkillDataHolder.getInstance().getSkillInfo(_id, set.getInteger("ptslevel", _level));
        if(skillInfo!=null && skillInfo.effect_point != 0)
            _effectPoint = Math.abs(skillInfo.effect_point);
        else
        ...
        final String affectLimit;
        if(skillInfo!=null && skillInfo.affect_limit!=null){
            affectLimit = skillInfo.affect_limit;
        }
        else{
            affectLimit = set.getString("affectLimit", null);
        }
        if(affectLimit!=null){
            st = new StringTokenizer(affectLimit, ";");
            for(int i=0; i<2 && st.hasMoreTokens(); i++){
                _affectLimit[i] = Integer.parseInt(st.nextToken());
            }
        }
    ...
    public boolean checkCondition(final SkillEntry skillEntry, final Creature activeChar, final Creature target, final boolean forceUse, final boolean dontMove, final boolean first) {
        ...
        if (player != null) {
            ...
            for(Event e : player.getEvents())
                if(e.notAllowedSkill(player, target, this)){
                    return false;
                }
    ...
    public SystemMsg checkTarget(final Creature activeChar, final Creature target, final Creature aimingTarget, final boolean forceUse, final boolean first) {
        ...
        if (!first && target != activeChar && (_targetType == SkillTargetType.TARGET_MULTIFACE || _targetType == SkillTargetType.TARGET_MULTIFACE_AURA || _targetType == SkillTargetType.TARGET_TUNNEL) && (_isBehind ? PositionUtils.isBehind(target, activeChar, 120) : !PositionUtils.isInFrontOf(activeChar, target, 60))) {
        ...
        if (player != null) {
            ...
            if (pcTarget != null) {
                ...
                if (isOffensive()) {
                    ...
                    if (((first && isAoE()) || isNotTargetAoE()) && getCastRange() < Integer.MAX_VALUE && !(GeoEngine.canSeeTarget(activeChar, target, activeChar.isFlying()) || !isNotTargetAoE() && GeoEngine.canSeeTarget(aimingTarget, target, activeChar.isFlying()))) {
                        return SystemMsg.CANNOT_SEE_TARGET;
                    }
                    ...
                    if(activeChar.isNpc() && player.getPvpFlag() != 0) // дебафающие купола дают эффект если кастер флагнут
                        return null;
                ...
                if (isTrigger()) // DS: для атакующих триггеров других проверок не нужно, все уже сделано в проверке атаки
                    return null;
                ...
                // DS: нет разницы между варами и обычными нефлагнутыми игроками
        ...
        if (isAoE() && isOffensive() && getCastRange() < Integer.MAX_VALUE && !(GeoEngine.canSeeTarget(activeChar, target, activeChar.isFlying()) || !isNotTargetAoE() && GeoEngine.canSeeTarget(aimingTarget, target, activeChar.isFlying()))) {
            return SystemMsg.CANNOT_SEE_TARGET;
        }
    ...
    public final Creature getAimingTarget(final Creature activeChar, final GameObject obj) {
        ...
        switch (_targetType) {
            ...
            case TARGET_ONE:
                return target != null && !(target == activeChar && isOffensive()) && (!_isUndeadOnly || target.isUndead()) ? target : null;
            case TARGET_PARTY_ONE:
                ...
                if (ptarget != null && player != null && player.getParty() != null && player.getParty().containsMember(ptarget) && !(target == activeChar && isOffensive()) && (!_isUndeadOnly || target.isUndead())) {
                    return target;
                }
                return null;
            case TARGET_AREA:
        ...
        switch (_targetType) {
            ...
            case TARGET_ALLY: {
                ...
                for (final Player target : World.getAroundPlayers(player, _skillRadius, 600)) {
                    ...
                    // Защита для эпик боссов
                    if (target.isInZone(ZoneType.epic) != player.isInZone(ZoneType.epic)) {
                        continue;
                    }
    ...
    private void addTargetsToList(final List<Creature> targets, final Creature aimingTarget, final Creature activeChar, final boolean forceUse) {
        int count = 0;
        int mobMaxCount = _affectLimit[1] == 0 ? 50 : Math.min(50,_affectLimit[0] + Rnd.get(0, _affectLimit[1]+1));
...
/*			if(_targetType == SkillTargetType.TARGET_AREA_AIM_CORPSE || _targetType == SkillTargetType.TARGET_AREA || _targetType == SkillTargetType.TARGET_MULTIFACE || _targetType == SkillTargetType.TARGET_TUNNEL) {
            ...
            targets.add(target);
            count++;
            if (isOffensive() && !activeChar.isRaid() && target.isNpc() && count > mobMaxCount) {
                break;
            }
        }
    }
    ...
    public final void getEffects(final SkillEntry skillEntry, final Creature effector, final Creature effected, final boolean calcChance,
        ...
        if (effected.isDoor() || (effected.isDead()&& isOffensive() && !isPreservedOnDeath())) {//cleanse
            return;
        }
            ...
            public void runImpl() {
                ...
                for (final EffectTemplate et : getEffectTemplates()) {
                    ...
                    for (Creature target : targets) {
                        if (target.isDead() && et.getEffectType()!= EffectType.DispelEffects && !isPreservedOnDeath()) {//cleanse
                            continue;
                        }
                        ...
                        if (e != null) {
                            ...
                            if (e.isOneTime()) {
                                ...
                                e.schedule();
                                if(effector!=effected && isOffensive())
    ...
    public final int getAOECastRange() {
        return _castRange > 40 && _skillRadius > _castRange ? Math.min(_castRange, _skillRadius) : Math.max(_castRange, _skillRadius);
    }
