    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("quest_accept")) {
            ...
            if (st.ownItemCount(q_proof_of_avenger) >= 100) {
                ...
                if (i1 < 200) {
                    st.giveReward(sealed_ring_of_aurakyria_gem, 5, true);
                    st.giveReward(ADENA_ID, 90000, true);
                    st.addExpAndSp(120000, 10000);
                } else if (i1 < 200 + 200) {
                    st.giveReward(sealed_sanddragons_earing_piece, 5, true);
                    st.giveReward(ADENA_ID, 80000, true);
                    st.addExpAndSp(120000, 10000);
                } else if (i1 < 200 + 200 + 100) {
                    st.giveReward(sealed_dragon_necklace_wire, 5, true);
                    st.giveReward(ADENA_ID, 40000, true);
                    st.addExpAndSp(120000, 10000);
                } else if (i1 < 200 + 200 + 100 + 500)
                    st.giveReward(ADENA_ID, 230000, true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (GetMemoState == 1 * 10 + 1)
            if (npcId == apostle_defender) {
                int i4 = Rnd.get(1000);
                if (i4 < 850)
                    if (st.ownItemCount(q_proof_of_avenger) + 1*st.getRateQuestsDrop() >= 100) {
                        if (st.ownItemCount(q_proof_of_avenger) < 100) {
                            st.setCond(2);
                        ...
                        st.giveItems(q_proof_of_avenger, 1, true);
