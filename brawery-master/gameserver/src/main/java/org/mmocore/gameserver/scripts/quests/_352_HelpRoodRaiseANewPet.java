                    ...
                    if (GetMemoState == 1)
                        if (st.ownItemCount(lienlik_egg1) < 1 && st.ownItemCount(lienlik_egg2) < 1)
                            ...
                            if (st.ownItemCount(lienlik_egg1) >= 10)
                                st.giveReward(ADENA_ID, st.ownItemCount(lienlik_egg1) * 34 + 4000, true);
                            else
                                st.giveReward(ADENA_ID, st.ownItemCount(lienlik_egg1) * 34 + 2000, true);
                            st.takeItems(lienlik_egg1, -1);
                            htmltext = "pet_manager_rood_q0352_07.htm";
                        } else if (st.ownItemCount(lienlik_egg2) >= 1) {
                            st.giveReward(ADENA_ID, 4000 + (st.ownItemCount(lienlik_egg1) * 34 + st.ownItemCount(lienlik_egg2) * 1025), true);
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (GetMemoState == 1)
            if (npcId == lienrik) {
                int i0 = Rnd.get(100);
                if (i0 < 46) {
                    st.giveItems(lienlik_egg1, 1, true);
                    st.soundEffect(SOUND_ITEMGET);
                } else if (i0 < 48) {
                    st.giveItems(lienlik_egg2, 1, true);
                    st.soundEffect(SOUND_ITEMGET);
                }
