...
public final class ItemFunctions {
    ...
    public static void addItem(final Playable playable, final int itemId, final long count, final boolean notify) {
        ...
        if(t == null) {
            Exception e = new NullPointerException("ItemTemplate is NULL");
            e.printStackTrace();
            return;
        }
        if (t.isStackable()) {
            player.getInventory().addItem(itemId, count);
        } else if(t.isTalisman() && player.isPlayer() && ((Player)player).isTalismanStack() && t.isShadowItem() && !player.getInventory().getItemsByItemId(itemId).isEmpty()){
            final ItemInstance product = player.getInventory().getItemsByItemId(itemId).get(0);
            product.setLifeTime(product.getLifeTime() + (int)count * t.getDurability());
