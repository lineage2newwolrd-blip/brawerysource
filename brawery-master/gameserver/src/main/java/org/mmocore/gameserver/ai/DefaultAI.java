...
public class DefaultAI extends CharacterAI implements Runnable {
    ...
    protected int MIN_PURSUE_RANGE;
    ...
    protected long  _runTimeout;
    /** Время телепорта при невозможности добежать */
    protected long _teleportTimeout = Long.MAX_VALUE;

    protected long _lastFactionNotifyTime = 0;
    protected long _minFactionNotifyInterval = 5000;
    protected boolean _isGlobal;

    protected long _castReady = Long.MAX_VALUE;
    protected int _teleportCounter;
    /**
    ...
    public DefaultAI(final NpcInstance actor) {
        ...
        MAX_PURSUE_RANGE = actor.getParameter("MaxPursueRange", actor.getReflection() != ReflectionManager.DEFAULT ? AiConfig.MAX_PURSUE_RANGE_DUNGEON : actor.isSevenSignsMonster()? AiConfig.MAX_PURSUE_SSQ_RANGE: actor.isUnderground() ? AiConfig.MAX_PURSUE_UNDERGROUND_RANGE : actor.isRaid() ? AiConfig.MAX_PURSUE_RANGE_RAID : AiConfig.MAX_PURSUE_RANGE);
        MIN_PURSUE_RANGE = actor.getParameter("MinPursueRange", AiConfig.MIN_PURSUE_RANGE);
    ...
    protected static SkillEntry selectTopSkillByDamage(final Creature actor, final Creature target, final double distance, final SkillEntry[] skills) {
        ...
        final RndNode<SkillEntry>[] nodes = Stream.of(skills).map(skill -> RndNode.of(
                skill,
                (int) Math.max((skill.getTemplate().getSimpleDamage(skill, actor, target) /skill.getTemplate().getHitTime() *1000)
                        * Math.min(skill.getTemplate().getAOECastRange() / distance, 1), 1))).toArray(RndNode[]::new);
    ...
    protected static SkillEntry selectTopSkillByDebuff(final Creature actor, final Creature target, final double distance, final SkillEntry[] skills) //FIXME
    {
        ...
        final RndNode[] nodes = Stream.of(skills)
    ...
    protected boolean isInAggroRange(Creature target) {
        ...
        if (ai != null && ai.hate > 0) {
            if (!target.isInRangeZ(actor.getLoc(), MIN_PURSUE_RANGE)) {
                return false;
            }
        } else if (!isAggressive() || !target.isInRangeZ(actor.getLoc(), actor.getAggroRange()) || !isInSpawnAggro(target)) {
            return false;
        }
    ...
    protected boolean isInSpawnAggro(Creature target) {
        return target.isInRangeZ(getActor().getSpawnedLoc(), 2000);
    }
    ...
    protected boolean randomAnimation() {
        ...
        if (actor.hasRandomAnimation() && !isActionsDisabled() && !actor.isMoving && !actor.isInCombat() && Rnd.chance(AiConfig.RND_ANIMATION_RATE)) {
    ...
    protected boolean findAggressionTarget() {
        ...
        if (now - _checkAggroTimestamp > AiConfig.AGGRO_CHECK_INTERVAL) {
            ...
            if (!actor.getAggroList().isEmpty()) {
                final List<Creature> hateList = actor.getAggroList().getHateList(MIN_PURSUE_RANGE);
    ...
    protected boolean thinkActive() {
        ...
        if (!actor.getParameter("noCheckAggression", false) && findAggressionTarget())
            return true;
        ...
        if (actor.isMinion()) {
            ...
            if (leader != null && !leader.isAlikeDead() &&!getActor().isIgnoreLeaderAction()) {
                if(leader.isRunning())
                    actor.setRunning();
                else
                    actor.setWalking();
                ...
                if (distance > 2000) {
                    ...
                    addTaskMove(leader.getMinionPosition(), true);
                    return true;
                }
            }
        }
    ...
    protected void onIntentionAttack(final Creature target) {
        ...
        setRunTimeout(getMaxRunTimeout() + System.currentTimeMillis());
        setGlobalAggro(0);
    ...
    protected boolean checkTarget(final Creature target, final int range) {
        ...
        final boolean hided = target.isPlayable() && (!canSeeInHide((Playable) target) || ((Playable) target).getNonAggroTime() > System.currentTimeMillis());
    ...
    public void setRunTimeout(long time)
    {
        _runTimeout = time;
    }
    ...
    protected void thinkAttack() {
        ...
        if(System.currentTimeMillis() > getRunTimeout())
            returnHome();
    ...
    protected boolean tryMoveToTarget(final Creature target, final int range) {
        ...
        if (_pathfindFails >= getMaxPathfindFails()) {
            ...
            if (target.isPlayable()) {
                ...
                if (hate == null || (actor.getReflection() != ReflectionManager.DEFAULT && actor instanceof RaidBossInstance)) // bless freya
                {
                    _pathfindFails = 0;
                    returnHome();
                    return false;
            ...
            if (actor.isInRange(target, MAX_PURSUE_RANGE) && GeoEngine.canSeeTarget(actor, target, false)) {
                _pathfindFails = 0;
            ...
            if(getTeleportTimeout()<System.currentTimeMillis()) {
                _pathfindFails = 0;
                setTeleportTimeout(Long.MAX_VALUE);
                returnHome();
                return false;
    ...
    protected boolean doTask() {
        ...
        switch (currentTask.type) {
            ...
            case ATTACK: {
                ...
                if (!checkTarget(target, MIN_PURSUE_RANGE)) {
                    return true;
                }
                ...
                if(actor.isMoving){
                    if(_castReady<System.currentTimeMillis()-6000L)
                        readyToCast(4000L);
                    return Rnd.chance(5);
                }
            ...
            case CAST: {
                ...
                if (!checkTarget(target, MIN_PURSUE_RANGE + castRange)) {
                    return true;
                }
    ...
    protected void onEvtAttacked(final Creature attacker, final SkillEntry skill, final int damage) {
        ...
        if (player != null) {
            ...
            // 2900 = Ancient Herb - Slayer, работает только на мобов
            if(player.getEffectList().getEffectsBySkillId(2900) != null && actor.isMonster() && actor.getNpcId() >= 22650 && actor.getNpcId() <= 22659)
            {
                actor.getAggroList().addDamageHate(player,(int)actor.getCurrentHp(),0);
                actor.doDie(player);
                return;
            }
        }
        ...
        readyToCast(4000L);
        setRunTimeout(System.currentTimeMillis() + getMaxRunTimeout());
    ...
    protected boolean maybeMoveToHome() {
        final NpcInstance actor = getActor();
        if (actor.isDead() || actor.isMinion() && !actor.getLeader().isDead() || getSuperPoint() != null) {
            return false;
        }
        ...
        if (!actor.moveToLocation(pos.x, pos.y, pos.z, 0, true) && !isInRange || _teleportCounter > 30) {
            teleportHome();
        }
    ...
    protected void returnHome(final boolean clearAggro, final boolean teleport) {
        final NpcInstance actor = getActor();
        final Location sloc = getActor().getSpawnedLoc();
        ...
        if (teleport) {
            if(AiConfig.USE_TELEPORT_SKILL)
                actor.broadcastPacketToOthers(new MagicSkillUse(actor, actor, 2036, 1, 500, 0));
            if (actor instanceof MonsterInstance && ((MonsterInstance)actor).getChampion()==0 && !(actor instanceof RaidBossInstance))
            ...
            //actor.moveToLocation(sloc, 0, true);
        }
    }
    ...
    protected Creature prepareTarget() {
        ...
        for (final Creature cha : hateList) {
            ...
            if (!checkTarget(cha, MIN_PURSUE_RANGE)) {
        ...
        if (skill != null) {
            ...
            if (actor.isMovementDisabled() && distance > skill.getAOECastRange() + 60) {
                ...
                if (skill.isOffensive()) {
                    ...
                    for (final Creature cha : actor.getAggroList().getHateList(MIN_PURSUE_RANGE)) {
    ...
    protected void notifyFriends(final Creature attacker, final int damage) {
        ...
        if (System.currentTimeMillis() - _lastFactionNotifyTime > _minFactionNotifyInterval) {
            ...
            if (actor.isMinion()) {
                ...
                if (master != null && !actor.isIgnoreLeaderAction() && master.isInRange(actor, master.isRaid()? 1500 : 600)) {
            ...
            for (final NpcInstance npc : activeFactionTargets()) {
                npc.getAI().notifyEvent(CtrlEvent.EVT_CLAN_ATTACKED, new Object[]{actor, attacker, damage, null});
            }
        }
    }
    ...
    protected void notifyDead(final Creature attacker) {
        ...
        //Оповестить минионов лидера об атаке
        final List<NpcInstance> minionList = master==null? actor.getPrivatesList().getAlivePrivates() : master.getPrivatesList().getAlivePrivates();
        if (minionList != null) {
            minionList.stream().filter(minion -> minion != actor).forEach(minion -> minion.getAI().notifyEvent(CtrlEvent.EVT_PARTY_DEAD, attacker, actor));
        }
        ...
        World.getAroundNpc(actor).stream().filter(npc -> !npc.isDead() &&  npc.isInFaction(actor) && npc.isInRangeZ(actor, range)).forEach(npc -> npc.getAI().notifyEvent(CtrlEvent.EVT_CLAN_DEAD, attacker, actor));
    }
    ...
    protected List<NpcInstance> activeFactionTargets() {
        ...
        final int range = actor.getFaction().getRange();
        final List<NpcInstance> npcFriends = World.getAroundNpc(actor).stream().filter(npc -> !npc.isDead() && npc.isInFaction(actor) && npc.isInRangeZ(actor, range)).collect(Collectors.toList());
        return npcFriends;
    }
    ...
    protected boolean defaultThinkBuff(final int rateSelf, final int rateFriends) {
        ...
        if (Rnd.chance(rateSelf)) {
            ...
            final SkillEntry[] skills = actorHp < 90 ? selectUsableSkills(actor, 0, _healSkills) : selectUsableSkills(actor, 0, _buffSkills);
            if (skills == null || skills.length == 0) {
                return false;
            }

            final SkillEntry skill = skills[Rnd.get(skills.length)];
            if(actor.getEffectList().getEffectsBySkillId(skill.getId())!=null)
                return false;
        ...
        if (Rnd.chance(rateFriends)) {
            if(actor.isMinion() && actor.getLeader()!= null) {
                NpcInstance leader = actor.getLeader();
                double leaderHp = leader.getCurrentHpPercents();
                SkillEntry[] skills = leaderHp < 95 ? selectUsableSkills(leader, 0, _healSkills) : selectUsableSkills(leader, 0, _buffSkills);
                if (skills != null && skills.length != 0) {
                    SkillEntry skill = skills[Rnd.get(skills.length)];
                    if (leader.getEffectList().getEffectsBySkillId(skill.getId()) != null)
                        return false;
                    addTaskBuff(leader, skill);
                    return true;
                }
            }
            ...
            for (final NpcInstance npc : activeFactionTargets()) {
                ...
                final SkillEntry[] skills = targetHp < 90 ? selectUsableSkills(actor, 0, _healSkills) : selectUsableSkills(actor, 0, _buffSkills);
                if (skills == null || skills.length == 0) {
                    continue;
                }

                final SkillEntry skill = skills[Rnd.get(skills.length)];
                if(npc.getEffectList().getEffectsBySkillId(skill.getId())!=null)
                    continue;
                addTaskBuff(actor, skill);
                return true;
            }
    ...
    protected boolean defaultFightTask() {
        ...
        if(_castReady > System.currentTimeMillis() && defaultThinkBuff(0, 20))
            return true;
        return chooseTaskAndTargets(null, target, distance);
    }

    public boolean isActionsDisabled() {return getActor().isActionsDisabled();}
    ...
    /**
     * Интервал, перед переключением в активный режим после атаки, если цель не атакуется
     * @return
     */
    public int getMaxRunTimeout()
    {
        return 60000;
    }
    ...
    protected void readyToCast(long time){
        _castReady = System.currentTimeMillis() + time;
    }
