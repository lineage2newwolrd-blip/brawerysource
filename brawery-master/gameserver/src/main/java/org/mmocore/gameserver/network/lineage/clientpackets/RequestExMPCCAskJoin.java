...
import org.mmocore.gameserver.network.lineage.serverpackets.ExAskJoinMPCC;
...
public class RequestExMPCCAskJoin extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (target.isInParty() && !target.getParty().isLeader(target)) {
            target = target.getParty().getGroupLeader();
        }
        ...
        if (target.getParty().isInCommandChannel()) {
            activeChar.sendPacket(new SystemMessage(SystemMsg.C1S_PARTY_IS_ALREADY_A_MEMBER_OF_THE_COMMAND_CHANNEL).addString(target.getName()));
            return;
        }

        if (target.isBusy()) {
            activeChar.sendPacket(new SystemMessage(SystemMsg.C1_IS_ON_ANOTHER_TASK).addString(target.getName()));
            return;
        }
        ...
        if (activeParty.isInCommandChannel()) {
            ...
            if (activeParty.getCommandChannel().getGroupLeader() != activeChar) {
                activeChar.sendPacket(SystemMsg.YOU_DO_NOT_HAVE_AUTHORITY_TO_INVITE_SOMEONE_TO_THE_COMMAND_CHANNEL);
                return;
            }
