...
{
	public argos_boss_32(final NpcInstance actor){super(actor);}
			...
			if (code_info.getCode() == 0)
			{
				...
				if (!AiUtils.IsNull(target) && DistFromMe(target) <= 1500)
				{
					if (target.isPlayer())
					{
						...
						{
							GiveItem1(target, 9734, 1);
							SetFlagJournal(target, 67, 16);
							ShowQuestMark(target, 67);
							SoundEffect(target, "ItemSound.quest_middle");
							DeleteItem1(target, 9740, AiUtils.OwnItemCount(target, 9740));
							Say(AiUtils.MakeFString(6754, "", "", "", "", ""));
						}
						else
						{
							Say(AiUtils.MakeFString(6755, "", "", "", "", ""));
						}
					}
				}
			}
	...
	protected void onEvtSpawn()
	{
		...
		if (c0 !=null)
		{
			...
			if (c0.isPlayer())
			{
				Say(AiUtils.MakeFString(6753, c0.getName(), "", "", "", ""));
			}
		}
		super.onEvtSpawn();
	}
