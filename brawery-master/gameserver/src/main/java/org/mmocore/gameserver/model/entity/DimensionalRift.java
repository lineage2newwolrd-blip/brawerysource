...
public class DimensionalRift extends Reflection {
    ...
    public void createNewKillRiftTimer(final long timer) {
    ...
    public void partyMemberInvited(final Player player) {
        createNewKillRiftTimer(100L);
    }

    public void partyMemberExited(final Player player) {
        if(getParty().getMemberCount() == 1 || getPlayersInside(false) < 2) {
            createNewKillRiftTimer(100L);
            return;
        }
        if(getPlayersInside(true) == 0) {
            for(Player p : getParty().getPartyMembers())
                if(p != null && p.getReflection() == DimensionalRift.this)
                    p.broadcastPacket(new SystemMessage(SystemMsg.RESURRECTION_WILL_TAKE_PLACE_IN_THE_WAITING_ROOM_AFTER_S1_SECONDS).addNumber(60));
            createNewKillRiftTimer(60000L);
        }
    }
    ...
    public void memberRevive(Player player)
    {
        if(isCollapseStarted())
            return;
        if(getPlayersInside(true) > 0 && getPlayersInside(false) > 1)
            stopKillRift();
    }
