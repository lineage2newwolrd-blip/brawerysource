...
public class Player extends Playable implements PlayerGroup {
    ...
    private long _shotDischargeTimeStamp = 0;
    ...
    private final PacketThrottler packetThrottler = new PacketThrottler(this);
    ...
    private boolean _cycleEnchant = false;
    ...
    private AbstractCustomObservableEvent _observableEvent;
    /**
    ...
    private boolean _hideVisualFlag;
    ...
    private ScheduledFuture<?> wearTask;
    ...
    public static Player restore(final int objectId) {
        ...
        try {
            ...
            if (rset.next() && rset2.next()) {
                ...
                if(player.getPlayerVariables().get(PlayerVariables.HIDE_VISUAL) != null)
                    player.setHideVisualFlag(true);
    ...
    public PlayerSex getSex() {
        return getPlayerTemplateComponent().getPlayerSex();
    }
    ...
    private void prepareToLogout() {
        ...
        if (ref != ReflectionManager.DEFAULT) {
            if(ref.getReturnLoc() != null) {
                ...
                teleToLocation(_stablePoint, ReflectionManager.DEFAULT);
            }
    ...
    public void purgeCache()
    {
        try
        {
            if(_connection != null)
            {
                CharacterSelectionInfo cl = new CharacterSelectionInfo(_connection.getLogin(), _connection.getSessionKey().playOkID1);
                sendPacket(RestartResponse.OK, cl);
                setTarget(null);
                sendPacket(new CharSelected(this, _connection.getSessionKey().playOkID1));
            }
        }
        catch(Exception e)
        {}
    ...
    private void updateEffectIconsImpl() {
        ...
        final AbstractCustomObservableEvent event = getEvent(AbstractCustomObservableEvent.class);
        if (event != null)
    ...
    public void setEnchantScroll(final ItemInstance scroll) {
        ...
        if(scroll == null) _cycleEnchant = false;
    }

    public void setCycleEnchant(final boolean cycle){ _cycleEnchant = cycle;}

    public boolean isCycleEnchant() { return _cycleEnchant;}
    ...
    public void rewardSkills(final boolean send) {
        ...
        if (AllSettingsConfig.AUTO_LEARN_SKILLS) {
            for (SkillLearn skillLearn : SkillAcquireHolder.getInstance().getMaxSkills(this)) {
...
/*
    ...
    public void setPolyId(final int polyid, final boolean force) {
        _polyNpcId = polyid;
    ...
    private void broadcastCharInfoImpl() {
        ...
        final L2GameServerPacket ci = isPolymorphed() ? new NpcInfo(this) : new CharInfo(this, false);
        final L2GameServerPacket civ = isPolymorphed() ? ci : new CharInfo(this, true);
        ...
        for (final Player player : World.getAroundObservers(this)) {
            if(!isInOlympiadMode() && !player.isHideVisualFlag())
                player.sendPacket(civ, exCi);
            else
    ...
    public void doPickupItem(final GameObject object) {
        ...
        synchronized (item) {
            ...
            if (item.isHerb()) {
                ...
                if (skills.length > 0) {
                    for (final SkillEntry skill : skills) {
                        // Ancient Herb 2900-2903 can be used from 76 lvl
                        if(item.isAncientHerb() && getLevel() < 76)
                            continue;
                        altUseSkill(skill, this);
                        if (getServitor() != null && getServitor().isSummon() && !getServitor().isDead()) {
                            getServitor().altUseSkill(skill, getServitor());
    ...
    public void onMagicUseTimer(final Creature aimingTarget, final SkillEntry skillEntry, final boolean forceUse) {
        if (!skillEntry.getTemplate().isHandler() && !skillEntry.getTemplate().isTrigger()) {
            if (isTransformed() && !getAllSkills().contains(skillEntry)) {
                if(isInFightClub() && skillEntry.getTemplate().getMagicType()==Skill.SkillMagicType.SPECIAL)
                    return;
    ...
    public IBroadcastPacket canUseItemOnEvent(ItemInstance item, boolean force)
    {
    ...
    protected void doPKPVPManage(Creature killer) {
        ...
        if (killer.isPlayer()) {
            ...
            pk.getPvpComponent().onKill(this);
    ...
    public boolean isBusy() {
        return isProcessingRequest() || isOutOfControl() || isInOlympiadMode() || denyOnPvpEvent() || isInStoreMode() || isInDuel() || getMessageRefusal() || isBlockAll() || isInvisible();
    }

    public boolean denyOnPvpEvent() {
        if (EventsConfig.AllowJoinParty) {
            return false;
        }
    ...
    public List<L2GameServerPacket> addPacketList(final Player forPlayer, final Creature dropper) {
        ...
        if (forPlayer.getObjectId() != getObjectId()) {
            list.add(isPolymorphed() ? new NpcInfo(this) : !isInOlympiadMode() && !forPlayer.isHideVisualFlag() ? new CharInfo(this, true) : new CharInfo(this, false));
        }
        ...
        if (getPrivateStoreType() != STORE_PRIVATE_NONE) {
                list.add(getPrivateStoreMsgPacket());
    ...
    public void scheduleDelete() {
        ...
        final long time = getPlayerVariables().getInt(PlayerVariables.NO_CARRIER_TIME, 0) * 1000;
        if (time <= 0) {
            stopMove();
            prepareToLogout();
            deleteMe();
    ...
    public L2GameServerPacket getPrivateStoreMsgPacket()
    {
        switch (getPrivateStoreType())
        {
            ...
            case STORE_PRIVATE_SELL_PACKAGE:
                return new ExPrivateStoreWholeMsg(this);
    ...
    public UnitMember getClanMembership() {
        return getSubUnit() == null ? null : getSubUnit().getUnitMember(getObjectId());
    }
    ...
    public void setPlayerAccess(final PlayerAccess pa) {
        ...
        setAccessLevel(isGM() || playerAccess.Menu ? 1 : 0);
    }
...
//			boolean checkRemove = IntStream.of(slotsId).anyMatch(slotId -> _cubics::remove != null);
    ...
    public boolean unChargeShots(final boolean spirit) {
        ...
        if (spirit) {
            ...
            _shotDischargeTimeStamp = System.currentTimeMillis();
        } else {
    ...
    public boolean canChargeShot() {
        return System.currentTimeMillis() - _shotDischargeTimeStamp >= AllSettingsConfig.ALT_SHOT_CORRECTION;
    }

    public void shotsReady()
    {
        _shotDischargeTimeStamp = 0;
    }
    ...
    public void enterMatchObserverMode(Location loc, Reflection reflect, final AbstractCustomObservableEvent event, int mode)
    {
        final WorldRegion observerRegion = World.getRegion(loc);
        if(observerRegion == null)
            return;

        final AbstractCustomObservableEvent oldEvent = getObservableEvent();
        if (!_observerMode.compareAndSet(oldEvent != null ? OBSERVER_STARTED : OBSERVER_NONE, OBSERVER_STARTING)) {
            return;
        }
        ...
        else
        {
            ...
            // Меняем интерфейс
            sendPacket(new ExEventMatchObserver(mode, event.getTeamName1(), event.getTeamName2()));
            _observePoint = new ObservePoint(this, true);
        }
        _observePoint.setLoc(loc);
        _observePoint.setReflection(reflect);

        setObservableEvent(event);

        // "Телепортируемся"
        sendPacket(new TeleportToLocation(this, loc));
    }

    public void leaveMatchObserverMode()
    {
        final AbstractCustomObservableEvent event = getObservableEvent();
        if (event == null) {
            return;
        }
        if (!_observerMode.compareAndSet(OBSERVER_STARTED, OBSERVER_LEAVING))
        ...
        // Меняем интерфейс
        sendPacket(new ExEventMatchObserver(0));
        updateEffectIcons();
    ...
    public void updateNobleSkills() {
        if (isNoble()) {
            ...
            if (OtherCustom.addNoblesseBlessing)
                super.addSkill(SkillTable.getInstance().getSkillEntry(Skill.SKILL_NOBLESSE_BLESSING, 1));
            else
    ...
    public void doRevive() {
        ...
        autoShot();
        if(isInParty() && getParty().isInReflection() && getParty().getReflection() instanceof DimensionalRift)
            ((DimensionalRift) getParty().getReflection()).memberRevive(this);
    }
    ...
    public int getRunSpeed() {
        ...
        } else if(isInObserverMode() && getObservePoint()!= null){
            return getObservePoint().getMoveSpeed();
        }
    ...
    public boolean isFalling() {
        return System.currentTimeMillis() - _lastFalling < 3000;
    }

    public void falling(final int height) {
        _lastFalling = System.currentTimeMillis();
        //падение для валидации учитываем всегда, а урон считаем только при соответствии условиям
    ...
    protected void onUpdateZones(final List<Zone> leaving, final List<Zone> entering) {
        ...
        if(isActionBlocked(Zone.BLOCKED_ACTION_MOVE)) {
            stopMove();
            sendPacket(SystemMsg.YOU_CANNOT_MOVE__YOU_ARE_TOO_ENCUMBERED, ActionFail.STATIC);
        }
    }
    ...
    {
        return getEvent(SingleMatchEvent.class) != null;
    }
    ...
    public boolean isInMeleeTransform() {
        return isTransformed() && getTransformation().getCommon().base_attack_type != TBaseAttackType.bow;
    }
    ...
    public void setObservableEvent(final AbstractCustomObservableEvent observableEvent) {
        _observableEvent = observableEvent;
    }

    public AbstractCustomObservableEvent getObservableEvent() {
    ...
    public int getClassId() {
        return getPlayerClassComponent().getClassId().getId();
    }

    @Override
    public int isSubClassActive() {
        return getPlayerClassComponent().isSubClassActive() ? 1 : 0;
    }

    @Override
    public int getRaceId() {
        return getPlayerClassComponent().getClassId().getRace().getId();
    }
    ...
    public boolean isHideVisualFlag() {
        return _hideVisualFlag;
    }

    public void setHideVisualFlag(boolean v) {
        _hideVisualFlag = v;
    }
        ...
        if (olympiadEnchant) {
            for (ItemInstance item : getInventory().getItems()) {
                if (!item.canBeEnchanted())
                    continue;
                if (item.isWeapon())
                    item.setTemporaryEnchant(getTempOlyEnch(item, OtherCustom.enchantOlyWeapon));
                if (item.isArmor())
                    item.setTemporaryEnchant(getTempOlyEnch(item, OtherCustom.enchantOlyArmor));
                if (item.isAccessory())
                    item.setTemporaryEnchant(getTempOlyEnch(item, OtherCustom.enchantOlyJewelry));
            }
        } else {
            for (ItemInstance item : getInventory().getItems())
    ...
    private int getTempOlyEnch(ItemInstance item, int configLvl) {
        return OtherCustom.enchantOlyExceptLowerItems ? Math.min(item.getEnchantLevel(), configLvl) : configLvl;
    }
    ...
    public void setWearTask(ScheduledFuture task)
    {
        this.wearTask = task;
    }

    public ScheduledFuture getWearTask()
    {
        return wearTask;
    }
