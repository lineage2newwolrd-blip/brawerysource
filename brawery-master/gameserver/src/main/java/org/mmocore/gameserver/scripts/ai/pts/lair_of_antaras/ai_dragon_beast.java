...
public class ai_dragon_beast extends ai_antaras_cave_raid_basic {
    public static final int USE_SKILL03_TIME = 4000;
    private static final int SPLIT_TIME = 4001;
    private int i_ai0;
    private int i_ai6;
    private static final SkillEntry fierce_attack = SkillTable.getInstance().getSkillEntry(6736, 1);
    ...
    protected void onEvtTimerFiredEx(int timer_id, Object arg1, Object arg2) {
        ...
        if (timer_id == USE_SKILL03_TIME) {
            if(i_ai0 == 1){
                AddTimerEx(USE_SKILL03_TIME, (30 + Rnd.get(30)) * 1000);

                final Creature agrTarget = getActor().getAggroList().getRandomHated();
                if (agrTarget != null) {
                    addTaskCast(agrTarget, fierce_attack);
                }
            }

        }
        else if (timer_id == SPLIT_TIME) {
