...
public class _226_TestOfHealer extends Quest {
    ...
    public _226_TestOfHealer() {
        ...
        addLevelCheck(39);
        addClassIdCheck(ClassId.knight, ClassId.cleric, ClassId.oracle, ClassId.elven_knight);
    }
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        if (st.ownItemCount(MARK_OF_HEALER_ID) > 0) {
            st.exitQuest(true);
            return "completed";
        }
        ...
        if (npcId == Bandellos) {
            if (cond == 0) {
                ...
                if (st.ownItemCount(GOLDEN_STATUE_ID) == 0) {
                    ...
                    if (!st.getPlayer().getPlayerVariables().getBoolean(PlayerVariables.PROF2_3)) {
                        ...
                        st.giveReward(ADENA_ID, 266980, true);
            ...
            if (cond == COND10 && st.ownItemCount(ORDER_OF_SORIUS_ID) > 0) {
                htmltext = "30674-01.htm";
            } else if (cond == COND12 && st.ownItemCount(SECRET_LETTER1_ID) > 0) {
    ...
    public String onKill(NpcInstance npc, QuestState st) {
        ...
        if (st.getCond() == d[0] && (d[2] == 0 || st.ownItemCount(d[2]) == 0)) {
            if (d[2] != 0) {
                st.giveItems(d[2], 1);
            }
            st.setCond(d[1]);
