...
public class CommunityRefine extends ScriptBbsHandler {
    ...
    private static final String indicator = "<table border=0 cellspacing=0 cellpadding=0 width=96 height=16 background=\"L2UI_CT1.ListCTRL_DF_Title_Down\"><tr><td align=center>Selected</tr></td></table>";
    private static final String item_button_indicator = "<button action=\"%button_bypass%\" value=\"%button_value%\" width=96 height=16 back=\"L2UI_CT1.ListCTRL_DF_Title_Down\" fore=\"L2UI_CT1.ListCTRL_DF_Title\"/>";
    ...
    public void showMainPage(Player player,  int page) {
        String html = getCache().getHtml(CBasicConfig.BBS_PATH + "/cabinet/refine/main.htm", player);
    ...
    private String getItemLine(String prefixDesc, int slot, Player player) {
        String itemTemplateObj = getCache().getHtml(CBasicConfig.BBS_PATH + "/cabinet/refine/item_object.htm", player);
    ...
    private String getIndicator(Player player, int slot) {
        if(player.getRefineComponent().getSelectedSlotId() == slot){
            return indicator;
        }
        return item_button_indicator.replaceFirst("%button_bypass%", getItemButtonBypass(slot)).replaceFirst("%button_value%", player.isLangRus() ? "Выбрать":"Choose");
    ...
    private String getSkillsHtml(Player player, int page) {
        ...
        String skillTemplateObj = getCache().getHtml(CBasicConfig.BBS_PATH + "/cabinet/refine/skill_object.htm", player);
    ...
    private String getPriceItemName(Player player) {
        ItemTemplate template = ItemTemplateHolder.getInstance().getTemplate(getPriceId(player));
        if (template == null)
            return "";
    ...
    private int getPriceId(Player player) {
        ...
        if (targetItem == null)
            ...
            return CServiceConfig.refineWeaponItem;
        else if (targetItem.isAccessory())
            return CServiceConfig.refineJewelryItem;
        else
            return 0;
    }
    ...
    private int getPriceCount(Player player) {
        ...
        if (targetItem == null)
            ...
            return CServiceConfig.refineWeaponPrice;
        else if (targetItem.isAccessory())
            return CServiceConfig.refineJewelryPrice;
        else
            return 0;
    }
