	...
	protected boolean useItemImpl(Player player, ItemInstance item, boolean ctrl)
	{
		...
		switch(itemId)
		{
			...
			case 7906:
				...
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2248, 1));
				break;
			//Blessing of Water	
			case 7907:
				player.broadcastPacket(new MagicSkillUse(player, player, 2249, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2249, 1));
				break;
			//Blessing of Wind
			case 7908:
				player.broadcastPacket(new MagicSkillUse(player, player, 2250, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2250, 1));
				break;
			//Blessing of Earth
			case 7909:
				player.broadcastPacket(new MagicSkillUse(player, player, 2251, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2251, 1));
				break;
			//Blessing of Darkness
			case 7910:
				player.broadcastPacket(new MagicSkillUse(player, player, 2252, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2252, 1));
				break;
			//Blessing of Sanctity
			case 7911:
				player.broadcastPacket(new MagicSkillUse(player, player, 2253, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2253, 1));
				break;
			//Fire Resist Potion
			case 9997:
				player.broadcastPacket(new MagicSkillUse(player, player, 2335, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2335, 1));
				break;
			//Water Resist Potion
			case 9998:
				player.broadcastPacket(new MagicSkillUse(player, player, 2336, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2336, 1));
				break;
			//Earth Resist Potion
			case 9999:
				player.broadcastPacket(new MagicSkillUse(player, player, 2338, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2338, 1));
				break;
			//Wind Resist Potion
			case 10000:
				player.broadcastPacket(new MagicSkillUse(player, player, 2337, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2337, 1));
				break;
			//Darkness Resist Potion
			case 10001:
				player.broadcastPacket(new MagicSkillUse(player, player, 2340, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2340, 1));
				break;
			//Holy Resist Potion
			case 10002:
				player.broadcastPacket(new MagicSkillUse(player, player, 2339, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2339, 1));
				break;
			//Warrior's Temporary Healing Potion
			case 13750:
				player.broadcastPacket(new MagicSkillUse(player, player, 2592, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(2592, 1));
				break;
			// Christmas Red Sock
			case 14612:
				player.broadcastPacket(new MagicSkillUse(player, player, 23017, 1, 0, 0));
				player.altOnMagicUseTimer(player, SkillTable.getInstance().getSkillEntry(23017, 1));
				break;
			default:
				return false;
		}
