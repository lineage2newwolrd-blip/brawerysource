...
public abstract class RewardListInfo {
	...
	public static void ratedGroupedRewardList(StringBuilder tmp, NpcInstance npc, RewardList list, Player player, double mod) {
		...
		for (RewardGroup g : list) {
			...
			double rateDrop = (npc instanceof RaidBossInstance ? ServerConfig.RATE_DROP_RAIDBOSS : npc.isSiegeGuard() ? ServerConfig.RATE_DROP_SIEGE_GUARD : g.isHerb() ? ServerConfig.RATE_DROP_HERB * player.getRateItems() : ServerConfig.RATE_DROP_ITEMS * player.getRateItems());
			...
			int gmult = (int) Math.ceil(gchance * grate / RewardList.MAX_CHANCE);
			tmp.append("<tr><td><img src=\"L2UI.SquareBlank\" width=270 height=10> </td></tr>");
			tmp.append("<tr><td>");
			tmp.append("<table width=270 border=0 bgcolor=333333>");
			if (ServerConfig.ALT_MULTI_DROP && !g.isAdena() && gmult > 1)
				tmp.append("<tr><td width=170><font color=\"a2a0a2\">Group Chance: </font><font color=\"b09979\">[").append(gmult - 1).append("..").append(gmult).append("]</font></td>");
			else
				tmp.append("<tr><td width=170><font color=\"a2a0a2\">Group Chance: </font><font color=\"b09979\">").append(pf.format(gchance * (g.isAdena() ? 1.0 : grate) / RewardList.MAX_CHANCE)).append("</font></td>");
			...
			for (RewardData d : items) {
				long minDrop = g.isAdena() ? Math.round(d.getMinDrop() * grate) : d.getMinDrop();
				long maxDrop = g.isAdena() ? Math.round(d.getMaxDrop() * grate) : d.getMaxDrop();
				...
				tmp.append("<font color=\"b09979\">[").append(minDrop).append("..").append(maxDrop).append("]&nbsp;");
