...
public class OnShiftAction_NpcInstance extends ScriptOnShiftActionHandler<NpcInstance> {
	...
	public boolean call(NpcInstance npc, Player player) {
		...
		if(player.isGM()) {
			if(npc.getAI().isPTSAI()) {
				dialog = dialog.replaceFirst("%AI%",
						String.valueOf(npc.getAI()) +
								",<br1><a action=\"bypass -h htmbypass_actions.OnActionShift:aiparams\">AI Params</a>" +
								"<br1>state: " + ((NpcAI) npc.getAI()).stateAsString());
			}
			else {
	...
	public void effects(Player player, NpcInstance npc, String[] par) {
		...
		if(!list.isEmpty()) {
			for(Effect e : list) {
				if(!e.isHidden() && e.isTimeLeft())
					dialog.append("<table width=250><tr><td fixwidth=34><img src=\"").append(e.getSkill().getTemplate().getIcon()).append("\" width=32 height=32></td><td width=200 align=left>").
							append(e.getSkill().getName()).append(" <font color=\"LEVEL\">").append(e.getSkill().getLevel()).append(" level</font>").append(" <font color=FF0000>").append(e.getTimeLeft()).
							append(player.isLangRus() ? " сек</font></td></tr></table><br1>" : " sec</font></td></tr></table><br1>");
			}
		}
	...
	public void aggro(Player player, NpcInstance npc, String[] par) {
		...
		Set<AggroList.HateInfo> set = new TreeSet<>(AggroList.HateComparator.getInstance().reversed());
	...
	@Bypass("actions.OnActionShift:aiparams")
	public void ai_params(Player player, NpcInstance npc, String[] par) {
		if(player == null || npc == null)
			return;
		String dialog = HtmCache.getInstance().getHtml("scripts/OnActionShift/aiparams.htm", player);
		if(npc.getAI() instanceof NpcAI) {
			NpcAI ai = ((NpcAI) npc.getAI());
			int i = 0;
			for(String param : ai.onShiftMap.keySet()) {
				if(i == 20)
					break;
				dialog = dialog.replaceFirst("%param" + i + "%", param);
				dialog = dialog.replaceFirst("%value" + i + "%", ai.onShiftMap.get(param));
				i++;
			}
			while(i<=20){
				dialog = dialog.replaceFirst("%param" + i + "%", "");
				dialog = dialog.replaceFirst("%value" + i + "%", "");
				i++;
			}
		}
		HtmlMessage msg = new HtmlMessage(npc);
