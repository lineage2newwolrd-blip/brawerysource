...
public class ItemBroker
{
	...
	public void main(Player player, NpcInstance npc, String[] var)
	{
		...
		HtmlMessage htmlMessage = new HtmlMessage(npc).setFile("scripts/services/itembroker/main.htm");
		htmlMessage.addVariable("type", type);
	...
	public void equipment(Player player, NpcInstance npc, String[] var)
	{
		...
		HtmlMessage htmlMessage = new HtmlMessage(npc).setFile("scripts/services/itembroker/equipment.htm");
		htmlMessage.addVariable("type", type);
	...
	public void equipment4(Player player, NpcInstance npc, String[] var)
	{
		...
		HtmlMessage htmlMessage = new HtmlMessage(npc).setFile("scripts/services/itembroker/equipment4.htm");
		htmlMessage.addVariable("type", type);
	...
	public void list(Player player, NpcInstance npc, String[] var)
	{
		...
		for(TreeMap<Long, Item> tempItems : allItems.values())
		{
			...
			for(Entry<Long, Item> entry : tempItems.entrySet())
			{
				...
				if(tempItem.enchant < minEnchant)
					continue;
				ItemTemplate temp = tempItem.item != null ? tempItem.item.getItem() : ItemTemplateHolder.getInstance().getTemplate(tempItem.itemId);
		...
		out.append("<font color=\"FF9900\">[npc_%objectId%_services.ItemBroker:main ");
		out.append(type);
		out.append("|««]&nbsp;&nbsp;");
		...
		out.append("</font><table width=100%>");
		...
		if(items.size() > 0)
		{
			...
			{
				Item item = iter.next();
				ItemTemplate temp = item.item != null ? item.item.getItem() : ItemTemplateHolder.getInstance().getTemplate(item.itemId);
				if(temp == null)
					continue;
	...
	public void path(Player player, NpcInstance npc, String[] var)
	{
		...
		RadarControl rc = new RadarControl(RadarControl.RadarState.SHOW_RADAR, RadarControl.RadarType.ARROW, item.player);
		player.sendPacket(rc);

		// Показываем игроку торговца, если тот скрыт
		if(player.getPlayerVariables().getBoolean(PlayerVariables.NO_TRADERS))
		{
			player.sendPacket(new CharInfo(trader, !player.isHideVisualFlag()));
			player.sendPacket(trader.getPrivateStoreMsgPacket());
		}
	...
	{
		NpcInfo info = _npcInfos.get(npc.getObjectId());
		long refresh = System.currentTimeMillis() - 1000L * ServicesConfig.ITEM_BROKER_REFRESH_DELAY;
		if(info == null || info.lastUpdate < refresh)
		{
			info = new NpcInfo();
				...
				switch(type)
				{
					...
					case Player.STORE_PRIVATE_MANUFACTURE:
						...
						for(ManufactureItem mitem : createList)
						{
							...
							Recipe recipe = RecipeHolder.getInstance().getRecipeId(recipeId);
							if(recipe == null)
								continue;

							ItemTemplate temp = ItemTemplateHolder.getInstance().getTemplate(recipe.getItemId());
							if(temp == null)
								continue;
							TreeMap<Long, Item> oldItems = items.get(temp.getName());
							if(oldItems == null)
