...
public class AutoAcpService
{
	...
	public void setAuto(final Player player, final NpcInstance npc, final String[] arg)
	{
		...
		{
			final Acp acp = Acp.valueOf(arg[0]);
			double percent = Integer.parseInt(arg[1]);
			boolean validUse = true;
			if(AcpConfig.acpUsePremium || AcpConfig.acpUseCert){
				validUse = (AcpConfig.acpUsePremium && player.getPremiumAccountComponent().hasBonus()) || (AcpConfig.acpUseCert && !player.getInventory().getItemsByItemId(AcpConfig.certItemId).isEmpty());
			}
			if(!validUse)
			{
				if(AcpConfig.acpUsePremium)
					player.sendMessage(new CustomMessage("component.acp.premium").toString(player));
				else if(AcpConfig.acpUseCert)
					player.sendMessage(new CustomMessage("component.acp.certificate").toString(player));
				acp.setAuto(player, false);
				acp.start(player, false);
				generateHtml(player);
				return;
			}
			if(percent > 0) {
				acp.setPercent(player, percent);
				acp.setAuto(player, true);
				acp.start(player, true);
				generateHtml(player);
			}
			else{
				acp.setAuto(player, false);
	...
	public static void generateHtml(final Player activeChar)
	{
		...
		// int auto_hp_percent = (int) activeChar.getAcpComponent().getTemplate().getHpPercent();
		//final int auto_mp_percent = (int) activeChar.getAcpComponent().getTemplate().getMpPercent();
		final String auto_cp = !activeChar.getAcpComponent().getTemplate().isAutoCp() ? OFF : "<font color=\"669900\">" + (int)activeChar.getAcpComponent().getTemplate().getCpPercent() + "%</font>" ;
		final String auto_small_cp = !activeChar.getAcpComponent().getTemplate().isAutoSmallCp() ? OFF : "<font color=\"669900\">" + (int)activeChar.getAcpComponent().getTemplate().getSmallCpPercent() + "%</font>";
		final String auto_hp = !activeChar.getAcpComponent().getTemplate().isAutoHp() ? OFF : "<font color=\"669900\">" + (int)activeChar.getAcpComponent().getTemplate().getHpPercent() + "%</font>";
		final String auto_mp = !activeChar.getAcpComponent().getTemplate().isAutoMp() ? OFF : "<font color=\"669900\">" + (int)activeChar.getAcpComponent().getTemplate().getMpPercent() + "%</font>";
		final String auto_small_cp_str_boolean = !activeChar.getAcpComponent().getTemplate().isAutoSmallCp() ? "<button action=\"" + "bypass -h htmbypass_services.AutoACPService:setAuto small_cp $percent_small_cp" + "\" value=\"SET\" fore=\"L2UI_CT1.ListCTRL_DF_Title\" back=\"l2ui_ct1.ListCTRL_DF_Title_Down\" width=\"32\" height=\"16\">" : "<button action=\"" + "bypass -h htmbypass_services.AutoACPService:setAuto small_cp 0" + "\" value=\"OFF\" fore=\"L2UI_CT1.ListCTRL_DF_Title\" back=\"l2ui_ct1.ListCTRL_DF_Title_Down\" width=\"32\" height=\"16\">";
		final String auto_cp_str_boolean = !activeChar.getAcpComponent().getTemplate().isAutoCp() ? "<button action=\"" + "bypass -h htmbypass_services.AutoACPService:setAuto cp $percent_cp" + "\" value=\"SET\" fore=\"L2UI_CT1.ListCTRL_DF_Title\" back=\"l2ui_ct1.ListCTRL_DF_Title_Down\" width=\"32\" height=\"16\">" : "<button action=\"" + "bypass -h htmbypass_services.AutoACPService:setAuto cp 0" + "\" value=\"OFF\" fore=\"L2UI_CT1.ListCTRL_DF_Title\" back=\"l2ui_ct1.ListCTRL_DF_Title_Down\" width=\"32\" height=\"16\">";
		final String auto_hp_str_boolean = !activeChar.getAcpComponent().getTemplate().isAutoHp() ? "<button action=\"" + "bypass -h htmbypass_services.AutoACPService:setAuto hp $percent_hp" + "\" value=\"SET\" fore=\"L2UI_CT1.ListCTRL_DF_Title\" back=\"l2ui_ct1.ListCTRL_DF_Title_Down\" width=\"32\" height=\"16\">" : "<button action=\"" + "bypass -h htmbypass_services.AutoACPService:setAuto hp 0" + "\" value=\"OFF\" fore=\"L2UI_CT1.ListCTRL_DF_Title\" back=\"l2ui_ct1.ListCTRL_DF_Title_Down\" width=\"32\" height=\"16\">";
		final String auto_mp_str_boolean = !activeChar.getAcpComponent().getTemplate().isAutoMp() ? "<button action=\"" + "bypass -h htmbypass_services.AutoACPService:setAuto mp $percent_mp" + "\" value=\"SET\" fore=\"L2UI_CT1.ListCTRL_DF_Title\" back=\"l2ui_ct1.ListCTRL_DF_Title_Down\" width=\"32\" height=\"16\">" : "<button action=\"" + "bypass -h htmbypass_services.AutoACPService:setAuto mp 0" + "\" value=\"OFF\" fore=\"L2UI_CT1.ListCTRL_DF_Title\" back=\"l2ui_ct1.ListCTRL_DF_Title_Down\" width=\"32\" height=\"16\">";
	...
	public static void generateReuse(final Player activeChar)
	{
		...
		final double auto_hp_reuse = activeChar.getAcpComponent().getTemplate().getReuseHp() / 1000.;
		final double auto_mp_reuse = activeChar.getAcpComponent().getTemplate().getReuseMp() / 1000.;
		final HtmlMessage dialog = new HtmlMessage(5);
	...
	public static void generateShowItem(final Player activeChar, final Acp acp)
	{
		...
		for(final int item : list)
		{
			...
			if(itemNameLine.getName().length() > 60)
			{
				final Optional<ItemNameLine> customFunction = itemNameLine.getFunction("acp");
				block = block.replace("{name}", customFunction.isPresent() ? customFunction.get().getName() : "");
			}
			else
			{
				block = block.replace("{name}", itemNameLine.getName());
			}
			block = block.replace("{count}", finalCountItemStr);
			listStr += block;
			count++;
		}
